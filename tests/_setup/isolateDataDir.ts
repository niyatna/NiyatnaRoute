...
// but every process resolves DATA_DIR to the SAME default (~/.niyatnaroute) when the env
...
if (!process.env.DATA_DIR) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-test-"));
...
process.env.NIYATNAROUTE_SKIP_SYSTEM_TRUST = "1";
...
process.env.NIYATNAROUTE_SKIP_DNS_WRITE = "1";
