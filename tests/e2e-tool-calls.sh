...
NIYATNA_URL="${NIYATNA_URL:-http://localhost:20128}"
MODEL="${1:-${NIYATNA_MODEL:-}}"
AUTH_TOKEN="${NIYATNA_AUTH_TOKEN:-dummy}"
FAILURES=0
...
echo "URL: $NIYATNA_URL | Model: $MODEL"

echo
echo "[TEST 1] Non-streaming tool call integrity..."
response=$(curl -fsS -X POST "$NIYATNA_URL/v1/chat/completions" \
...
stream_output=$(curl -fsS -X POST "$NIYATNA_URL/v1/chat/completions" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
  -d @- <<JSON
{
...
models=$(curl -fsS "$NIYATNA_URL/v1/models" \
  -H "Authorization: Bearer $AUTH_TOKEN" \
