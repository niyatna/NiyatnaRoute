...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-smelt-settings-"));
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        ...
        assert.equal(written._managedBy, "niyatnaroute");
...
// ── Test 5: DELETE → removes NiyatnaRoute fields ────────────────────────────────

test("smelt-settings DELETE: removes NiyatnaRoute fields from existing config", async () => {
