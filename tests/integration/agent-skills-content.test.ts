 ...
 *  2. Zero niyatnaroute-* folders remain (post-prune: old niyatnaroute-* skill dirs were removed).
...
// ── §2: No niyatnaroute-* directories remain ────────────────────────────────────

test("skills/ has zero niyatnaroute-* directories (all pruned)", () => {
  if (!fs.existsSync(SKILLS_DIR)) {
    // If skills dir doesn't exist at all, nothing to prune
    return;
  }
  const entries = fs.readdirSync(SKILLS_DIR, { withFileTypes: true });
  const niyatnaRouteDirs = entries
    .filter((e) => e.isDirectory() && e.name.startsWith("niyatnaroute-"))
    .map((e) => e.name);
  assert.deepEqual(
    niyatnaRouteDirs,
    [],
    `Found niyatnaroute-* directories that should have been pruned: ${niyatnaRouteDirs.join(", ")}`,
  );
});
