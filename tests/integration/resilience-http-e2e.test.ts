...
function createServerProcess(dataDir: string, port: number) {
  ...
  const child = spawn(process.execPath, ["scripts/dev/run-next-playwright.mjs", "dev"], {
    ...
    env: {
      ...
      NIYATNA_DISABLE_BACKGROUND_SERVICES: "true",
      NIYATNA_DISABLE_TOKEN_HEALTHCHECK: "true",
      NIYATNA_DISABLE_LOCAL_HEALTHCHECK: "true",
      NIYATNA_HIDE_HEALTHCHECK_LOGS: "true",
      NIYATNA_E2E_BOOTSTRAP_MODE: "open",
    },
