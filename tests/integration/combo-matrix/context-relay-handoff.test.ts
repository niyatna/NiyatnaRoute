...
//      - request carries x-niyatnaroute-session-id header
...
// reads "x-session-id" (not "x-niyatnaroute-session-id") and prefixes the result with "ext:".
...
//     internal summary request (identified by _niyatnarouteInternalRequest flag in body)
...
function installHandoffAwareFetch() {
  ...
  globalThis.fetch = async (url: any, init: any = {}) => {
    ...
    if (bodyObj._niyatnarouteInternalRequest === "universal-handoff") {
...
// Without the x-niyatnaroute-session-id header, sessionId = null → block is skipped.
test("context-relay universal handoff: does NOT fire when x-niyatnaroute-session-id header is absent", async () => {
  ...
  assert.equal(
    handoff,
    null,
    "handoff must NOT be written when x-niyatnaroute-session-id header is absent (sessionId gate)"
  );
