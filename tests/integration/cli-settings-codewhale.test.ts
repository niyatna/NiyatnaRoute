...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-codewhale-settings-"));
...
test("codewhale-settings POST: writes primary ~/.codewhale/config.toml for a fresh install", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      assert.ok(content.includes("managed by NiyatnaRoute"), "Config should have NiyatnaRoute marker");
...
test("codewhale-settings GET: falls back to legacy ~/.deepseek/config.toml when primary is absent", async () => {
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      '# managed by NiyatnaRoute (plan 14)\n[openai]\nbase_url = "http://localhost:20128"\n'
    );
    ...
    const body = await res.json();
    if (body.config) {
      assert.ok(body.config.includes("managed by NiyatnaRoute"));
      assert.equal(body.hasNiyatnaRoute, true);
    }
  } finally {
    process.env.HOME = origHome;
    fs.rmSync(tmpHome, { recursive: true, force: true });
