...
// coletava tests/unit/docs/) e apodreceu — filtrava dirs `niyatnaroute*`, mas os skills
...
// invariante de uso é "referencia as env vars ($NIYATNAROUTE_URL/NIYATNAROUTE_KEY) OU
// comandos da CLI (`niyatnaroute …`)" — 3 skills (health/keys/batches) usam só a CLI.
...
test("each skill dir has SKILL.md with frontmatter", async () => {
  ...
  for (const dir of dirs) {
    ...
    if (!dir.startsWith("omni-")) {
      assert.ok(
        content.includes("NIYATNAROUTE_") || content.includes("niyatnaroute "),
        `${dir}: missing usage references (NIYATNAROUTE_* env vars or niyatnaroute CLI commands)`
      );
    }
  }
});
