...
test("#9123: setting ONLY NIYATNA_ALLOW_LOCAL_PROVIDER_URLS=true should relax the guard used for search-provider outbound calls (currently does not)", async () => {
  const { areLocalProviderUrlsAllowed, getProviderOutboundGuard } = await import(
    "../../src/shared/network/outboundUrlGuardPolicy.ts"
  );

  withEnv({ NIYATNA_ALLOW_LOCAL_PROVIDER_URLS: "true" }, () => {
    assert.equal(areLocalProviderUrlsAllowed(), true);
    assert.notEqual(
      getProviderOutboundGuard(),
      "public-only",
      "BUG #9123: NIYATNA_ALLOW_LOCAL_PROVIDER_URLS should relax getProviderOutboundGuard() " +
...
test("#9123 control: NIYATNA_ALLOW_PRIVATE_PROVIDER_URLS=true DOES relax the same guard", async () => {
  const { getProviderOutboundGuard } = await import(
    "../../src/shared/network/outboundUrlGuardPolicy.ts"
  );
  withEnv({ NIYATNA_ALLOW_PRIVATE_PROVIDER_URLS: "true" }, () => {
