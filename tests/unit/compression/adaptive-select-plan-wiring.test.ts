...
import {
  ...
  shouldAutoTrigger,
} from "@niyatna/open-sse/services/compression/strategySelector.ts";
import { getDefaultCompressionConfig } from "@niyatna/open-sse/services/compression/stats.ts";
import type { CompressionConfig } from "@niyatna/open-sse/services/compression/types.ts";
...
test("adaptive floor: bypasses auto-trigger, escalates a base plan to fit", () => {
  ...
  const tel: {
    value: import("@niyatna/open-sse/services/compression/adaptiveCompression/types.ts").AdaptiveTelemetry | null;
  } = { value: null };
