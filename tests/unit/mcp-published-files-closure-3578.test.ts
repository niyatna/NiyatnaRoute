...
// #3578 — `niyatnaroute --mcp` crashed on npm installs with ERR_MODULE_NOT_FOUND for
...
function resolveImport(fromFile: string, spec: string): string | null {
  ...
  else if (spec.startsWith("@niyatnaroute/open-sse/"))
    base = path.join("open-sse", spec.slice("@niyatnaroute/open-sse/".length));
  else if (spec === "@niyatnaroute/open-sse") base = path.join("open-sse", "index");
