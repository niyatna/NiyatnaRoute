...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-api-auth-"));
...
test("getApiKeyMetadata recognizes NIYATNAROUTE_API_KEY environment variable", async () => {
...
test("requireManagementAuth returns null for NIYATNAROUTE_API_KEY env passthrough", async () => {
  await setupAuth();
