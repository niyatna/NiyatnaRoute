...
const BIN = path.join(ROOT, "bin", "niyatnaroute.mjs");
...
test("CLI data-dir resolver preserves an existing legacy ~/.niyatnaroute before XDG/app data", async () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-cli-data-dir-"));
  const home = path.join(tmp, "home");
  const legacyDir = path.join(home, ".niyatnaroute");
...
test("CLI startup loads later non-conflicting .env files without overriding earlier values", () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-cli-env-layers-"));
  const home = path.join(tmp, "home");
  const dataDir = path.join(tmp, "data");
  const cwd = path.join(tmp, "cwd");
  const appDataDir =
    process.platform === "win32"
      ? path.join(tmp, "appdata", "niyatnaroute")
      : path.join(home, ".niyatnaroute");
  ...
  try {
    ...
    fs.writeFileSync(
      ...
      "NIYATNAROUTE_BASE_URL=https://data.example/v1\n",
      "utf-8"
    );
    fs.writeFileSync(
      path.join(appDataDir, ".env"),
      ["NIYATNAROUTE_BASE_URL=https://appdata.example/v1", "NIYATNAROUTE_HTTP_TIMEOUT_MS=1234", ""].join(
        "\n"
      ),
      "utf-8"
    );
    fs.writeFileSync(
      path.join(cwd, ".env"),
      ["NIYATNAROUTE_BASE_URL=https://cwd.example/v1", "PORT=34567", ""].join("\n"),
      "utf-8"
    );
    ...
    assert.equal(current.NIYATNAROUTE_BASE_URL, "https://data.example/v1");
    assert.equal(current.NIYATNAROUTE_HTTP_TIMEOUT_MS, "1234");
