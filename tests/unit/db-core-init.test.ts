...
function cleanupGlobalDb() {
  try {
    if (globalThis.__niyatnarouteDb?.open) {
      globalThis.__niyatnarouteDb.close();
    }
  } catch {}

  delete globalThis.__niyatnarouteDb;
}
...
test("getDbInstance creates sqlite schema, metadata and applies migrations", serial, async () => {
  const dataDir = makeTempDir("niyatnaroute-db-core-");
  ...
  try {
    await withEnv({ DATA_DIR: dataDir, NEXT_PHASE: undefined }, async () => {
      ...
      const versions = db
        .prepare("SELECT version FROM _niyatnaroute_migrations ORDER BY version")
        .all()
        .map((row) => row.version);
...
test("module exports honor DATA_DIR from the environment", serial, async () => {
  const dataDir = makeTempDir("niyatnaroute-db-core-env-");
...
test(
  ...
  serial,
  async () => {
    const fakeHome = makeTempDir("niyatnaroute-home-");
        ...
        async () => {
          ...
          const expectedDir =
            process.platform === "win32"
              ? path.join(fakeHome, "AppData", "Roaming", "niyatnaroute")
              : path.join(fakeHome, ".niyatnaroute");
...
test("build phase uses an in-memory database without creating sqlite files", serial, async () => {
  const dataDir = makeTempDir("niyatnaroute-db-build-");
...
test("invalid DATA_DIR (a file where a dir is expected) surfaces as a startup failure", serial, async () => {
  const sandboxDir = makeTempDir("niyatnaroute-db-bad-path-");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("niyatnaroute-db-legacy-empty-");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("niyatnaroute-db-legacy-data-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    createLegacySchemaDb(sqliteFile, { withData: true });
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("niyatnaroute-db-missing-max-concurrent-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
    const seedDb = new Database(sqliteFile);
    const now = new Date().toISOString();
    ...
    seedDb.exec(`
      ...
      INSERT INTO _niyatnaroute_migrations (version, name) VALUES ('001', 'initial_schema');
      INSERT INTO _niyatnaroute_migrations (version, name) VALUES ('029', 'webhooks_templates');
    `);
    seedDb
      .prepare(
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("niyatnaroute-db-legacy-call-logs-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("niyatnaroute-db-probe-recover-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("niyatnaroute-db-probe-latest-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
...
test(
  ...
  serial,
  async () => {
    const dataDir = makeTempDir("niyatnaroute-db-probe-abort-");
    const sqliteFile = path.join(dataDir, "storage.sqlite");
