...
describe("config-generator", () => {
  ...
  describe("assertSafeCatalogUrl (SSRF guard, CodeQL #326)", () => {
    it("allows the loopback NiyatnaRoute target (the legitimate default) and returns a URL", async () => {
      const { assertSafeCatalogUrl } = await import(
        "../../../src/lib/cli-helper/config-generator/opencode.ts"
      );
      // The catalog source IS the user's own NiyatnaRoute — localhost must stay allowed.
    ...
    it("allows a public NiyatnaRoute Cloud target", async () => {
      const { assertSafeCatalogUrl } = await import(
        "../../../src/lib/cli-helper/config-generator/opencode.ts"
      );
      assert.doesNotThrow(() => assertSafeCatalogUrl("https://api.niyatnaroute.online/v1/models"));
    });
  ...
  describe("generateConfig", () => {
    ...
    it("returns success for valid hermes config", async () => {
      ...
      assert.ok(String(result.content || "").includes("niyatnaroute"));
    });
  ...
  describe("hermes-agent (rich multi-role)", () => {
    ...
    it("generateHermesAgentConfig returns yaml string for valid payload", async () => {
      ...
      assert.ok(result.yaml.includes("provider: niyatnaroute"));
    });
  ...
  describe("opencode (context-aware)", () => {
    ...
    it("emits limit.context from the catalog (no hardcoded fallback)", async () => {
      ...
      try {
        ...
        const models = cfg.provider.niyatnaroute.models;
    ...
    it("does NOT fabricate a default context when the catalog has no entry", async () => {
      ...
      try {
        ...
        const noCtx = cfg.provider.niyatnaroute.models["NO_CTX_COMBO"];
        assert.strictEqual(
          noCtx.limit?.context,
          undefined,
    ...
    it("prefers max_context_window_tokens when context_length is absent", async () => {
      ...
      try {
        ...
        const cfg = JSON.parse(out);
        assert.strictEqual(cfg.provider.niyatnaroute.models.llama3.limit.context, 8192);
      } finally {
        stub.restore();
      }
    });
    ...
    it("writes a top-level model prefixed with provider id when options.model is supplied", async () => {
      ...
      try {
        ...
        const cfg = JSON.parse(out);
        assert.strictEqual(cfg.model, "niyatnaroute/MASTER");
      } finally {
        stub.restore();
      }
    });
    ...
    it("auto-pulls the Opencode FREE Omni combo context (the user-reported case)", async () => {
      ...
      try {
        ...
        const cfg = JSON.parse(out);
        assert.strictEqual(
          cfg.provider.niyatnaroute.models["Opencode FREE Omni"].limit.context,
          200000,
