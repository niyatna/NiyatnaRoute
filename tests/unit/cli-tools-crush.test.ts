...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-crush-settings-"));
...
test("crush-settings POST: writes crush.json with an openai-compat providers.niyatnaroute block", async () => {
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        ...
        const provider = written.providers?.niyatnaroute;
        assert.ok(provider, "providers.niyatnaroute must be written");
...
test("crush-settings DELETE: removes only the niyatnaroute provider entry", async () => {
  ...
  try {
      ...
      JSON.stringify({
        providers: {
          niyatnaroute: {
            ...
            api_key: "sk-test",
            models: [
              { id: "openai/gpt-5", name: "NiyatnaRoute: openai/gpt-5", context_window: 128000 },
            ],
          },
    ...
    if (res.status === 200) {
      ...
      if (fs.existsSync(configPath)) {
        const written = JSON.parse(fs.readFileSync(configPath, "utf-8"));
        assert.equal(written.providers?.niyatnaroute, undefined);
