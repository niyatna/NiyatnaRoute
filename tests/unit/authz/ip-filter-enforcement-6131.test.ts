...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-ipenforce-6131-"));
...
const ORIGINAL_STAMP_TOKEN = process.env.NIYATNAROUTE_PEER_STAMP_TOKEN;

test.after(() => {
  core.resetDbInstance();
  fs.rmSync(TEST_DATA_DIR, { recursive: true, force: true });
  if (ORIGINAL_STAMP_TOKEN === undefined) delete process.env.NIYATNAROUTE_PEER_STAMP_TOKEN;
  else process.env.NIYATNAROUTE_PEER_STAMP_TOKEN = ORIGINAL_STAMP_TOKEN;
});
