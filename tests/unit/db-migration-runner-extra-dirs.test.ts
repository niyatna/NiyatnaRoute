 ...
 *
 * This suite pins the extension point: `NIYATNA_EXTRA_MIGRATIONS_DIRS` maps
...
process.env.NIYATNA_MIGRATIONS_DIR = CORE_DIR;
...
function runWithExtras(extraSpec: string | null): RunResult {
  const prev = process.env.NIYATNA_EXTRA_MIGRATIONS_DIRS;
  if (extraSpec === null) delete process.env.NIYATNA_EXTRA_MIGRATIONS_DIRS;
  else process.env.NIYATNA_EXTRA_MIGRATIONS_DIRS = extraSpec;
  ...
  try {
    ...
    db.close();
    if (prev === undefined) delete process.env.NIYATNA_EXTRA_MIGRATIONS_DIRS;
    else process.env.NIYATNA_EXTRA_MIGRATIONS_DIRS = prev;
  }
}
...
test("rodar duas vezes não reaplica as migrations do diretório extra", async () => {
  ...
  const prev = process.env.NIYATNA_EXTRA_MIGRATIONS_DIRS;
  process.env.NIYATNA_EXTRA_MIGRATIONS_DIRS = `ee=${eeDir}`;
  const db = new Database(":memory:");
  try {
