...
test("renameProcessTitle renames a bare 'next-server' title to 'niyatnaroute'", () => {
  assert.equal(renameProcessTitle("next-server"), "niyatnaroute");
});

test("renameProcessTitle preserves a version suffix after 'next-server'", () => {
  assert.equal(renameProcessTitle("next-server (v16.2.9)"), "niyatnaroute (v16.2.9)");
});
...
test("renameProcessTitle is idempotent when called again on an already-renamed title", () => {
  ...
  assert.equal(once, "niyatnaroute (v16.2.9)");
  assert.equal(twice, "niyatnaroute (v16.2.9)");
});
