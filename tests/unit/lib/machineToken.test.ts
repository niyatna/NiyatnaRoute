...
test("getMachineTokenSync respects NIYATNAROUTE_CLI_SALT env var", () => {
  const before = getMachineTokenSync();
  process.env.NIYATNAROUTE_CLI_SALT = "__test_salt__";
  const withEnv = getMachineTokenSync();
  delete process.env.NIYATNAROUTE_CLI_SALT;
