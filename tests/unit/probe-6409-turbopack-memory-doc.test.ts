 ...
 * NIYATNA_USE_TURBOPACK=1). OmniRoute pins `next@^16.2.6` (resolved 16.2.9),
 ...
 *
 * A working escape hatch already exists (`NIYATNA_USE_TURBOPACK=0` reverts to
 ...
 * NIYATNA_USE_TURBOPACK does not mention memory/RAM at all.
 */
...
test("#6409 ENVIRONMENT.md documents the memory tradeoff of the Turbopack build default", () => {
  ...
  const row = lines.find((l) => l.includes("NIYATNA_USE_TURBOPACK"));
  assert.ok(row, "ENVIRONMENT.md must document NIYATNA_USE_TURBOPACK");

  const mentionsMemory = /\b(memory|RAM|GB)\b/i.test(row ?? "");
  assert.ok(
    mentionsMemory,
    "NIYATNA_USE_TURBOPACK row must mention the memory/RAM tradeoff so contributors on " +
