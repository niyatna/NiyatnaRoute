...
function runCli(
  ...
  for (const key of ["NIYATNA_BASE_URL", "PORT", "STORAGE_ENCRYPTION_KEY"]) {
    delete cleanEnv[key];
  }
...
test("a key masked by an earlier .env is named, with both files and without its value", () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(dirs.cwd, ".env"), "NIYATNA_BASE_URL=https://cwd.example/v1\n");

    const stderr = runCli(dirs).stderr ?? "";

    assert.match(stderr, /NIYATNA_BASE_URL/);
...
test("a key each file declares once says nothing", () => {
  ...
  try {
    ...
    const stderr = runCli(dirs).stderr ?? "";
    assert.ok(!/NIYATNA_BASE_URL|PORT/.test(stderr), `nothing to report: ${stderr}`);
  } finally {
    fs.rmSync(dirs.tmp, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 });
  }
});
...
test("a key the environment already set is reported too — that is #6194", () => {
  ...
  try {
    ...
    const stderr = runCli(dirs, { NIYATNA_BASE_URL: "https://shell.example/v1" }).stderr ?? "";
