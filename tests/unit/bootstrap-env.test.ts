...
function withTempEnv(fn) {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-bootstrap-test-"));
  ...
  try {
    fn({ tempRoot, tempCwd, tempHome, dataDir: path.join(tempHome, ".niyatnaroute") });
  } finally {
...
test("bootstrapEnv prefers ~/.niyatnaroute/.env over server.env", () => {
  withTempEnv(({ dataDir }) => {
