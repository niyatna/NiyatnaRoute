...
// ─── transformToNiyatnaRoute ────────────────────────────────

describe("transformToNiyatnaRoute", () => {
  test("converts LiteLLM per-token pricing to NiyatnaRoute per-million format", () => {
    const raw = {
      "openai/gpt-4o": {
    ...
    const result = transformToNiyatnaRoute(raw);
...
describe("pricing merge precedence", () => {
  test("user overrides > synced > defaults conceptual order", () => {
    ...
    const synced = transformToNiyatnaRoute(raw);
