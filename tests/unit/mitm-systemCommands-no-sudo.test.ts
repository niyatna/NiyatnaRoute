 ...
 * feature adds a third opt-out trigger — the `NIYATNA_NO_SUDO` env flag — so
...
test(
  "resolveSudoSpawn: with NIYATNA_NO_SUDO=1, strips `sudo -S` and needs no password",
  () => {
    const prev = process.env.NIYATNA_NO_SUDO;
    ...
    try {
      ...
      if (prev === undefined) delete process.env.NIYATNA_NO_SUDO;
      else process.env.NIYATNA_NO_SUDO = prev;
    }
  }
);
...
test(
  ...
  () => {
    const prev = process.env.NIYATNA_NO_SUDO;
    delete process.env.NIYATNA_NO_SUDO;
    try {
      const { finalCommand, finalArgs, stripSudo, needsPassword } = resolveSudoSpawn(
        "sudo",
        ["-S", "cp", "/tmp/ca.pem", "/usr/local/share/ca-certificates/ca.crt"],
...
test("isNoSudoEnv: truthiness matrix matches isTruthyEnvFlag semantics", () => {
  ...
  try {
    for (const [value, expected] of cases) {
      if (value === undefined) delete process.env.NIYATNA_NO_SUDO;
      else process.env.NIYATNA_NO_SUDO = value;
      assert.equal(isNoSudoEnv(), expected, `NIYATNA_NO_SUDO=${JSON.stringify(value)}`);
    }
  } finally {
    if (prev === undefined) delete process.env.NIYATNA_NO_SUDO;
    else process.env.NIYATNA_NO_SUDO = prev;
...
test("resolveSudoSpawn: env flag alone (no overrides) drives stripping on non-root sudo host", () => {
  ...
  const prev = process.env.NIYATNA_NO_SUDO;
  process.env.NIYATNA_NO_SUDO = "yes";
  try {
