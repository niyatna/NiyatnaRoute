...
// ─── #3476: discoverability of `niyatnaroute runtime repair` ──────────────────────
//
// When better-sqlite3's native binding mismatches the Node ABI, NiyatnaRoute prints
...
// A self-heal command already exists (`niyatnaroute runtime repair`); these tests
...
test("serve.mjs ABI-mismatch hint source mentions `runtime repair`", () => {
  ...
  assert.match(
    src,
    /runtime repair/,
    "serve.mjs should surface `niyatnaroute runtime repair` as a recovery hint"
  );
});
