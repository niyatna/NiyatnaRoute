...
function runCli(dataDir: string): { code: number | null; stderr: string } {
  ...
  const isolatedHome = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-key-home-"));
  try {
...
// the CLI must persist the key into DATA_DIR (not just ~/.niyatnaroute) so Docker/custom-DATA_DIR
...
test("CLI generates STORAGE_ENCRYPTION_KEY into DATA_DIR on first run (#1622)", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-key-a-"));
  try {
    runCli(dir);
...
test("CLI refuses to auto-generate a key when a database already exists (#1622)", () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-key-b-"));
  try {
