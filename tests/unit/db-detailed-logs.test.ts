...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-db-detailed-logs-"));
...
test("saveRequestDetailLog persists protected payloads and compacted stream summaries", () => {
  ...
  assert.deepEqual((row as any).provider_response._niyatnaroute_stream, {
    format: "sse-json",
