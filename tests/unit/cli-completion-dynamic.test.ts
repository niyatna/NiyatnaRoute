...
test("runCompletionCommand bash retorna 0 e string não-vazia", async () => {
  ...
  assert.ok(out.includes("niyatnaroute"), "bash script should mention niyatnaroute");
  assert.ok(out.includes("_niyatnaroute"), "bash script should define _niyatnaroute function");
});
...
test("runCompletionCommand fish retorna 0", async () => {
  ...
  const out = chunks.join("");
  assert.ok(out.includes("niyatnaroute"), "fish script should mention niyatnaroute");
});
...
test("completion scripts incluem combos/providers/models no cache dinamicamente", async () => {
  ...
  const out = chunks.join("");
  assert.ok(
    out.includes("completion-cache.json") || out.includes("niyatnaroute_get_cache"),
