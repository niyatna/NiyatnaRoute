...
// #3501). attachNiyatnaRouteMetaHeaders + now are injected so the static headers, the meta payload,
...
function makeDeps(now = 1000) {
  ...
  const deps = {
    attachNiyatnaRouteMetaHeaders: (headers: Record<string, string>, meta: Record<string, unknown>) => {
      metaCalls.push({ headers, meta });
      headers["x-niyatnaroute-meta"] = "attached";
    },
    now: () => now,
