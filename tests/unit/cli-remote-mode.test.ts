...
test.before(() => {
  ...
  origBaseUrl = process.env.NIYATNA_BASE_URL;
  origApiKey = process.env.NIYATNA_API_KEY;
  origContext = process.env.NIYATNA_CONTEXT;
  process.env.DATA_DIR = tmpDir;
  delete process.env.NIYATNA_BASE_URL;
  delete process.env.NIYATNA_API_KEY;
  delete process.env.NIYATNA_CONTEXT;
});

test.after(() => {
  if (origDataDir === undefined) delete process.env.DATA_DIR;
  else process.env.DATA_DIR = origDataDir;
  if (origBaseUrl === undefined) delete process.env.NIYATNA_BASE_URL;
  else process.env.NIYATNA_BASE_URL = origBaseUrl;
  if (origApiKey === undefined) delete process.env.NIYATNA_API_KEY;
  else process.env.NIYATNA_API_KEY = origApiKey;
  if (origContext === undefined) delete process.env.NIYATNA_CONTEXT;
  else process.env.NIYATNA_CONTEXT = origContext;
  try {
...
test("buildHeaders: active-context token wins over an opts.apiKey echoing the ambient env", async () => {
  // Regression: users keep NIYATNA_API_KEY (their inference key) in the shell.
  ...
  process.env.NIYATNA_API_KEY = "sk-ambient-inference-key";
  try {
    const { buildHeaders } = await import("../../bin/cli/api.mjs");
    ...
    if (prev === undefined) delete process.env.NIYATNA_API_KEY;
    else process.env.NIYATNA_API_KEY = prev;
  }
});
...
test("buildHeaders: ambient NIYATNA_API_KEY is the fallback when no context credential", async () => {
