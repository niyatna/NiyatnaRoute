...
import { deriveDefaultPlan } from "@niyatnaroute/open-sse/services/compression/deriveDefaultPlan.ts";

// ─── isolated temp DB ─────────────────────────────────────────────────────────

const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-ctx-combos-default-route-"));
