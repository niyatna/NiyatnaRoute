...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-command-code-auth-"));
...
test("Command Code auth assist keeps auth URL callback on CLI localhost contract", async () => {
  process.env.NIYATNAROUTE_PUBLIC_BASE_URL = "https://niyatnaroute.example.com/base-path";
