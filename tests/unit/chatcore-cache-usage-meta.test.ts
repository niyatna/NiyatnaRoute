...
// shapes (null when no cache fields), and the _niyatnaroute meta attachment/merge.
...
test("attachLogMeta merges compact meta into _niyatnaroute", () => {
  const out = attachLogMeta({ a: 1, _niyatnaroute: { keep: true } }, { added: 2, drop: null });
  assert.deepEqual(out, { a: 1, _niyatnaroute: { keep: true, added: 2 } });
});

test("attachLogMeta wraps non-object payloads", () => {
  const out = attachLogMeta(null, { added: 2 });
  assert.deepEqual(out, { _niyatnaroute: { added: 2 }, _payload: null });
});
