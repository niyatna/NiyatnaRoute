 ...
 * (`serve --tray`) or the Electron Appearance tab; a plain `niyatnaroute serve`
 * user had no way to enable it. The `autostart` command now exposes the
 * shorthand the reporter asked for (`niyatnaroute autostart on` / `... true`) via
...
test("status is the default action (bare `niyatnaroute autostart` is a safe read-only)", () => {
