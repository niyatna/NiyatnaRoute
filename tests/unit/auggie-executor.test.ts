...
const { AuggieExecutor, buildAuggiePrompt, resolveAuggieBin, resolveAuggieModel } =
  await import("@niyatnaroute/open-sse/executors/auggie");

const TMP_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-auggie-test-"));
...
test("refreshCredentials() is a no-op (auggie has no NiyatnaRoute-managed credentials)", async () => {
  const executor = new AuggieExecutor();
...
test("execute() spawns a model discovered via auto-fetch", async () => {
  const { __resetAuggieModels } = await import("@niyatnaroute/open-sse/executors/auggie");
  __resetAuggieModels();
  const bin = writeFakeBin(
