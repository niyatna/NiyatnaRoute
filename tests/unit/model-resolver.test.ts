...
const modelResolverDataDir = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-model-resolver-"));
...
test(
  ...
  withEnv("NIYATNAROUTE_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS", "true", async () => {
...
test("getModelInfoCore routes unprefixed Claude models to Claude Code from settings toggle", async () => {
  ...
  delete process.env.NIYATNAROUTE_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS;
  ...
  try {
    ...
    if (previousEnvFlag === undefined) {
      ...
      process.env.NIYATNAROUTE_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS = previousEnvFlag;
    }
  }
});
...
test("getModelInfoCore lets settings toggle disable Claude Code preference", async () => {
  const previousEnvFlag =
    process.env.NIYATNAROUTE_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS;
  process.env.NIYATNAROUTE_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS = "true";
