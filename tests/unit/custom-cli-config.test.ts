...
test("buildAliasEnvVar sanitizes aliases for env variable export", () => {
  assert.equal(buildAliasEnvVar("review"), "NIYATNAROUTE_MODEL_REVIEW");
  assert.equal(buildAliasEnvVar("plan mode"), "NIYATNAROUTE_MODEL_PLAN_MODE");
...
test("custom CLI generators include default model and alias mappings", () => {
  const envScript = buildCustomCliEnvScript({
    ...
    apiKey: "sk_test_123",
    defaultModel: "niyatnaroute/fast",
    aliasMappings: [
  ...
  assert.match(envScript, /export OPENAI_MODEL="niyatnaroute\/fast"/);
  assert.match(envScript, /export NIYATNAROUTE_MODEL_REVIEW="cc\/claude-sonnet-4-5-20250929"/);
