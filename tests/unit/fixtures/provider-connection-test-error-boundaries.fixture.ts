...
const originalDisableHealthCheck = process.env.NIYATNA_DISABLE_CREDENTIAL_HEALTH_CHECK;
const pluginsDir = path.join(testRoot, "plugins");
const testDataDir = path.join(testRoot, "data");
fs.mkdirSync(pluginsDir, { recursive: true });
fs.mkdirSync(testDataDir, { recursive: true });
process.env.NIYATNA_PLUGINS_DIR = pluginsDir;
...
test.after(async () => {
  ...
  if (originalDisableHealthCheck === undefined) {
    delete process.env.NIYATNA_DISABLE_CREDENTIAL_HEALTH_CHECK;
  } else {
    process.env.NIYATNA_DISABLE_CREDENTIAL_HEALTH_CHECK = originalDisableHealthCheck;
  }
