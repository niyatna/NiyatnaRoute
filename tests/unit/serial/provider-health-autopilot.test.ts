...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-health-autopilot-"));
...
const accountFallback = await import("@niyatnaroute/open-sse/services/accountFallback");
...
test("provider health autopilot action accepts LAN dashboard requests (#6277)", async () => {
  ...
  assert.equal(request.headers.get("x-niyatnaroute-peer-ip"), null);
