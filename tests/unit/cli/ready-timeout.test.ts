...
describe("resolveReadyTimeoutMs", () => {
  const savedEnv = process.env.NIYATNA_READY_TIMEOUT_MS;

  beforeEach(() => {
    // Reset env between tests
    if (savedEnv === undefined) {
      delete process.env.NIYATNA_READY_TIMEOUT_MS;
    } else {
      process.env.NIYATNA_READY_TIMEOUT_MS = savedEnv;
    }
  });
  ...
  test("explicit override wins over env var", () => {
    process.env.NIYATNA_READY_TIMEOUT_MS = "300000";
    assert.equal(resolveReadyTimeoutMs({ timeoutMs: 90_000 }), 90_000);
  });

  test("reads NIYATNA_READY_TIMEOUT_MS env var", () => {
    process.env.NIYATNA_READY_TIMEOUT_MS = "180000";
    assert.equal(resolveReadyTimeoutMs(), 180_000);
  });

  test("falls back to default when env var is non-numeric", () => {
    process.env.NIYATNA_READY_TIMEOUT_MS = "not-a-number";
    assert.equal(resolveReadyTimeoutMs(), 60_000);
  });

  test("falls back to default when env var is zero", () => {
    process.env.NIYATNA_READY_TIMEOUT_MS = "0";
    assert.equal(resolveReadyTimeoutMs(), 60_000);
  });

  test("falls back to default when env var is negative", () => {
    process.env.NIYATNA_READY_TIMEOUT_MS = "-5000";
    assert.equal(resolveReadyTimeoutMs(), 60_000);
  });
