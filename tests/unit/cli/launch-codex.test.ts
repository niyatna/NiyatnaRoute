...
test("buildCodexEnv strips stale OpenAI/Codex creds from the child env (defense-in-depth)", () => {
  ...
  assert.equal(env.NIYATNAROUTE_API_KEY, "oma_live_x");
...
test("buildCodexEnv uses a no-auth sentinel when no token is given", () => {
  ...
  assert.equal(env.NIYATNAROUTE_API_KEY, "niyatnaroute-no-auth");
});
...
test("buildCodexProviderArgs defines the niyatnaroute provider inline (works without config.toml)", () => {
  const args = buildCodexProviderArgs("http://vps:20128");
  const joined = args.join(" ");
  assert.ok(joined.includes('model_provider="niyatnaroute"'));
  assert.ok(joined.includes('model_providers.niyatnaroute.base_url="http://vps:20128/v1"'));
  assert.ok(joined.includes('model_providers.niyatnaroute.env_key="NIYATNAROUTE_API_KEY"'));
  assert.ok(joined.includes('model_providers.niyatnaroute.wire_api="responses"'));
  assert.ok(joined.includes("model_providers.niyatnaroute.requires_openai_auth=false"));
