...
test.before(() => {
  tmpDir = mkdtempSync(join(tmpdir(), "niyatnaroute-autostart-linux-"));
...
test("resolveCliPath finds niyatnaroute.mjs from argv", async () => {
  ...
  const unitPath = join(tmpDir, ".config", "systemd", "user", "niyatnaroute.service");
  const desktopPath = join(tmpDir, ".config", "autostart", "niyatnaroute.desktop");
  ...
  if (existsSync(unitPath)) {
    ...
    assert.match(unit, /ExecStart=.*niyatnaroute\.mjs.*serve --no-open/m);
