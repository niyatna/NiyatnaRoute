...
const TEST_DATA_DIR = mkdtempSync(join(tmpdir(), "niyatnaroute-sse-shim-"));
...
test("src/sse service wrappers delegate to open-sse and shared infrastructure", () => {
  ...
  assert.match(tokenRefreshSource, /@niyatnaroute\/open-sse\/services\/tokenRefresh\.ts/);
  assert.match(modelSource, /@niyatnaroute\/open-sse\/services\/model\.ts/);
