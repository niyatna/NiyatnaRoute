...
function withEnv(value: string | undefined, fn: () => void) {
  const previous = process.env.NIYATNA_EMERGENCY_FALLBACK;
  if (value === undefined) {
    delete process.env.NIYATNA_EMERGENCY_FALLBACK;
  } else {
    process.env.NIYATNA_EMERGENCY_FALLBACK = value;
  }
  resetEmergencyFallbackEnvCache();
  try {
    fn();
  } finally {
    restoreEnv("NIYATNA_EMERGENCY_FALLBACK", previous);
    resetEmergencyFallbackEnvCache();
  }
}
...
function withFeatureFlagOverride(value: string, fn: () => void) {
  try {
    setFeatureFlagOverride("NIYATNA_EMERGENCY_FALLBACK", value);
    resetEmergencyFallbackEnvCache();
    fn();
  } finally {
    removeFeatureFlagOverride("NIYATNA_EMERGENCY_FALLBACK");
    resetEmergencyFallbackEnvCache();
  }
}
...
test("NIYATNA_EMERGENCY_FALLBACK=false disables the 402 redirect", () => {
  withEnv("false", () => {
    assert.equal(isEmergencyFallbackEnvEnabled(), false);
    const decision = shouldUseFallback(402, "", false);
    assert.equal(decision.shouldFallback, false);
    assert.match(decision.reason, /NIYATNA_EMERGENCY_FALLBACK/);
  });
});

test("NIYATNA_EMERGENCY_FALLBACK=0 disables the budget-keyword redirect", () => {
  withEnv("0", () => {
