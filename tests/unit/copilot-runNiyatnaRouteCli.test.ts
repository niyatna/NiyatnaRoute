...
test("runNiyatnaRouteCli: missing command returns error", async () => {
...
test("runNiyatnaRouteCli: empty command returns error", async () => {
  const { getCopilotTool } = await import("../../src/lib/copilot/tools.ts");
  const tool = getCopilotTool("runNiyatnaRouteCli");
  assert.ok(tool);
...
test("runNiyatnaRouteCli: returns CLI-not-found when niyatnaroute unavailable", async () => {
  const { getCopilotTool } = await import("../../src/lib/copilot/tools.ts");
  const tool = getCopilotTool("runNiyatnaRouteCli");
  assert.ok(tool);
  ...
  try {
    ...
    assert.ok(
      result.includes("niyatnaroute CLI not found in PATH"),
