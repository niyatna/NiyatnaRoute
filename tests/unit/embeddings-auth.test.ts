...
test("isValidApiKey", async (t) => {
  const originalEnv = process.env.NIYATNAROUTE_API_KEY;

  await t.test("should return true if key matches NIYATNAROUTE_API_KEY", async () => {
    process.env.NIYATNAROUTE_API_KEY = "test-key";
...
test("POST /v1/embeddings authentication", async (t) => {
  ...
  process.env.NIYATNAROUTE_API_KEY = originalOmniKey;
});
