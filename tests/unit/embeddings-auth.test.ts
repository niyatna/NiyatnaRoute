...
test("isValidApiKey", async (t) => {
  const originalEnv = process.env.NIYATNA_API_KEY;

  await t.test("should return true if key matches NIYATNA_API_KEY", async () => {
    process.env.NIYATNA_API_KEY = "test-key";
...
test("POST /v1/embeddings authentication", async (t) => {
  ...
  process.env.NIYATNA_API_KEY = originalOmniKey;
});
