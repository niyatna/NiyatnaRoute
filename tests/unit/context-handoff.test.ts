...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-context-handoff-"));
...
test("maybeGenerateHandoff persists a structured handoff once the threshold is reached", async () => {
  ...
  assert.equal(calls[0].body._niyatnarouteSkipContextRelay, true);
  assert.equal(calls[0].body._niyatnarouteInternalRequest, "context-handoff");
});
