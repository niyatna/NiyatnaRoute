...
// forwards. On the stdio transport `niyatnaroute_ccr_retrieve` therefore fell back
...
// the registered niyatnaroute_ccr_retrieve handler with an `extra` carrying that
...
test("static tool loops forward `extra` so stdio callers keep their scope/identity (#6178)", async () => {
  ...
  const retrieve = getRegisteredHandler(server, "niyatnaroute_ccr_retrieve");
