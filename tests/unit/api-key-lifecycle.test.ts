...
const ORIGINAL_NIYATNAROUTE_API_KEY = process.env.NIYATNAROUTE_API_KEY;
...
test.after(() => {
  fs.rmSync(TEST_DATA_DIR, { recursive: true, force: true });
  if (ORIGINAL_NIYATNAROUTE_API_KEY === undefined) delete process.env.NIYATNAROUTE_API_KEY;
  else process.env.NIYATNAROUTE_API_KEY = ORIGINAL_NIYATNAROUTE_API_KEY;
...
test("validateApiKey accepts configured environment API keys", async () => {
  process.env.NIYATNAROUTE_API_KEY = "sk-env-lifecycle-test";
