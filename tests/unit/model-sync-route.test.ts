...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-model-sync-route-"));
...
test("model sync route falls back to in-process discovery when internal self-fetch throws", async () => {
  ...
  const expectedPort = process.env.NIYATNAROUTE_PORT || process.env.PORT || "20128";
