...
test("Dockerfile exposes NIYATNA_BASE_PATH as a builder-stage build arg", () => {
  const dockerfile = fs.readFileSync(path.join(REPO_ROOT, "Dockerfile"), "utf8");
  assert.match(dockerfile, /ARG NIYATNA_BASE_PATH/);
  assert.match(dockerfile, /ENV NIYATNA_BASE_PATH=\$NIYATNA_BASE_PATH/);
});

test("docker-compose forwards NIYATNA_BASE_PATH into image builds", () => {
  const compose = fs.readFileSync(path.join(REPO_ROOT, "docker-compose.yml"), "utf8");
  assert.match(compose, /NIYATNA_BASE_PATH: \$\{NIYATNA_BASE_PATH:-\}/);
  assert.match(compose, /- NIYATNA_BASE_PATH=\$\{NIYATNA_BASE_PATH:-\}/);
});
...
test("Hard Rule #13: shell basePath helpers never interpolate into sed/awk", () => {
  ...
  for (const rel of shellFiles) {
    ...
    assert.doesNotMatch(
      source,
      /\b(?:sed|awk)\b.*\$\{?NIYATNA_BASE_PATH/,
      `${rel} must not expand NIYATNA_BASE_PATH into sed/awk`
    );
    assert.doesNotMatch(
      source,
      /(?:sed|awk).*NIYATNA_BASE_PATH/,
      `${rel} must not pass NIYATNA_BASE_PATH on a sed/awk command line`
    );
  }
  ...
  assert.match(ensure, /process\.env\.NIYATNA_BASE_PATH/);
});
