...
describe("resolveApiKey", () => {
  ...
  it("falls back to sk_niyatnaroute when both are null", async () => {
    mockKeyStore.clear();

    const result = await resolveApiKey(null, null);
    assert.equal(result, "sk_niyatnaroute");
  });

  it("falls back to sk_niyatnaroute when both are undefined", async () => {
    mockKeyStore.clear();
