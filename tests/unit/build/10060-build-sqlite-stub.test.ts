 ...
 *
 * The reliable build signal is NIYATNA_BUILDING=1 (set by
 ...
 *
 *   1. build-next-isolated.mjs exports NIYATNA_BUILDING=1 into the build env.
...
describe("#10060 build env carries NIYATNA_BUILDING", () => {
  it("resolveNextBuildEnv sets NIYATNA_BUILDING=1", () => {
    const env = resolveNextBuildEnv({}, "linux");
    assert.equal(env.NIYATNA_BUILDING, "1");
  });
...
describe("#10060 getDbInstance stubs SQLite during build", () => {
  const savedBuilding = process.env.NIYATNA_BUILDING;
  const savedPhase = process.env.NEXT_PHASE;

  beforeEach(() => {
    delete process.env.NEXT_PHASE;
    process.env.NIYATNA_BUILDING = "1";
  });

  afterEach(() => {
    if (savedBuilding === undefined) delete process.env.NIYATNA_BUILDING;
    else process.env.NIYATNA_BUILDING = savedBuilding;
  ...
  it("returns a no-op stub (never the native better-sqlite3 driver) under the build signal", async () => {
    // Import fresh so isBuildPhase is evaluated with NIYATNA_BUILDING set.
