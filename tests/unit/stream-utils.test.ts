...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-stream-utils-"));
...
test("createSSEStream passthrough converts textual tool-call content into structured call log tool_calls", async () => {
  ...
  const toolArgs = JSON.stringify({
    command: 'sqlite3 /root/.o\u200dmniroute/niyatnaroute.db ".tables"',
  });
  ...
  assert.deepEqual(JSON.parse(choice.message.tool_calls[0].function.arguments), {
    command: 'sqlite3 /root/.niyatnaroute/niyatnaroute.db ".tables"',
  });
...
test("createSSEStream passthrough converts split textual tool-call content at completion", async () => {
  ...
  assert.equal(choice.message.tool_calls[0].function.name, "terminal");
  assert.deepEqual(JSON.parse(choice.message.tool_calls[0].function.arguments), {
    command: 'sqlite3 ~/.niyatnaroute/niyatnaroute.db ".tables"',
  });
  assert.doesNotMatch(text, /\[Tool call: terminal\]/);
});
...
Arguments: {"path":"/opt/NiyatnaRoute/src","target":"files"}`;
        ...
        candidates: [
          {
            content: {
              parts: [
                {
                  text: "[Tool call: search_files_ide{file_glob:*combos*.ts,path:/opt/NiyatnaRoute,target:files}]",
                },
              ],
            },
          },
...
test("createSSEStream Responses passthrough converts textual tool-call deltas before streaming", async () => {
  let onCompletePayload = null;
  const toolText = `[Tool call: terminal]
Arguments: {"command":"systemctl status niyatnaroute"}`;
  const text = await readTransformed(
    [
      `data: ${JSON.stringify({
...
test("compactStructuredStreamPayload wraps primitive summaries with Niyatnaroute stream metadata", () => {
...
test("createSSEStream passthrough drops empty choices array chunks", async () => {
  ...
  // "[NiyatnaRoute] Upstream returned an empty response. Please retry." chunk. That
  ...
  assert.doesNotMatch(text, /\[NiyatnaRoute\] Upstream returned an empty response/);
