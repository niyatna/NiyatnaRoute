...
describe("getAutoUpdateConfig", () => {
  it("defaults to npm or source mode locally", () => {
    const config = autoUpdate.getAutoUpdateConfig({ DATA_DIR: "/tmp/niyatnaroute" });
    assert.ok(config.mode === "npm" || config.mode === "source");
    assert.equal(config.repoDir, "/workspace/niyatnaroute");
  ...
  it("reads docker-compose settings from env", () => {
    const config = autoUpdate.getAutoUpdateConfig({
      ...
      AUTO_UPDATE_REPO_DIR: "/srv/niyatnaroute",
      AUTO_UPDATE_COMPOSE_FILE: "/srv/niyatnaroute/docker-compose.yml",
      AUTO_UPDATE_COMPOSE_PROFILE: "base",
      AUTO_UPDATE_SERVICE: "niyatnaroute-base",
    ...
    assert.equal(config.repoDir, "/srv/niyatnaroute");
    assert.equal(config.composeFile, "/srv/niyatnaroute/docker-compose.yml");
    assert.equal(config.composeProfile, "base");
    assert.equal(config.composeService, "niyatnaroute-base");
...
describe("buildSourceUpdateScript", () => {
  it("includes git checkout, env sync, and rebuild steps", () => {
    ...
    assert.match(script, /pm2 restart niyatnaroute --update-env \|\| true/);
  });
});
