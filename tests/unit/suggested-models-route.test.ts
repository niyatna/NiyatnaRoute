...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-suggested-models-route-"));
...
test("route source: imports and uses buildErrorBody (Hard Rule #12)", async () => {
  ...
  assert.match(
    ...
    "must import buildErrorBody from @niyatnaroute/open-sse/utils/error"
  );
