/**
 * Unit tests: NIYATNA_SKIP_DNS_WRITE guard on addDNSEntries / removeDNSEntries.
 *
 ...
 * NIYATNA_SKIP_DNS_WRITE=1 by default, so the guard tests are inherently
 ...
 * installed (slim containers) or under `NIYATNA_NO_SUDO` (#6122) — so the
...
function guardEnv(value: string | undefined): () => void {
  const prev = process.env.NIYATNA_SKIP_DNS_WRITE;
  if (value === undefined) {
    delete process.env.NIYATNA_SKIP_DNS_WRITE;
  } else {
    process.env.NIYATNA_SKIP_DNS_WRITE = value;
  }
  return () => {
    if (prev === undefined) delete process.env.NIYATNA_SKIP_DNS_WRITE;
    else process.env.NIYATNA_SKIP_DNS_WRITE = prev;
  };
}
...
test("addDNSEntries: returns early when NIYATNA_SKIP_DNS_WRITE=1", async () => {
  resetSpawnCalls();
...
test("addDNSEntries: proceeds when NIYATNA_SKIP_DNS_WRITE=0", async () => {
  resetSpawnCalls();
  const { addDNSEntries } = await importDnsConfig();
...
test("removeDNSEntries: returns early when NIYATNA_SKIP_DNS_WRITE=1", async () => {
  resetSpawnCalls();
...
test("removeDNSEntries: proceeds when NIYATNA_SKIP_DNS_WRITE=0", async () => {
  resetSpawnCalls();
  const { removeDNSEntries } = await importDnsConfig();
  const restore = guardEnv("0");
  try {
