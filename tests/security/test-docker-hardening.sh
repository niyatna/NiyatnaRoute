...
IMAGE_NAME="${1:-niyatnaroute-local-hardened}"
CONTAINER_NAME="${2:-niyatnaroute-hardening-test}"
HOST_PORT="${3:-20129}"
...
docker build -t "${IMAGE_NAME}" "${WORKDIR}" >/tmp/niyatnaroute_hardening_build.log
echo "      Build done."
...
docker run -d \
  ...
  -e DATA_DIR=/app/data \
  "${IMAGE_NAME}" >/tmp/niyatnaroute_hardening_container_id.txt
