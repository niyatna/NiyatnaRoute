...
<!-- skill:custom-start -->
<!-- Migrated from skills/niyatnaroute-monitoring/SKILL.md (preserved curated content) -->

# NiyatnaRoute — Monitoring & Health
...
```bash
curl $NIYATNAROUTE_URL/api/health \
...
```bash
curl $NIYATNAROUTE_URL/api/monitoring/health \
  -H "Authorization: Bearer $NIYATNAROUTE_KEY"
```
...
```bash
curl $NIYATNAROUTE_URL/api/providers/metrics \
  -H "Authorization: Bearer $NIYATNAROUTE_KEY"
```
...
## Via MCP (if NiyatnaRoute is your MCP server)

```
niyatnaroute_get_health            → full system health snapshot
niyatnaroute_get_provider_metrics  → p50/p95/p99 + circuit state per provider
niyatnaroute_get_session_snapshot  → cost, tokens, errors for current session
niyatnaroute_check_quota           → quota balance + percent remaining + reset time
niyatnaroute_db_health_check       → diagnose + auto-repair database drift
```

## Quota check

```bash
curl $NIYATNAROUTE_URL/api/quota \
  -H "Authorization: Bearer $NIYATNAROUTE_KEY"
```
...
```bash
curl -X POST $NIYATNAROUTE_URL/api/budget/guard \
  -H "Authorization: Bearer $NIYATNAROUTE_KEY" \
...
NiyatnaRoute logs every MCP tool call to `mcp_audit` table. Query via API:

```bash
curl "$NIYATNAROUTE_URL/api/mcp/status" \
  -H "Authorization: Bearer $NIYATNAROUTE_KEY"
```
...
- `503` on health endpoint → NiyatnaRoute is starting up; retry in 5s
