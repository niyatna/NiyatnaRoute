---
name: omni-cli-tools
description: "Manage CLI tool integrations exposed via the API. List, configure, and invoke CLI tool plugins that extend NiyatnaRoute's automation surface."
---
...
Read Crush CLI NiyatnaRoute config

Local-only. Reads the NiyatnaRoute provider block in Crush's config.
...
Write Crush CLI NiyatnaRoute config

Local-only. Registers NiyatnaRoute as an `openai-compat` provider in Crush's config.
...
Remove NiyatnaRoute from Crush CLI config

Local-only. Removes the NiyatnaRoute provider block from Crush's config.
...
Read CodeWhale CLI NiyatnaRoute config

Local-only. Reads the NiyatnaRoute config block from `~/.codewhale/config.toml` (with `~/.deepseek/config.toml` legacy fallback).
...
Write CodeWhale CLI NiyatnaRoute config

Local-only. Writes the NiyatnaRoute config block in CodeWhale TOML format.
...
Remove NiyatnaRoute from CodeWhale CLI config

Local-only. Removes the NiyatnaRoute config block from CodeWhale's config.
