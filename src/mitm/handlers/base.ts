 ...
 *   - forwarding to the NiyatnaRoute router (Next.js API)
 *   - SSE piping
 ...
 * Routes through `@niyatnaroute/open-sse/utils/error.sanitizeErrorMessage` (Hard Rule #12)
...
async function safeErrorMessage(err: unknown): Promise<string> {
  try {
    const mod = (await import("@niyatnaroute/open-sse/utils/error")) as {
...
export abstract class MitmHandlerBase {
   ...
   *   3. Call `this.fetchRouter(...)` for the NiyatnaRoute router round-trip.
  ...
  /**
   * Forward the prepared body to the NiyatnaRoute router (Next.js API).
   * Adds AgentBridge correlation headers (`x-niyatnaroute-source`, `x-niyatnaroute-agent`)
  ...
  protected async fetchRouter(
    ...
    const base = process.env.NIYATNAROUTE_BASE_URL ?? "http://127.0.0.1:20128";
    ...
    return fetch(url, {
      ...
      headers: {
        ...
        "x-niyatnaroute-agent": this.agentId,
