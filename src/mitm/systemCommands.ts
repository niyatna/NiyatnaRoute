 ...
 *
 * Slim Docker images (e.g. `node:24-trixie-slim` used by NiyatnaRoute's runtime
...
/**
 * Truthy-env check for `NIYATNAROUTE_NO_SUDO`. Inlined (not imported from
...
export function isNoSudoEnv(): boolean {
  const value = process.env.NIYATNAROUTE_NO_SUDO;
 ...
 * the operator opts into root-less mode via `NIYATNAROUTE_NO_SUDO` (#6122). No
...
export function execFileWithPassword(
  ...
  // `NIYATNAROUTE_NO_SUDO` (root-less / user-namespace deployments — #6122), skip
...
export async function runElevatedPowerShell(script: string): Promise<string> {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "niyatnaroute-elevate-"));
  const scriptName = `niyatnaroute-elevate-${crypto.randomUUID()}.ps1`;
