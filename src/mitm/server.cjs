...
const ROUTER_BASE_URL = (
  process.env.NIYATNAROUTE_BASE_URL ||
...
// Inspector. The token is injected by manager.ts (same value the NiyatnaRoute
 ...
 * shape. The route-only namespace is reserved for client-facing NiyatnaRoute model ids;
...
async function passthrough(req, res, bodyBuffer) {
  ...
  // Defense-in-depth loop guard (Gap 14). The x-niyatnaroute-source header is the
...
async function intercept(req, res, bodyBuffer, override, sourceModel) {
  ...
  // The NiyatnaRoute router uses these to distinguish AgentBridge traffic from
  ...
  try {
    ...
    const response = await fetch(forward.url, {
      ...
      headers: {
        ...
        "x-niyatnaroute-source": "agent-bridge",
        "x-niyatnaroute-agent": agentId,
      },
    ...
    if (!response.ok) {
      ...
      throw new Error(`NiyatnaRoute ${response.status}: ${errText}`);
    }
...
async function startMitmServer() {
  ...
  const server = https.createServer(sslOptions, async (req, res) => {
    ...
    if (req.headers["x-niyatnaroute-source"] === "niyatnaroute") {
      vlog(1, `[MITM] → PASSTHROUGH (NiyatnaRoute source loop)`);
