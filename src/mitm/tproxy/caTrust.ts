 ...
 * trust store under a DEDICATED slot (`niyatnaroute-tproxy-ca.crt`), separate from
 * the static MITM cert (`niyatnaroute-mitm.crt` in `cert/install.ts`), so the two
...
export const TPROXY_CA_CERT_NAME = "niyatnaroute-tproxy-ca.crt";
...
export async function installTproxyCa(
  ...
  if (process.env.NIYATNAROUTE_SKIP_SYSTEM_TRUST === "1" && deps.run === undefined) {
    console.log("[tproxy-ca] NIYATNAROUTE_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
