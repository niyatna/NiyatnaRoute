...
export default function DroidToolCard({
  ...
  // (#618) Match any custom:NiyatnaRoute-<i> entry (multi-model).
  const isNiyatnaRouteEntry = (m) => typeof m?.id === "string" && m.id.startsWith("custom:NiyatnaRoute");

  const getConfigStatus = () => {
    if (!cliReady) return null;
    const currentConfig = droidStatus.settings?.customModels?.find(isNiyatnaRouteEntry);
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      // (#618) Pre-fill the multi-model list from every custom:NiyatnaRoute-<i>
      // entry, preserving the original index order.
      const existing = (droidStatus.settings?.customModels || [])
        .filter(isNiyatnaRouteEntry)
        .slice()
  ...
  const getManualConfigs = () => {
    ...
    const keyToDisplay =
      selectedKeyObj?.key || (!cloudEnabled ? "sk_niyatnaroute" : "<API_KEY_FROM_DASHBOARD>");
          ...
          {!checkingDroid && cliReady && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current Base URL — first NiyatnaRoute entry, any index (#618) */}
                {droidStatus?.settings?.customModels?.find(isNiyatnaRouteEntry)?.baseUrl && (
                  <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={!droidStatus?.hasNiyatnaRoute}
                  loading={restoring}
                >
