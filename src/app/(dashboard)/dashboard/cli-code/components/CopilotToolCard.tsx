 ...
 *
 * Feature request: https://github.com/niyatnaroute/NiyatnaRoute/issues/142
 */
export default function CopilotToolCard({
  ...
  const [selectedModels, setSelectedModels] = useState<Set<string>>(() => {
    ...
    try {
      const saved = localStorage.getItem("niyatnaroute-copilot-selected-models");
  ...
  const [selectedApiKeyId, setSelectedApiKeyId] = useState(() => {
    if (typeof window !== "undefined") {
      const savedKey = localStorage.getItem("niyatnaroute-cli-key-copilot");
  ...
  useEffect(() => {
    if (selectedModels.size > 0) {
      localStorage.setItem(
        "niyatnaroute-copilot-selected-models",
  ...
  const generateConfig = () => {
    ...
    const config = {
      name: "NiyatnaRoute",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.niyatnaroute}`,
      models,
    };

    const responsesConfig = {
      name: "NiyatnaRoute-responses",
      vendor: "azure",
      apiKey: `\${input:chat.lm.secret.niyatnaroute}`,
  ...
  const handleApiKeyChange = (value: string) => {
    ...
    if (value) localStorage.setItem("niyatnaroute-cli-key-copilot", value);
  };
