...
export default function HermesAgentToolCard({
  ...
  const setRoleSelection = (roleId: string, model: string, provider = "NiyatnaRoute") => {
    setSelections((prev) => ({ ...prev, [roleId]: { model, provider } }));
  };

  const applyToAll = (model: string) => {
    const newSel: Record<string, RoleSelection> = {};
    HERMES_ROLES.forEach((r) => (newSel[r.id] = { model, provider: "NiyatnaRoute" }));
  ...
  const effectiveRoles = React.useMemo(() => {
    // If user has pending changes, treat selected roles as NiyatnaRoute
    if (Object.keys(selections).length > 0) {
      const map: Record<string, any> = {};
      HERMES_ROLES.forEach((r) => {
        if (selections[r.id]) {
          map[r.id] = { usingNiyatnaRoute: true };
  ...
  // Count of roles that are (or will be) routed via NiyatnaRoute
  const configuredRolesCount = HERMES_ROLES.filter((role) => {
    // Pending selection always counts as NiyatnaRoute intent
    if (selections[role.id]) return true;

    const info = effectiveRoles[role.id];
    if (!info) return false;

    // Support both shapes: detector shape (usingNiyatnaRoute) and settings shape (provider + base_url)
    if (typeof info.usingNiyatnaRoute === "boolean") {
      return info.usingNiyatnaRoute;
    }
    return (
      info?.provider === "niyatnaroute" ||
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            {HERMES_ROLES.map((role) => {
              ...
              // - If user has selected something in this session (pending): show as via NiyatnaRoute
              // - Else if current from disk: show real provider name + "(not NiyatnaRoute)" or "NiyatnaRoute"
              let badge: { label: string; pending: boolean; outsideNiyatnaRoute: boolean } | null =
                null;

              if (sel) {
                // pending change made via the Select modal / quick apply → will be routed via NiyatnaRoute
                const prov = sel.provider || "NiyatnaRoute";
                badge = {
                  label: t("hermesViaNiyatnaRoute", { provider: prov }),
                  pending: true,
                  outsideNiyatnaRoute: false,
                };
              } else if (current) {
                const isOmni =
                  current?.provider === "niyatnaroute" ||
                  (current?.base_url || "").includes("20128") ||
                  (current?.base_url || "").includes("localhost");

                if (isOmni) {
                  badge = { label: "NiyatnaRoute", pending: false, outsideNiyatnaRoute: false };
                } else {
                  const realProvider = current.provider || t("other");
                  badge = {
                    label: t("hermesNotNiyatnaRoute", { provider: realProvider }),
                    pending: false,
                    outsideNiyatnaRoute: true,
                  };
                }
              }
        ...
        onSelect={(model: any) => {
          if (modalRole) {
            ...
            if (modelValue) {
              ...
              const prov =
                (model && (model.provider || model.providerId || model.group)) || "NiyatnaRoute";
