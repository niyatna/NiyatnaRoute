...
// the `niyatnaroute setup-crush` CLI command agree on one canonical location.
...
/**
 * Crush's config uses a `providers.<id>` map. NiyatnaRoute is registered under
 * the `niyatnaroute` provider id as an `openai-compat` provider — same shape
...
const hasNiyatnaRouteConfig = (settings: Record<string, unknown> | null): boolean => {
  ...
  const niyatnaroute = providers?.niyatnaroute as Record<string, unknown> | undefined;
  return (
    !!niyatnaroute &&
    niyatnaroute.type === "openai-compat" &&
    typeof niyatnaroute.base_url === "string" &&
    niyatnaroute.base_url.length > 0
  );
};
...
// POST — write NiyatnaRoute settings to crush.json (providers.niyatnaroute)
...
// DELETE — remove NiyatnaRoute provider from Crush config
