...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [healthRes, combosRes] = await Promise.allSettled([
      fetch(`${NIYATNAROUTE_BASE_URL}/api/monitoring/health`, { signal: AbortSignal.timeout(5000) }),
      fetch(`${NIYATNAROUTE_BASE_URL}/api/combos`, { signal: AbortSignal.timeout(5000) }),
    ]);
    ...
    return NextResponse.json({
      ...
      source: "niyatnaroute-auto-combo",
    });
  } catch {
    return NextResponse.json({
      provider: {
