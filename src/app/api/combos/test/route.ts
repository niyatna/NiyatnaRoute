...
import { resolveNestedComboTargets } from "@niyatnaroute/open-sse/services/combo.ts";
    ...
    try {
      res = await fetch(internalUrl, {
        ...
        headers: {
          ...
          // NiyatnaRoute's semantic cache or other request reuse layers.
          "X-NiyatnaRoute-No-Cache": "true",
          ...(target.connectionId ? { "X-NiyatnaRoute-Connection": target.connectionId } : {}),
