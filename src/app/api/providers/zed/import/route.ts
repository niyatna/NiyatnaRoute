 ...
 *
 * For backwards compatibility, when `NIYATNA_ZED_IMPORT_LEGACY_ONE_STEP=true`
...
const LEGACY_ONE_STEP_ENABLED = process.env.NIYATNA_ZED_IMPORT_LEGACY_ONE_STEP === "true";
...
export async function POST(request: Request): Promise<NextResponse<ImportResponse> | Response> {
  ...
  try {
    ...
    if (confirmed !== null) {
      ...
      console.warn(
        "[Zed Import] NIYATNA_ZED_IMPORT_LEGACY_ONE_STEP=true — importing all keychain credentials without per-account confirmation. This mode is deprecated and will be removed in v3.9."
      );
    }
