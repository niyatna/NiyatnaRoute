...
import { estimateTokens } from "@niyatna/open-sse/services/contextManager.ts";
import { adaptBodyForCompression } from "@niyatna/open-sse/services/compression/bodyAdapter.ts";
import { resolveOmniGlyphTransport } from "@niyatna/open-sse/services/compression/imageTransportPolicy.ts";
import type {
  CompressionConfig,
  CompressionResult,
...
} from "@niyatna/open-sse/handlers/chatCore/compressionAnalyticsWrite.ts";
...
export async function applyResponsesWsCompression(
  ...
  try {
    ...
    const { selectCompressionStrategy, applyCompressionAsync } =
      await import("@niyatna/open-sse/services/compression/strategySelector.ts");
