...
import { NIYATNA_WEB_SEARCH_FALLBACK_TOOL_NAME } from "@niyatna/open-sse/services/webSearchFallback.ts";
import { NIYATNA_WEB_FETCH_FALLBACK_TOOL_NAME } from "@niyatna/open-sse/services/webFetchInterception.ts";
    ...
    toolCalls.map(async (call) => {
      try {
        ...
        if (builtinHandlerName) {
          ...
          const result = await builtinSkills[builtinHandlerName](
            call.arguments,
            {
...
export async function executeServerOwned(
  ...
  for (const call of calls) {
    ...
    const executeFn = async (executionId: string): Promise<unknown> => {
      if (builtinHandlerName && builtinHandlerName in builtinSkills) {
        return builtinSkills[builtinHandlerName](call.arguments, {
          apiKeyId: context.apiKeyId,
          sessionId: context.sessionId,
          provider: context.provider,
          model: context.model,
