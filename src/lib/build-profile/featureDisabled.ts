/**
 * Helper used by the `NIYATNAROUTE_BUILD_PROFILE=minimal` stubs to surface a
...
export class FeatureDisabledError extends Error {
  ...
  constructor(featureName: string) {
    super(
      `Feature "${featureName}" is disabled in this build (NIYATNAROUTE_BUILD_PROFILE=minimal). ` +
        `Install the full niyatnaroute artifact instead of niyatnaroute-secure if you need this feature.`
    );
