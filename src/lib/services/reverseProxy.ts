 ...
 * can be iframed inside the NiyatnaRoute dashboard without CORS issues.
 *
 * Security:
 ...
 *     to prevent credential leakage between NiyatnaRoute and the embedded service.
 ...
 * Prevents NiyatnaRoute session cookies and Authorization from leaking upstream.
 */
 ...
 *   NiyatnaRoute origin, which would conflict with session management.
 * - content-security-policy / content-security-policy-report-only: the
 *   embedded service's CSP is irrelevant inside the NiyatnaRoute iframe.
 * - x-frame-options: would block the iframe entirely if set to DENY/SAMEORIGIN
 *   by the embedded service (NiyatnaRoute controls framing via its own CSP).
