...
import { getCachedPassword, setCachedPassword } from "@/mitm/manager.stub";
 ...
 * Priority: system daemon socket → NiyatnaRoute custom socket.
...
async function getActiveSocketPath(): Promise<string> {
  ...
  // Fallback to NiyatnaRoute custom socket
...
async function getDefaultHostname() {
  try {
    ...
    const normalized = `niyatnaroute-${machineId.slice(0, 8)}`.replace(/[^a-zA-Z0-9-]/g, "-");
