...
import { cooldownUntilMs } from "@niyatnaroute/open-sse/services/accountFallback.ts";
...
declare global {
  var __niyatnarouteConnRecovery:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null }
    | undefined;
}

function getRecoveryState() {
  if (!globalThis.__niyatnarouteConnRecovery) {
    globalThis.__niyatnarouteConnRecovery = { initialized: false, interval: null };
  }
  return globalThis.__niyatnarouteConnRecovery;
}
...
function isRecoverySchedulerDisabled(): boolean {
  return (
    isEnvFlagEnabled("NIYATNAROUTE_DISABLE_CONNECTION_RECOVERY") ||
    isEnvFlagEnabled("NIYATNAROUTE_DISABLE_BACKGROUND_SERVICES") ||
    isBuildProcess() ||
...
/**
 * Resolve the tick interval (ms) from NIYATNAROUTE_CONNECTION_RECOVERY_INTERVAL_MS,
...
export function resolveConnectionRecoveryIntervalMs(
  rawValue: string | undefined = typeof process !== "undefined"
    ? process.env.NIYATNAROUTE_CONNECTION_RECOVERY_INTERVAL_MS
    : undefined
): number {
