...
export function getOmpCredentials(providerId: string) {
  ...
  if (!Database) return { hasNiyatnaRoute: false, baseUrl: null, apiKey: null };
  ...
  try {
    ...
    if (row?.data) {
      ...
      return { hasNiyatnaRoute: true, baseUrl: parsed.baseUrl || null, apiKey: parsed.apiKey || null };
    }
    return { hasNiyatnaRoute: false, baseUrl: null, apiKey: null };
  } catch {
    return { hasNiyatnaRoute: false, baseUrl: null, apiKey: null };
  }
}
