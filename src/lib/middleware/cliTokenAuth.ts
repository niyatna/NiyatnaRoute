...
const HEADER_NAME = "x-niyatnaroute-cli-token";
...
/**
 * Validates the CLI machine-id token sent by the local niyatnaroute CLI.
 * Only accepted from loopback IPs. Disabled via NIYATNAROUTE_DISABLE_CLI_TOKEN=true.
 */
export async function isCliTokenAuthValid(request: Request): Promise<boolean> {
  if (process.env.NIYATNAROUTE_DISABLE_CLI_TOKEN === "true") return false;
