...
declare global {
  var __niyatnaroutePreRequestRegistry:
    | {
        initialized: boolean;
...
function getRegistryState() {
  if (!globalThis.__niyatnaroutePreRequestRegistry) {
    globalThis.__niyatnaroutePreRequestRegistry = {
      initialized: false,
      hooks: new Map(),
      middlewares: new Map(),
      logs: [],
  ...
  return globalThis.__niyatnaroutePreRequestRegistry;
}
...
function compileHookCode(code: string, hookName: string): HookMiddleware {
  ...
  try {
    script = new vm.Script(`(async () => { ${code} })();`, {
      filename: `niyatnaroute-hook:${hookName}`,
    });
  } catch (err: unknown) {
