 ...
 * types, the provider-id mapping table, and the raw→NiyatnaRoute transform
...
// ─── Provider mapping: models.dev provider ID → NiyatnaRoute provider IDs/aliases ──
//
// models.dev uses canonical provider IDs (e.g. "openai", "anthropic", "google").
// NiyatnaRoute uses both full IDs and short aliases (e.g. "cc" for claude, "cx" for codex).
// We map each models.dev provider to ALL NiyatnaRoute identifiers that should receive
...
export const MODELS_DEV_PROVIDER_MAP: Record<string, string[]> = {
  ...
  // that is the historical alias pair. But NiyatnaRoute's catalog & combo targets
  ...
  // Additional providers that may overlap with NiyatnaRoute
  alibaba: ["ali", "alibaba"],
...
/**
 * Map a models.dev provider ID to NiyatnaRoute provider IDs.
...
/**
 * Transform models.dev raw data → NiyatnaRoute PricingByProvider format.
 *
 * models.dev costs are already in $/1M tokens (same as NiyatnaRoute format).
...
export function transformModelsDevToPricing(raw: ModelsDevData): PricingByProvider {
  ...
  for (const [providerId, providerData] of Object.entries(raw)) {
    const niyatnaRouteProviders = mapProviderId(providerId);
    ...
    for (const [modelId, model] of Object.entries(providerData.models || {})) {
      ...
      // Write to ALL mapped NiyatnaRoute providers
      for (const omniProvider of niyatnaRouteProviders) {
