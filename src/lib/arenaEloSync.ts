...
/**
 * Maps Arena leaderboard categories to NiyatnaRoute task-type categories.
 *
...
/**
 * NiyatnaRoute model aliases: canonical name → known aliases.
 * Creates additional DB entries for each alias so that models
 * are findable under any name NiyatnaRoute uses internally.
 */
 ...
 *
 * Leaderboard categories are mapped to NiyatnaRoute task types:
 * - "text" → default, review, documentation, debugging
 * - "code" → coding
 *
 * Known NiyatnaRoute model aliases are also expanded into additional entries.
 *
