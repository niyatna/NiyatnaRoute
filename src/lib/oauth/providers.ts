 ...
 * navigate back to NiyatnaRoute instead of stalling on localhost.
 */
export function resolveBrowserOAuthRedirectUri(
  ...
  const publicBaseUrl =
    normalizeBaseUrl(env.NEXT_PUBLIC_BASE_URL) || normalizeBaseUrl(env.NIYATNAROUTE_PUBLIC_BASE_URL);
