 ...
 * with the running NiyatnaRoute server when saving tokens.
 */
...
export function getServerCredentials(): ServerCredentials {
  return {
    server: process.env.NIYATNAROUTE_SERVER || process.env.SERVER_URL || getDefaultApiServer(),
    token: process.env.NIYATNAROUTE_TOKEN || process.env.CLI_TOKEN || "",
    userId: process.env.NIYATNAROUTE_USER_ID || process.env.CLI_USER_ID || "cli",
  };
}
