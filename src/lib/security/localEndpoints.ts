 ...
 * `globalThis.__niyatnaRequestHeaders` is not populated per-request, so the loopback/
 ...
 *
 * Rules (apply only when `__niyatnaRequestHeaders` is wired by the caller):
 ...
 *      NIYATNAROUTE_LOCAL_ENDPOINTS_ENABLED=1 is set. The flag is opt-in so
 *      accidental dev deployments do not expose the API.
 *   3. Trust-list the NiyatnaRoute desktop app via a shared bearer token
 *      (NIYATNAROUTE_LOCAL_ENDPOINTS_TOKEN). The desktop app injects the header
...
export function isLocalRequestAllowed(): { allowed: true } | { allowed: false; reason: string } {
  const headers = (globalThis as { __niyatnaRequestHeaders?: Headers }).__niyatnaRequestHeaders;
  if (headers) {
    // 1. Bearer token path (desktop app trust)
    const expected = process.env.NIYATNAROUTE_LOCAL_ENDPOINTS_TOKEN;
    if (expected) {
  ...
  // Production opt-in
  if (process.env.NODE_ENV === "production" && process.env.NIYATNAROUTE_LOCAL_ENDPOINTS_ENABLED !== "1") {
