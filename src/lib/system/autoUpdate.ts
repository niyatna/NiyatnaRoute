...
export function getAutoUpdateConfig(env: NodeJS.ProcessEnv = process.env): AutoUpdateConfig {
  const dataDir = env.DATA_DIR || "/tmp/niyatnaroute";
  const repoDir = env.AUTO_UPDATE_REPO_DIR || "/workspace/niyatnaroute";
  ...
  return {
    ...
    composeService: env.AUTO_UPDATE_SERVICE || "niyatnaroute-cli",
...
export function buildNpmUpdateScript(latest: string): string {
  return [
    ...
    `npm install -g niyatnaroute@${latest} --include=optional --ignore-scripts --legacy-peer-deps`,
    "if command -v pm2 >/dev/null 2>&1; then",
    "  pm2 restart niyatnaroute || true",
    "fi",
...
export function buildSourceUpdateScript(latest: string, gitRemote = "origin"): string {
  ...
  return [
    ...
    "npm run build",
    "if command -v pm2 >/dev/null 2>&1; then",
    "  pm2 restart niyatnaroute --update-env || true",
    "fi",
