 ...
 * ONLY from `npm info niyatnaroute version --json` (the `npm` CLI binary). When that binary
...
const NPM_REGISTRY_LATEST_URL = "https://registry.npmjs.org/niyatnaroute/latest";
...
const GITHUB_RELEASES_LATEST_URL =
  "https://api.github.com/repos/niyatnaroute/NiyatnaRoute/releases/latest";
...
export async function getLatestVersionFromNpmCli(): Promise<string | null> {
  try {
    ...
    const { stdout } = await execFileAsync(
      "npm",
      ["info", "niyatnaroute", "version", "--json"],
  ...
  try {
    const res = await fetchImpl(GITHUB_RELEASES_LATEST_URL, {
      ...
      headers: {
        ...
        "User-Agent": "niyatnaroute-version-check",
