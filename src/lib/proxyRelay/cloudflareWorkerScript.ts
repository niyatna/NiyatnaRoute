/**
 * Cloudflare Worker source emitter for the NiyatnaRoute proxy relay.
 *
 ...
 *
 * The NiyatnaRoute variant intentionally diverges from the upstream PR:
...
export function buildCloudflareWorkerUploadRequest(
  ...
  const boundary = `----NiyatnaRouteCFWorker${randomUUID().replace(/-/g, "")}`;
  const CRLF = "\r\n";
  const parts: Buffer[] = [
    Buffer.from(
      `--${boundary}${CRLF}` +
