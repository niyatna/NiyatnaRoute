 ...
 * source is the user's OWN NiyatnaRoute instance, so loopback/private hosts are the
...
/**
 * Fetch the live `/v1/models` catalog from NiyatnaRoute. The catalog is the
 ...
 */
export async function fetchNiyatnaRouteCatalog(
  baseUrl: string,
  apiKey: string,
  timeoutMs = 5_000
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `NiyatnaRoute /v1/models returned ${response.status} ${response.statusText}`
      );
    }
...
/**
 * Generate a full `opencode.json` document for NiyatnaRoute. The catalog is the
...
export async function generateOpencodeConfig(
  ...
  const providerId = options.providerId?.trim() || "niyatnaroute";
  ...
  if (fetchCatalog) {
    const result = await fetchNiyatnaRouteCatalog(baseURL, options.apiKey, timeoutMs);
