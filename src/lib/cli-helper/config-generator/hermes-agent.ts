 ...
 *   model: string;                    // the model name the user chose from NiyatnaRoute
 * }
 *
 * interface HermesAgentConfigPayload {
 *   baseUrl: string;                  // usually the NiyatnaRoute base URL
...
export async function generateHermesAgentConfig(
  ...
  const resolvedKey = apiKey || "YOUR_NIYATNAROUTE_API_KEY_HERE";
  ...
  // Build the providers.niyatnaroute entry (shared)
  const normalizedBase = normalizeBaseUrl(baseUrl);
  const niyatnarouteProvider = {
  ...
  const next: any = {
    ...
    providers: {
      ...(existing.providers || {}),
      niyatnaroute: niyatnarouteProvider,
    },
  };
