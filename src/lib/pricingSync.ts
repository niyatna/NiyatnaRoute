...
// ─── Provider mapping: LiteLLM provider → NiyatnaRoute aliases ─────
...
/**
 * Transform LiteLLM raw data → NiyatnaRoute PricingByProvider format.
 *
 * Conversion: cost_per_token × 1_000_000 → $/1M tokens (NiyatnaRoute format).
 ...
 */
export function transformToNiyatnaRoute(raw: Record<string, LiteLLMModelInfo>): PricingByProvider {
  ...
  for (const [modelKey, info] of Object.entries(raw)) {
    ...
    // Map to NiyatnaRoute providers
    const litellmProvider = info.litellm_provider || "";
    const niyatnaRouteProviders = LITELLM_PROVIDER_MAP[litellmProvider];

    if (niyatnaRouteProviders) {
      for (const provider of niyatnaRouteProviders) {
  ...
  try {
    ...
    for (const source of validSources) {
      if (source === "litellm") {
        ...
        const transformed = transformToNiyatnaRoute(raw);
