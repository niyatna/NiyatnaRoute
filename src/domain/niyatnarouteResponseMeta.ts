...
export function formatNiyatnaRouteCost(costUsd: unknown): string {
  const normalized = toFiniteNumber(costUsd);
  return normalized > 0 ? normalized.toFixed(10) : "0.0000000000";
}

/**
 * Build the `X-NiyatnaRoute-Decision` composite header value: `strategy=<name>;
 ...
 * (same alias normalization the `X-NiyatnaRoute-Provider` header already applies)
...
export function buildNiyatnaRouteResponseMetaHeaders({
   ...
   * Emitted as `X-NiyatnaRoute-Cost-Saved` only when provided (omitted on normal
   ...
   * request) surfaced via `X-NiyatnaRoute-Decision`. See #6022.
   */
  strategy?: string | null;
  usage?: UsageLike;
  ...
  const headers: Record<string, string> = {
    [NIYATNAROUTE_RESPONSE_HEADERS.cacheHit]: toHeaderValue(String(cacheHit)),
    [NIYATNAROUTE_RESPONSE_HEADERS.latencyMs]: toHeaderValue(String(toNonNegativeInteger(latencyMs))),
    [NIYATNAROUTE_RESPONSE_HEADERS.responseCost]: toHeaderValue(formatNiyatnaRouteCost(costUsd)),
    [NIYATNAROUTE_RESPONSE_HEADERS.tokensIn]: toHeaderValue(String(tokens.input)),
    [NIYATNAROUTE_RESPONSE_HEADERS.tokensOut]: toHeaderValue(String(tokens.output)),
    [NIYATNAROUTE_RESPONSE_HEADERS.version]: toHeaderValue(APP_CONFIG.version),
  };

  if (typeof model === "string" && model.trim().length > 0) {
    headers[NIYATNAROUTE_RESPONSE_HEADERS.model] = toHeaderValue(model);
  }

  if (typeof requestId === "string" && requestId.trim().length > 0) {
    headers[NIYATNAROUTE_RESPONSE_HEADERS.requestId] = toHeaderValue(requestId);
  }

  if (typeof provider === "string" && provider.trim().length > 0) {
    headers[NIYATNAROUTE_RESPONSE_HEADERS.provider] = toHeaderValue(getProviderAlias(provider));
  }
  ...
  if (costSavedUsd != null) {
    headers[NIYATNAROUTE_RESPONSE_HEADERS.costSaved] = toHeaderValue(
      formatNiyatnaRouteCost(costSavedUsd)
    );
  }

  const attempts = toNonNegativeInteger(fallbackAttempts);
  if (attempts > 0) {
    headers[NIYATNAROUTE_RESPONSE_HEADERS.fallbackAttempts] = toHeaderValue(String(attempts));
  }

  const decisionValue = buildNiyatnaRouteDecisionHeaderValue({ strategy, provider, latencyMs });
  if (decisionValue !== null) {
    headers[NIYATNAROUTE_RESPONSE_HEADERS.decision] = decisionValue;
  }
...
/**
 * Single choke-point for attaching the X-NiyatnaRoute-* response meta headers.
...
/**
 * Attach the X-NiyatnaRoute-* meta headers onto an already-built Response, ADDING
