...
export const NIYATNAROUTE_PEER_TRACE_HEADER = "X-NiyatnaRoute-Peer-Trace";

type HeaderSource = Headers | Record<string, unknown> | null | undefined;
type PeerEnvironment = {
  NIYATNAROUTE_INSTANCE_ID?: string;
  NIYATNAROUTE_PEER_URLS?: string;
  NIYATNAROUTE_PEER_MAX_HOPS?: string;
};
...
function getInstanceId(env: PeerEnvironment): string | null {
  const value = env.NIYATNAROUTE_INSTANCE_ID?.trim() ?? "";
  return INSTANCE_ID_PATTERN.test(value) ? value : null;
}

function getMaxPeerHops(env: PeerEnvironment): number {
  const parsed = Number.parseInt(env.NIYATNAROUTE_PEER_MAX_HOPS ?? "", 10);
...
export function isConfiguredNiyatnaRoutePeer(
  ...
  return (env.NIYATNAROUTE_PEER_URLS ?? "")
    .split(",")
...
export function inspectPeerRequest(
  ...
  const trace = parsePeerTrace(readHeader(headers, NIYATNAROUTE_PEER_TRACE_HEADER));
...
/**
 * Append this instance to the peer trace for an explicitly allowlisted NiyatnaRoute URL.
...
export function applyPeerTraceHeader(
  ...
  const trace = parsePeerTrace(readHeader(clientHeaders, NIYATNAROUTE_PEER_TRACE_HEADER));
  if (!trace.includes(instanceId)) trace.push(instanceId);
  const traceHeaderLower = NIYATNAROUTE_PEER_TRACE_HEADER.toLowerCase();
  for (const key of Object.keys(outgoingHeaders)) {
    if (key.toLowerCase() === traceHeaderLower) delete outgoingHeaders[key];
  }
  outgoingHeaders[NIYATNAROUTE_PEER_TRACE_HEADER] = trace.join(",");
  return true;
}
