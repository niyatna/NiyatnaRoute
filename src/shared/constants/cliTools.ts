...
import { getClaudeCodeDefaultModels } from "@niyatnaroute/open-sse/config/providerRegistry";
...
export const CLI_TOOLS: Record<string, CliCatalogEntry> = {
  ...
  opencode: {
    ...
    notes: [
      ...
      {
        type: "warning",
        text: 'Thinking variant example: opencode run "implement this feature" --model niyatnaroute/claude-sonnet-4-5-thinking --variant high',
      },
    ],
    guideSteps: [
  ...
  qwen: {
    ...
    notes: [
      ...
      {
        type: "info",
        text: "The API key is stored only as NIYATNAROUTE_API_KEY in ~/.qwen/.env, leaving your existing provider credentials untouched.",
      },
    ],
    guideSteps: [
