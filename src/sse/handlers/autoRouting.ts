...
import type { AutoVariant } from "@niyatna/open-sse/services/autoCombo/autoPrefix.ts";
import {
  AUTO_TEMPLATE_VARIANTS,
  VALID_AUTO_VARIANTS,
} from "@niyatna/open-sse/services/autoCombo/builtinCatalog.ts";
import {
  parseAutoSuffix,
  type AutoCategory,
  type AutoTier,
} from "@niyatna/open-sse/services/autoCombo/suffixComposition.ts";
import {
  isValidModelFamily,
  type ModelFamily,
} from "@niyatna/open-sse/services/autoCombo/modelFamily.ts";
...
async function applyAutoPrefix(
  ...
  try {
    const { parseAutoPrefix } =
      await import("@niyatna/open-sse/services/autoCombo/autoPrefix.ts");
