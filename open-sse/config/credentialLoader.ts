...
type CredGlobals = typeof globalThis & { __niyatnarouteCredNoFileLogged?: boolean };
...
export function loadProviderCredentials<T extends Record<string, unknown>>(providers: T): T {
  ...
  if (!existsSync(credPath)) {
    if (!credGlobals().__niyatnarouteCredNoFileLogged) {
      console.log("[CREDENTIALS] No external credentials file found, using defaults.");
      credGlobals().__niyatnarouteCredNoFileLogged = true;
    }
