export const PROVIDER_PLUGIN_MANIFEST_HEADER = "X-NiyatnaRoute-Provider-Manifest-Url";
...
export function resolveProviderPluginManifestUrl(origin?: string | null): string {
  const configured = process.env.NIYATNAROUTE_PROVIDER_MANIFEST_URL?.trim();
  ...
  const protocol = process.env.NIYATNAROUTE_PUBLIC_PROTOCOL || "http";
