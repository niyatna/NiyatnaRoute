...
// #1731v2 guard: a provider circuit-breaker-open response (503 + `X-NiyatnaRoute-Provider-Breaker`
// header / `provider_circuit_open` error code, see providerCircuitOpenResponse) is an NiyatnaRoute
...
export function isProviderCircuitOpenResult(
  ...
  const breakerHeader = result.headers?.get?.("x-niyatnaroute-provider-breaker");
