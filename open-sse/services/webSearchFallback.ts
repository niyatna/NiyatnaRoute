...
export const NIYATNAROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME = "niyatnaroute_web_search";
...
function buildFallbackTool(tool: JsonRecord, targetFormat?: string | null): JsonRecord {
  const name = NIYATNAROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME;
...
// built-in web-search tool has to be converted to the niyatnaroute_web_search function
...
export function supportsNativeWebSearchFallbackBypass({
  ...
  // native tool untouched instead of rewriting it to niyatnaroute_web_search. Mirrors the
...
export function prepareWebSearchFallbackBody<T extends JsonRecord>(
  ...
  if (!toolNames.has(NIYATNAROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME)) {
  ...
  if (isBuiltInWebSearchToolChoice(body.tool_choice)) {
    ...
    nextBody.tool_choice = (
      isResponsesTarget
        ? { type: "function", name: NIYATNAROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME }
        : { type: "function", function: { name: NIYATNAROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME } }
    ) as T["tool_choice"];
  }
  ...
  return {
    ...
    fallback: {
      enabled: true,
      toolName: NIYATNAROUTE_WEB_SEARCH_FALLBACK_TOOL_NAME,
