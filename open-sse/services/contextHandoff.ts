...
export const SKIP_UNIVERSAL_HANDOFF_FLAG = "_niyatnarouteSkipUniversalHandoff";
...
async function generateHandoffAsync(options: {
  ...
  const summaryBody = {
    ...
    temperature: 0.1,
    _niyatnarouteSkipContextRelay: true,
    _niyatnarouteInternalRequest: "context-handoff",
  };
...
async function generateUniversalHandoffAsync(options: {
  ...
  const summaryBody: Record<string, unknown> = {
    ...
    temperature: 0.1,
    _niyatnarouteSkipContextRelay: true,
    _niyatnarouteInternalRequest: "universal-handoff",
  };
