 ...
 *
 *   X-NiyatnaRoute-Mode:            fast | balanced | quality | <raw mode-pack name>  (#6024/#6025)
 *   X-NiyatnaRoute-Budget:          <max USD per request>                             (#6023)
 *   X-NiyatnaRoute-Budget-Fallback: cheapest | strict                                 (#3470)
 *
...
/**
 * Resolve the `X-NiyatnaRoute-Mode` header value into a mode-pack override.
 *
...
/**
 * Parse the `X-NiyatnaRoute-Budget` header into a hard per-request cost ceiling (USD).
...
/**
 * Parse the `X-NiyatnaRoute-Budget-Fallback` header into a budget-fallback policy override.
...
export function resolveRequestAutoControls(headers: {
  ...
  const modeHeader = headers.get("x-niyatnaroute-mode")?.trim() || null;
  const budgetHeader = headers.get("x-niyatnaroute-budget")?.trim() || null;
  const budgetFallbackHeader = headers.get("x-niyatnaroute-budget-fallback")?.trim() || null;
