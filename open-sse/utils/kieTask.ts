...
const FALLBACK_KIE_CALLBACK_URL = "https://niyatnaroute.local/api/kie/callback";
...
function getConfiguredKieCallbackUrl(): string {
  const explicit =
    process.env.KIE_CALLBACK_URL?.trim() || process.env.NIYATNAROUTE_KIE_CALLBACK_URL?.trim();
  if (explicit) return explicit;

  return (
    callbackUrlFromBaseUrl(process.env.NIYATNAROUTE_PUBLIC_URL) ||
