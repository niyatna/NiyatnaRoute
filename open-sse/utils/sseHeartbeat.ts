 ...
 * Set NIYATNA_SSE_COMMENTS=on to enable comment-shaped heartbeats and telemetry trailers.
 * #10524: defaults to disabled — strict SSE clients (WorkBuddy, etc.) break on `: x-omniroute-*`
 * comment lines. Operators who want the telemetry can opt in with NIYATNA_SSE_COMMENTS=on.
 */
export function sseCommentsEnabled(): boolean {
  // SSR/edge safety: `process` is not defined in Workers/Deno/edge runtimes.
  if (typeof process === "undefined") return false;
  const v = process.env.NIYATNA_SSE_COMMENTS;
...
export function createSseHeartbeatTransform({
  ...
  // crash on `:` comment heartbeats. NIYATNA_SSE_COMMENTS=off disables comment-shaped
