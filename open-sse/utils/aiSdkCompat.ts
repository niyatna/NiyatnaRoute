 ...
 * `stream`. Without this hint, NiyatnaRoute previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // NiyatnaRoute accumulates the provider stream and converts it to JSON for the client
  ...
  // NiyatnaRoute fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve NiyatnaRoute's streaming default unless
