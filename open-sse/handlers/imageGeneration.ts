...
export async function handleOpenAIImageEdit({
  ...
  const boundary = `----NiyatnaRouteImageEdit${randomUUID().replace(/-/g, "")}`;
  const CRLF = "\r\n";
...
export async function handleImageEdit({
  ...
  if (!cached?.entry.context) {
    ...
    return saveImageErrorResult({
      ...
      startTime,
      error:
        "chatgpt-web image edit only works for images recently generated through this NiyatnaRoute instance " +
