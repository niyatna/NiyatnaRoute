 ...
 * `x-niyatnaroute-no-memory` header. Mirrors the existing `x-niyatnaroute-no-cache`
...
export function isNoMemoryRequested(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-niyatnaroute-no-memory") || "")
    .trim()
    .toLowerCase();
...
/**
 * Per-request compression override via the `x-niyatnaroute-compression` header. Mirrors the
 * `x-niyatnaroute-no-memory` convention (#4290). Returns the raw trimmed value, or null when
...
export function resolveCompressionHeader(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-niyatnaroute-compression") || "").trim();
  return value || null;
}

/**
 * Per-request opt-in to unconditionally strip `reasoning_content` from the
 * non-streaming JSON response via the `x-niyatnaroute-strip-reasoning` header.
...
export function isStripReasoningRequested(
  ...
  const value = (getHeaderValueCaseInsensitive(headers, "x-niyatnaroute-strip-reasoning") || "")
    .trim()
    .toLowerCase();
  return value === "true" || value === "1" || value === "yes";
}
