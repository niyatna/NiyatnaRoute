 ...
 * shapes), and attach an `_niyatnaroute` meta blob to a log payload. Side-effect-free; behaviour is
...
export function attachLogMeta(
  ...
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { _niyatnaroute: compactMeta, _payload: payload ?? null };
  }
  const existing =
    payload._niyatnaroute &&
    typeof payload._niyatnaroute === "object" &&
    !Array.isArray(payload._niyatnaroute)
      ? payload._niyatnaroute
      : {};
  return {
    ...payload,
