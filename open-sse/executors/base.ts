...
/**
 * Strip the NiyatnaRoute provider prefix from versioned built-in tool model
 ...
 * leaked NiyatnaRoute prefix makes Anthropic reject the request. Mutates in place.
 */
      ...
      try {
        ...
        if (
          ...
          if (Array.isArray(tb.tools)) {
            ...
            // Also strip NiyatnaRoute provider prefix from versioned built-in tool
          ...
          //   x-niyatnaroute-effort:   low | medium | high | xhigh | max | off
          //   x-niyatnaroute-thinking: adaptive | off
          // A header value applies only when the corresponding body field is
          // not already set; "off" force-strips the field.
          const headerEffort = (
            clientHeaders?.["x-niyatnaroute-effort"] ?? clientHeaders?.["X-NiyatnaRoute-Effort"]
          )
            ?.trim()
            .toLowerCase();
          const headerThinking = (
            clientHeaders?.["x-niyatnaroute-thinking"] ?? clientHeaders?.["X-NiyatnaRoute-Thinking"]
          )
            ?.trim()
            .toLowerCase();
          ...
          if (effThinking === "adaptive") {
            ...
            // must opt in with x-niyatnaroute-thinking; force-injecting adaptive thinking
          ...
          // Anthropic correlate accounts behind one NiyatnaRoute.
          ...
          // is pinned to the captured CLI wire image, not NiyatnaRoute's Node.
