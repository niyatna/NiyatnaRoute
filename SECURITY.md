...
If you discover a security vulnerability in NiyatnaRoute, please report it responsibly:

1. **DO NOT** open a public GitHub issue
2. Use [GitHub Security Advisories](https://github.com/niyatnaroute/NiyatnaRoute/security/advisories/new)
...
NiyatnaRoute implements a multi-layered security model:
...
NiyatnaRoute ships a hot-reloadable **guardrails registry** (`src/lib/guardrails/`) with 3 built-in guardrails ordered by priority:
...
Custom guardrails register via `registerGuardrail(new MyGuardrail())`. The model is fail-open (exceptions never block traffic). Per-request opt-out via `x-niyatnaroute-disabled-guardrails` header. → See [`docs/security/GUARDRAILS.md`](docs/security/GUARDRAILS.md).
...
The published `niyatnaroute` npm artifact bundles the Next.js `output: "standalone"`
...
`NIYATNAROUTE_BUILD_PROFILE=minimal npm run build`. That replaces the four
