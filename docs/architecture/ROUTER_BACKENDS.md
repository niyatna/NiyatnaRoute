...
> **Status:** Accepted · **Context:** [#5670](https://github.com/niyatnaroute/NiyatnaRoute/issues/5670),
> [#5603](https://github.com/niyatnaroute/NiyatnaRoute/issues/5603) · **Contract:** `domain/routing/routerBackends.ts`
> (typed registry — code lands with [#5868](https://github.com/niyatnaroute/NiyatnaRoute/pull/5868))
...
1. **Lifecycle** (`RouterBackendLifecycle`) — _how the engine runs_:
   - `in-process` — runs inside the NiyatnaRoute Node process (the native TS pipeline).
   - `supervised` — a local child process NiyatnaRoute installs/starts/stops/health-checks
     via `ServiceSupervisor`, then consumes as a provider connection.
   - `external` — an HTTP endpoint NiyatnaRoute dispatches to but does **not** manage
...
[#5868](https://github.com/niyatnaroute/NiyatnaRoute/pull/5868)) declares every engine once, with its
...
[#5817](https://github.com/niyatnaroute/NiyatnaRoute/pull/5817); until it merges,
...
- **Selection** (`resolveRelayRoutingBackend`): a single global env toggle —
  `NIYATNAROUTE_RELAY_BACKEND` / `RELAY_ROUTING_BACKEND` ∈ {`ts`, `bifrost`, `auto`}.
...
([#5869](https://github.com/niyatnaroute/NiyatnaRoute/pull/5869) manifest +
[#5870](https://github.com/niyatnaroute/NiyatnaRoute/pull/5870) `shouldTryBifrostForRequest`),
