...
NiyatnaRoute can fire HTTP webhooks on platform events. Use them to integrate with
...
When a webhook has a `secret`, NiyatnaRoute signs the JSON body and sends:
...
> Header names use the `X-Webhook-*` prefix (not `X-NiyatnaRoute-*`). The signature
