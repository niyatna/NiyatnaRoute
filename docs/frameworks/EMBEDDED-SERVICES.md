...
Embedded services are locally-installed process sidecar tools that NiyatnaRoute installs, supervises, and
exposes as first-class routing targets. Unlike external providers (which are reached over the internet
via API keys), embedded services run on the same machine as NiyatnaRoute and communicate over loopback.
...
| --------------- | ----------------------------------------------- | :----------: | ------------------------------------------------------------------------------------------------------------------ |
| **9Router**     | `9router`                                      |    20130     | AI router that NiyatnaRoute can use as a sub-provider. Models exposed as `9router/{sub}/{model}`                     |
...
- NiyatnaRoute installs them under `DATA_DIR/services/{name}/` (isolated from NiyatnaRoute's own `package.json`)
- NiyatnaRoute spawns and monitors them as child processes
- NiyatnaRoute injects an ephemeral API key into the child's environment and rotates it without downtime (where applicable)
...
| API key management                    | NiyatnaRoute generates, encrypts at-rest (AES-256-GCM), and injects via env |
...
`package.json` and `node_modules/`. Does not conflict with NiyatnaRoute's own deps.
...
the next time NiyatnaRoute boots (if the service is installed).
...
- Strips incoming `cookie` and `authorization` headers (no leakage of NiyatnaRoute session)
...
2. Verify `npm` is in PATH: `which npm` from the same user account that runs NiyatnaRoute.
...
NiyatnaRoute's default port 20128.
...
- `DATA_DIR` or its parent is not writable by the NiyatnaRoute process.
- Running inside Docker rootless without write access to the mapped volume.

**Fix:**

1. Check `DATA_DIR` (default: `~/.niyatnaroute/`): `ls -la ~/.niyatnaroute/`
2. Ensure the NiyatnaRoute process user owns the directory: `chown -R $USER ~/.niyatnaroute/`
...
3. Try the install manually: `npm install {package}@latest --prefix ~/.niyatnaroute/services/{name}/`.
...
Yes. Both services follow the same local-first model as NiyatnaRoute itself. They run
on the same machine and communicate over loopback. "Production" here means the VPS
or local server where NiyatnaRoute is deployed, not a remote cloud provider.
...
2. Check structured logs in NiyatnaRoute's pino output filtered by
   `service:supervisor` namespace.
3. Inspect the DB row: `sqlite3 ~/.niyatnaroute/niyatnaroute.db "SELECT * FROM version_manager WHERE tool='9router'"`.
