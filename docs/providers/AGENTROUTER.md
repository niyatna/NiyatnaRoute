...
For most users, **no special setup is required**. NiyatnaRoute ships a built-in
...
NiyatnaRoute also supports AgentRouter (and similar relays) through the **Claude Code
...
- NiyatnaRoute running with the `ENABLE_CC_COMPATIBLE_PROVIDER` feature flag enabled
  (see below).
...
setting an environment variable before starting NiyatnaRoute:
...
```bash
docker run -d --name niyatnaroute \
...
**`Invalid JSON response from provider (reset after Ns)` from the niyatnaroute logs** —
...
model actually resolves in the niyatnaroute logs (`ROUTING` tag shows
