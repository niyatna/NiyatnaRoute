---
title: "NiyatnaRoute — Deployment Guide on VM with Cloudflare"
version: 3.8.40
lastUpdated: 2026-06-28
---

# NiyatnaRoute — Deployment Guide on VM with Cloudflare
...
Complete guide to install and configure NiyatnaRoute on a VM (VPS) with domain managed via Cloudflare.
...
## 2. Install NiyatnaRoute

### 2.1 Create configuration directory

```bash
mkdir -p /opt/niyatnaroute
```

### 2.2 Create environment variables file

```bash
cat > /opt/niyatnaroute/.env << 'EOF'
# === Security ===
...
NIYATNAROUTE_WS_BRIDGE_SECRET=REPLACE-WITH-WS-BRIDGE-SECRET  # REQUIRED em produção: usado pelo Codex Responses WS bridge
...
# NIYATNAROUTE_PUBLIC_BASE_URL=https://llms.seudominio.com

# === Cloud Sync (optional) ===
# CLOUD_URL=https://cloud.niyatnaroute.online
# NEXT_PUBLIC_CLOUD_URL=https://cloud.niyatnaroute.online
EOF
```
...
```bash
docker pull niyatnaroute/niyatnaroute:latest

docker run -d \
  --name niyatnaroute \
  --restart unless-stopped \
  --env-file /opt/niyatnaroute/.env \
  -p 20128:20128 \
...
```bash
docker ps | grep niyatnaroute
docker logs niyatnaroute --tail 20
```
...
```bash
cat > /etc/nginx/sites-available/niyatnaroute << 'NGINX'
...
Keep reverse-proxy stream timeouts aligned with your NiyatnaRoute timeout env vars. If you raise
`FETCH_TIMEOUT_MS` / `STREAM_IDLE_TIMEOUT_MS`, raise `proxy_read_timeout` / `proxy_send_timeout`
above the same threshold.

NiyatnaRoute uses `NEXT_PUBLIC_BASE_URL` as the canonical browser-facing origin for OAuth
...
`NIYATNAROUTE_TRUST_PROXY` if NiyatnaRoute is not directly reachable by clients and your proxy
...
# Enable NiyatnaRoute
ln -sf /etc/nginx/sites-available/niyatnaroute /etc/nginx/sites-enabled/niyatnaroute
...
```bash
docker pull niyatnaroute/niyatnaroute:latest
docker stop niyatnaroute && docker rm niyatnaroute
docker run -d --name niyatnaroute --restart unless-stopped \
  --env-file /opt/niyatnaroute/.env \
  -p 20128:20128 \
  -v niyatnaroute-data:/app/data \
  niyatnaroute/niyatnaroute:latest
...
```bash
docker logs -f niyatnaroute          # Real-time stream
docker logs niyatnaroute --tail 50   # Last 50 lines
```
...
docker cp niyatnaroute:/app/data ./backup-$(date +%F)

# Or compress the entire volume
docker run --rm -v niyatnaroute-data:/data -v $(pwd):/backup \
  alpine tar czf /backup/niyatnaroute-data-$(date +%F).tar.gz /data
```

### Restore from backup

```bash
docker stop niyatnaroute
docker run --rm -v niyatnaroute-data:/data -v $(pwd):/backup \
  alpine sh -c "rm -rf /data/* && tar xzf /backup/niyatnaroute-data-YYYY-MM-DD.tar.gz -C /"
docker start niyatnaroute
```
...
cd niyatnarouteCloud
npm install
npx wrangler login
npx wrangler deploy
```

See also [TUNNELS_GUIDE.md](./TUNNELS_GUIDE.md) for the in-repo Cloudflare Tunnel walkthrough. The standalone `niyatnarouteCloud/` worker lives in a separate companion repo.
...
| 20128 | NiyatnaRoute   | Localhost only (via nginx) |
