...
> **TL;DR**: NiyatnaRoute uses **SQLite with WAL journaling** as its primary store, with **AES-256-GCM** encryption at rest for sensitive fields. This guide covers the schema, migrations, backup/recovery, and operational runbooks.
...
NiyatnaRoute chose SQLite over PostgreSQL/MySQL for several reasons:
...
For **single-user, single-instance** deployments (the primary NiyatnaRoute use case), SQLite is simpler and faster.
...
| OS      | Path                                                     |
| ------- | -------------------------------------------------------- |
| Linux   | `~/.niyatnaroute/storage.sqlite`                            |
| macOS   | `~/.niyatnaroute/storage.sqlite`                            |
| Windows | `%USERPROFILE%\.niyatnaroute\storage.sqlite`                |
...
```bash
DATA_DIR=/custom/path niyatnaroute
```
...
NiyatnaRoute's database has **94 domain modules** in `src/lib/db/`. Each module:
...
NiyatnaRoute has **94 module files** in `src/lib/db/`. Below is a sampling of core modules; see the directory listing for the complete list:
...
NiyatnaRoute uses **versioned, idempotent migrations** in `src/lib/db/migrations/`. Each migration is a single SQL file named `NNN_description.sql`.
...
1. Creates `_niyatnaroute_migrations` table if not exists
...
> **Backwards-incompatible changes** (e.g., dropping columns) are tricky. NiyatnaRoute does NOT support downgrade — once a migration is applied, the schema change is permanent. Plan accordingly.
...
NiyatnaRoute uses **`migrateLegacyEncryptedString()`** to handle two encryption schemes transparently:
...
niyatnaroute backup create --name pre-migration
...
```bash
# Via CLI
niyatnaroute restore pre-migration
...
niyatnaroute backup auto enable --cron "0 2 * * *" --retention 7
```
...
```bash
sqlite3 ~/.niyatnaroute/storage.sqlite ".backup /backups/niyatnaroute-hot.db"
```

This uses SQLite's online backup API — safe to run while NiyatnaRoute is running.
...
Long-running NiyatnaRoute instances benefit from occasional `VACUUM`:

```bash
sqlite3 ~/.niyatnaroute/storage.sqlite "VACUUM;"
```
...
```bash
sqlite3 ~/.niyatnaroute/storage.sqlite "PRAGMA integrity_check;"
# Should print: ok
```
...
```bash
sqlite3 ~/.niyatnaroute/storage.sqlite ".recover" > recovered.sql
sqlite3 recovered.db < recovered.sql
mv recovered.db ~/.niyatnaroute/storage.sqlite
```
...
```bash
niyatnaroute sync pull --merge   # or: niyatnaroute backup restore <backup-id>
```
...
sqlite3 ~/.niyatnaroute/storage.sqlite "PRAGMA wal_checkpoint(TRUNCATE);"
```
...
```bash
sqlite3 ~/.niyatnaroute/storage.sqlite "SELECT * FROM api_keys LIMIT 5;"
```

### Count Rows in All Tables

```bash
sqlite3 ~/.niyatnaroute/storage.sqlite <<EOF
...
```bash
# Stop NiyatnaRoute first
niyatnaroute stop

# Delete the DB file
rm ~/.niyatnaroute/storage.sqlite*
...
- If persistent, restart NiyatnaRoute
...
1. **Stop NiyatnaRoute** (prevent further attempts)
2. **Check the DB state** with `sqlite3`
3. **Manually fix** the partial migration
4. **Re-run** NiyatnaRoute (the migration will be retried)
