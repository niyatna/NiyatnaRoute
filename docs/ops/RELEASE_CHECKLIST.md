...
1. `npm stage list niyatnaroute` — find the stage id (also printed in the workflow summary).
...
`niyatnaroute` in stage-only mode so a leaked long-lived token cannot `npm publish`
directly from anywhere — CI can only stage; only the owner's 2FA releases.

**Broken-artifact playbook (unchanged):** `npm deprecate niyatnaroute@<bad> "<reason> — use <fixed>"`
...
- [ ] If source English docs changed, run `npm run i18n:run` (requires `NIYATNAROUTE_TRANSLATION_API_KEY` in `.env`) before tagging
...
- [ ] Test on fresh install: delete `~/.niyatnaroute/niyatnaroute.db` and run `npm run dev`
...
> **Operator note:** the remote VPS image directory remains `/usr/lib/node_modules/niyatnaroute/app/`.
...
- [ ] `niyatnaroute --tray` boots on macOS (systray2 installed into `~/.niyatnaroute/runtime/`)
- [ ] `niyatnaroute --tray` boots on Linux (requires DISPLAY; graceful error if not set)
- [ ] `niyatnaroute --tray` boots on Windows (PowerShell NotifyIcon, no extra binaries)
- [ ] `niyatnaroute config tray enable` creates autostart entry; disable removes it
- [ ] `npm install -g niyatnaroute@<this-version>` runs postinstall without fatal exit
- [ ] Update path keeps optional deps: `niyatnaroute update --apply` and the auto-updater
...
- [ ] `niyatnaroute status` works with no `.env` (CLI token path, loopback only)
...
- [ ] All 10 `skills/niyatnaroute*/SKILL.md` files are publicly fetchable via raw GitHub URL
