...
NiyatnaRoute can expose its local server (`http://localhost:20128`) to the public
...
All three backends are managed in-process — NiyatnaRoute starts/stops the underlying
...
- **Auto-install.** On first use, NiyatnaRoute downloads the latest `cloudflared`
...
login` + `cloudflared tunnel route dns ...`) are not orchestrated by
  NiyatnaRoute. URLs are ephemeral and will change every restart.
...
| Variable                                             | Purpose                                                                               |
| ---------------------------------------------------- | ------------------------------------------------------------------------------------- |
| `CLOUDFLARED_BIN`                                    | Override the binary path. If set and valid, NiyatnaRoute uses it instead of downloading. |
...
must be configured in the ngrok dashboard — NiyatnaRoute itself only forwards the
...
1. Install Tailscale (or let NiyatnaRoute do it — see `install` endpoint below).
2. Sign in (`tailscale login` or via NiyatnaRoute's `login` endpoint).
...
NiyatnaRoute's MITM password cache (`getCachedPassword` / `setCachedPassword`) for
...
When you expose NiyatnaRoute through a tunnel, the dashboard and OAuth flows must
...
and restart NiyatnaRoute before initiating OAuth. For ephemeral Cloudflare Quick
...
than one backend simultaneously is allowed; NiyatnaRoute will track each
independently.
...
NiyatnaRoute attempts to auto-install on first use. If the install is blocked
