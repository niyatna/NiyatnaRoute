...
> **TL;DR**: NiyatnaRoute ships with built-in health monitoring, provider autopilot, quota tracking, and observability hooks. This guide covers the dashboard, alerts, and troubleshooting.
...
NiyatnaRoute has **3 layers of monitoring**:
...
│  └─ MCP tools: niyatnaroute_get_health, niyatnaroute_get_session_snapshot │
└──────────────────────────────────────────────────────────────┘
```
...
NiyatnaRoute supports **3 alert channels**:
...
Create a `~/.niyatnaroute/dashboard.json`:
...
- NiyatnaRoute's quota tracking may be **stale** — the provider's truth is upstream
...
- Run `sqlite3 ~/.niyatnaroute/storage.sqlite "PRAGMA integrity_check;"`
- If "ok" — false alarm, the health check is being too strict
- If anything else — **stop NiyatnaRoute** and follow the [disaster recovery guide](./DATABASE_GUIDE.md#disaster-recovery)
