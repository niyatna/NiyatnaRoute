---
title: "NiyatnaRoute Fly.io Deployment Guide"
version: 3.8.40
lastUpdated: 2026-06-28
---

# NiyatnaRoute Fly.io Deployment Guide

This document describes the actual deployment process for NiyatnaRoute on Fly.io, covering two scenarios:
...
This guide is based on a verified working configuration for the current project. The application name is `niyatnaroute`.
...
- Access URL: `https://niyatnaroute.fly.dev/`
...
- `app = 'niyatnaroute'` determines which Fly application the deployment targets
...
```toml
app = 'niyatnaroute-yourname'
```

Note:

- Make sure the application you see in the console matches the `app` value in `fly.toml`
- If you previously used a different name, such as `oroute`, do not confuse it with `niyatnaroute`
...
```powershell
flyctl apps create niyatnaroute
```

If you changed the application name, replace `niyatnaroute` with your chosen name.
...
These parameters have been used in actual deployments on the current `niyatnaroute` application:
...
- `NIYATNAROUTE_WS_BRIDGE_SECRET` (required in production — used for WebSocket bridge authentication)
...
| `NIYATNAROUTE_WS_BRIDGE_SECRET`  | Required in production | WebSocket bridge authentication secret                |
...
| ---------------------- | --------------------------- |
| `DATA_DIR`             | `/data`                     |
| `NEXT_PUBLIC_BASE_URL` | `https://niyatnaroute.fly.dev` |
...
1. **Set `NEXT_PUBLIC_BASE_URL` to your public HTTPS domain**
   ...
   ```powershell
   flyctl secrets set NEXT_PUBLIC_BASE_URL=https://niyatnaroute.fly.dev -a niyatnaroute
   ```

   If you are using a custom domain, replace it with the corresponding domain (e.g. `https://niyatnaroute.yourdomain.com`).
...
2. **Configure the callback URL on the provider console**
   ...
   - `https://niyatnaroute.fly.dev/callback`
...
- Intended for the current project `niyatnaroute`
...
flyctl secrets set `
  ...
  NIYATNAROUTE_WS_BRIDGE_SECRET=$wsBridgeSecret `
  DATA_DIR=/data `
  NEXT_PUBLIC_BASE_URL=https://niyatnaroute.fly.dev `
  -a niyatnaroute
```
...
```bash
flyctl secrets set NIYATNAROUTE_WS_BRIDGE_SECRET=$(openssl rand -hex 32) -a niyatnaroute
```

Notes:

- `NIYATNAROUTE_WS_BRIDGE_SECRET` is required in production; missing it will break the WebSocket bridge handshake

If you also want to set an initial password:

```powershell
flyctl secrets set INITIAL_PASSWORD=your-strong-password -a niyatnaroute
```
...
```powershell
flyctl secrets list -a niyatnaroute
```

If the `Secrets` page in the console does not show the expected variables, check:

- That you are viewing the `niyatnaroute` application
...
```powershell
flyctl secrets set KEY=value -a niyatnaroute
```
...
If the current repository is a fork and you want to sync updates from the upstream `https://github.com/niyatnaroute/NiyatnaRoute`, follow the workflow below.
...
```powershell
git remote add upstream https://github.com/niyatnaroute/NiyatnaRoute.git
```
...
5. `flyctl deploy`
6. `flyctl status -a niyatnaroute`
7. `flyctl logs --no-tail -a niyatnaroute`
...
```powershell
flyctl status -a niyatnaroute
```

### 10.2 View Startup Logs

```powershell
flyctl logs --no-tail -a niyatnaroute
```
...
```powershell
try {
  (Invoke-WebRequest -Uri "https://niyatnaroute.fly.dev" -MaximumRedirection 5 -UseBasicParsing).StatusCode
} catch {
...
- You are viewing a different application (e.g. `oroute` instead of `niyatnaroute`)
...
1. `flyctl auth login`
2. `flyctl apps create niyatnaroute`
3. `flyctl secrets set ... -a niyatnaroute`
4. `flyctl deploy`
5. `flyctl logs --no-tail -a niyatnaroute`
