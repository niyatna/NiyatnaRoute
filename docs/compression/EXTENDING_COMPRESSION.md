...
> **TL;DR**: NiyatnaRoute's compression engine is **pluggable** — you can register custom engines, ship language packs for new languages, and compose stacked pipelines. This guide shows how.
...
````ts
import type { CompressionEngine } from "niyatnaroute/compression/engines/types";
import { registerCompressionEngine } from "niyatnaroute/compression/engines/registry";
...
```
~/.niyatnaroute/compression/engines/my-engine.ts    # User-level
...
} from "@niyatnaroute/open-sse/services/compression/engines/registry";
...
Caveman-style compression uses **language-specific rule packs** to handle fillers, hedging, and verbose patterns in each natural language. NiyatnaRoute ships with **6 language packs**: `en`, `es`, `fr`, `de`, `ja`, `pt-BR`.
...
```ts
import { loadRulePack } from "niyatnaroute/compression/ruleLoader";
...
```
~/.niyatnaroute/compression/rules/hi/filler.json  # User-level
...
NiyatnaRoute selects **ONE mode per request** based on configuration, auto-trigger thresholds, and combo overrides.
...
`@niyatnaroute/open-sse/services/compression/strategySelector`):
...
NiyatnaRoute's compression engines credit several upstream projects in the README
...
pack, how does that reach NiyatnaRoute?** This section is the authoritative answer.
...
rule/filter in NiyatnaRoute's own format** (see
...
NiyatnaRoute's own `smartcrusher` / `toon` / `tabular` layers. There is no live
...
1. **Don't vendor** — re-express the upstream rule/filter in NiyatnaRoute's format.
