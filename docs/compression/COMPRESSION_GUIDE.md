---
title: "🗜️ Prompt Compression Guide — NiyatnaRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🗜️ Prompt Compression Guide — NiyatnaRoute
...
NiyatnaRoute implements a modular prompt compression pipeline that runs **proactively** before requests hit upstream providers. This means your token savings happen transparently — no changes needed to your workflow.
...
NiyatnaRoute documents compression savings from two sources: upstream project benchmarks and
NiyatnaRoute's own engine composition.
...
For overlapping tool/context payloads, the default NiyatnaRoute combo stacks the engines:
...
Send the `x-niyatnaroute-compression` request header to override the compression plan for a single
...
The applied plan is echoed back in the `X-NiyatnaRoute-Compression: <mode>; source=<source>` response
...
Beyond the 7 standard modes, NiyatnaRoute includes several advanced compression
...
} from "@niyatnaroute/open-sse/services/compression/cachingAware";
...
```ts
import { applyAging } from "@niyatnaroute/open-sse/services/compression/progressiveAging";
