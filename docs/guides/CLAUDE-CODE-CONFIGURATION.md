---
title: "Claude Code CLI — Configuration with NiyatnaRoute"
version: 3.8.40
lastUpdated: 2026-07-24
---

# Claude Code CLI — Configuration with NiyatnaRoute

Point the **Claude Code** CLI (`claude`) at NiyatnaRoute — local or a remote VPS —
...
```bash
# Launch Claude Code against a local NiyatnaRoute (auto-detects the active context)
niyatnaroute launch

# Against a remote NiyatnaRoute (after `niyatnaroute connect <host>`, this is automatic)
niyatnaroute launch --remote http://192.168.0.15:20128 --api-key oma_live_xxx

# Generate per-model profiles, then launch one
niyatnaroute setup-claude            # writes ~/.claude/profiles/<name>/settings.json
niyatnaroute launch --profile glm52  # Claude Code using glm/glm-5.2 via NiyatnaRoute
```
...
| `ANTHROPIC_AUTH_TOKEN`                       | Sent as `Authorization: Bearer …` — use your NiyatnaRoute access token / API key          |
...
`niyatnaroute launch` sets all of these for you: it resolves the base URL + token
from the active context (so `niyatnaroute connect <vps>` then `niyatnaroute launch`
...
`niyatnaroute setup-claude` fetches the live `/v1/models` catalog and writes one
...
> `niyatnaroute launch --profile <name>` (it injects `ANTHROPIC_AUTH_TOKEN` from the
> active context), or export `ANTHROPIC_AUTH_TOKEN` yourself and run
> `CLAUDE_CONFIG_DIR=~/.claude/profiles/<name> claude`.

**Auto-sync after model discovery (opt-in).** NiyatnaRoute can regenerate these same
...
auto-sync" → Claude Code), or set `NIYATNAROUTE_AUTO_SYNC_CLAUDE_PROFILES=true` (it also honors
...
niyatnaroute setup-claude --remote http://192.168.0.15:20128 --api-key oma_live_xxx

# Only some providers
niyatnaroute setup-claude --only glm,kimi

# Preview without writing
niyatnaroute setup-claude --dry-run

# Launch a profile
niyatnaroute launch --profile kimi-k27
```
...
Claude Code routes to capability tiers. Map each to an NiyatnaRoute model via env /
...
Once you've run `niyatnaroute connect <host>` (see
[Remote Mode](./REMOTE-MODE.md)), `niyatnaroute launch` and `niyatnaroute setup-claude`
...
`/v1`** and restart `claude` (env is read once at startup). `niyatnaroute launch`
...
NiyatnaRoute refuses to guess. Fix it either way: pin a prefixed id with
...
`NIYATNAROUTE_PREFER_CLAUDE_CODE_FOR_UNPREFIXED_CLAUDE_MODELS=true` (default off;
...
**Auth errors** — the profile holds no token. Use `niyatnaroute launch --profile`
