...
Complete guide for configuring providers, creating combos, integrating CLI tools, and deploying NiyatnaRoute.
...
**Pro Tip:** Use Opus for complex tasks, Sonnet for speed. NiyatnaRoute tracks quota per model!

Claude and Claude Code-compatible routes preserve `max` thinking effort for Opus and Sonnet
models. Haiku models do not accept the `max` effort tier, so NiyatnaRoute downgrades that
...
1. Subscribe: [Moonshot AI](https://platform.kimi.ai?aff=niyatnaroute)
2. Get API key → Dashboard → Add API Key
...
Settings → Models → Advanced:
  ...
  OpenAI API Key: [from niyatnaroute dashboard]
...
```json
{
  "env": {
    ...
    "ANTHROPIC_AUTH_TOKEN": "your-niyatnaroute-api-key"
  }
}
```
...
export OPENAI_API_KEY="your-niyatnaroute-api-key"
codex "your prompt"
```
...
```json
{
  "agents": {
    "defaults": {
      "model": { "primary": "niyatnaroute/if/kimi-k2.7-code" }
    }
  },
  "models": {
    "providers": {
      "niyatnaroute": {
        ...
        "apiKey": "your-niyatnaroute-api-key",
...
mkdir -p ~/.niyatnaroute

# Create .env file (see .env.example)
cp .env.example ~/.niyatnaroute/.env

# Start server
niyatnaroute
# Or with custom port:
niyatnaroute --port 3000
```

The CLI automatically loads `.env` from `~/.niyatnaroute/.env` or `./.env`.

### Uninstalling

When you no longer need NiyatnaRoute, we provide two quick scripts for a clean removal:
...
> Note: To run these commands, navigate to the NiyatnaRoute project folder (if you cloned it) and run them. Alternatively, if globally installed, you can simply run `npm uninstall -g niyatnaroute`.

### VPS Deployment

```bash
git clone https://github.com/niyatnaroute/NiyatnaRoute.git
cd NiyatnaRoute && npm install && npm run build

export JWT_SECRET="your-secure-secret-change-this"
export INITIAL_PASSWORD="your-password"
export DATA_DIR="/var/lib/niyatnaroute"
export PORT="20128"
...
npm run start
# Or: pm2 start npm --name niyatnaroute -- start
```
...
pm2 start npm --name niyatnaroute -- start

# Or with custom memory limit
NIYATNAROUTE_MEMORY_MB=512 pm2 start npm --name niyatnaroute -- start
...
docker build -t niyatnaroute:cli .

# Portable mode (recommended)
docker run -d --name niyatnaroute -p 20128:20128 --env-file ./.env -v niyatnaroute-data:/app/data niyatnaroute:cli
```
...
Void Linux users can package and install NiyatnaRoute natively using the `xbps-src` cross-compilation framework. This automates the Node.js standalone build along with the required `better-sqlite3` native bindings.
...
| --------------------------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| `JWT_SECRET`                            | `niyatnaroute-default-secret-change-me` | JWT signing secret (**change in production**)                                                             |
| `INITIAL_PASSWORD`                      | `CHANGEME`                           | First login password                                                                                      |
| `DATA_DIR`                              | `~/.niyatnaroute`                       | Data directory (db, usage, logs)                                                                          |
...
| `NEXT_PUBLIC_CLOUD_URL`                 | `https://niyatnaroute.dev`              | Cloud sync endpoint base URL (replaces legacy `CLOUD_URL`)                                                |
...
| `NIYATNAROUTE_MEMORY_MB`                   | `512`                                | Node.js heap limit in MB                                                                                  |
...
### Chaining NiyatnaRoute Peers

Another NiyatnaRoute gateway can be added as a **Custom OpenAI-compatible** provider. Use the
...
```bash
# gateway-a
NIYATNAROUTE_INSTANCE_ID=gateway-a
NIYATNAROUTE_PEER_URLS=http://gateway-b:20128/v1
NIYATNAROUTE_PEER_MAX_HOPS=4
```

```bash
# gateway-b
NIYATNAROUTE_INSTANCE_ID=gateway-b
NIYATNAROUTE_PEER_URLS=http://gateway-a:20128/v1
NIYATNAROUTE_PEER_MAX_HOPS=4
```

Only requests sent to an explicitly allowlisted peer URL receive the
`X-NiyatnaRoute-Peer-Trace` header. A gateway rejects a repeated instance ID or exhausted hop
...
into multiple running NiyatnaRoute instances.
...
- Quick Tunnels are not auto-restored after an NiyatnaRoute or container restart; re-enable them from the dashboard when needed
...
- **Semantic Cache** — Auto-caches non-streaming, temperature=0 responses (bypass with `X-NiyatnaRoute-No-Cache: true`)
- **Request Idempotency** — Deduplicates requests within 5s via `Idempotency-Key` or `X-Request-Id` header
- **Progress Tracking** — Opt-in SSE `event: progress` events via `X-NiyatnaRoute-Progress: true` header

---

### Translator Playground

Access via **Dashboard → Translator**. Debug and visualize how NiyatnaRoute translates API requests between providers.
...
NiyatnaRoute also accepts `x_session_id` and returns the effective session key in `X-NiyatnaRoute-Session-Id`.
...
NiyatnaRoute implements provider-level resilience with five components:
...
4. **Wait For Cooldown** — If every candidate connection is already cooling down, NiyatnaRoute can wait for the earliest cooldown and retry the same client request automatically.
...
- Migrate NiyatnaRoute between machines
...
NiyatnaRoute supports audio transcription via the OpenAI-compatible endpoint:
...
NiyatnaRoute clamps it down to that cap before sending the upstream request.
...
NiyatnaRoute is an **MCP server** (Model Context Protocol). Any MCP-compatible IDE or agent host can call NiyatnaRoute tools directly — no extra wrapper required.

### MCP transports

- **SSE**: `http://localhost:9999/api/mcp/sse`
- **Streamable HTTP**: `http://localhost:9999/api/mcp/stream`
- **stdio**: `niyatnaroute --mcp` (for IDE plugins that prefer stdio)
...
Use the SSE URL `http://localhost:9999/api/mcp/sse` and a Bearer API key generated in **Dashboard → API Keys**.

### Scopes

MCP tools are grouped into 10 scopes: `analytics`, `auth`, `billing`, `combos`, `health`, `keys`, `models`, `providers`, `system`. Each Bearer key can be limited to specific scopes — see [MCP-SERVER.md](../frameworks/MCP-SERVER.md) for the full tool catalog.

---

## 🧠 Skills System

NiyatnaRoute exposes an extensible **skill framework** (`src/lib/skills/`) so agents can run domain-specific routines (e.g. `code-review`, `summarize`, `extract-facts`, `web-research`).
...
NiyatnaRoute persists **long-term conversational memory** with hybrid retrieval:
...
Subscribe to NiyatnaRoute events for real-time monitoring and automation.
...
- Every payload includes `X-NiyatnaRoute-Signature` (HMAC-SHA256) for verification
...
NiyatnaRoute integrates with cloud coding agents (**OpenAI Codex Cloud**, **Devin**, **Jules**, **Antigravity**) so you can dispatch long-running tasks from the same dashboard that handles your local routing.

- Create tasks in **Dashboard → Cloud Agents** or via `POST /api/v1/agents/tasks`
- Track status, logs, and artifacts per task
- Bring-your-own API key per provider — credentials never leave the NiyatnaRoute instance
...
You can manage every NiyatnaRoute resource (providers, combos, keys, settings) over HTTP using a **Bearer key with the `manage` scope**.
...
curl http://localhost:20128/api/providers \
  -H "Authorization: Bearer $NIYATNAROUTE_MANAGE_KEY"
...
curl http://localhost:20128/api/keys -H "Authorization: Bearer $NIYATNAROUTE_MANAGE_KEY"
curl -X POST http://localhost:20128/api/keys -H "Authorization: Bearer $NIYATNAROUTE_MANAGE_KEY" \
...
NiyatnaRoute ships an internal CLI (`niyatnaroute …`) for setup, diagnostics, and runtime control. This is **separate from the "CLI Tools" page in the dashboard**, which configures third-party CLIs (Claude Code, Cursor, Codex, Cline, …) so they can talk to NiyatnaRoute.

```bash
niyatnaroute setup                    # Interactive wizard (password, providers, combos)
niyatnaroute setup --non-interactive  # CI-friendly
niyatnaroute doctor                   # Health diagnostics (data dir, DB, providers, ports)
niyatnaroute providers available      # List supported providers
niyatnaroute providers list           # List configured connections
niyatnaroute providers test <id>      # Live test a provider connection
niyatnaroute combos list              # List combos
niyatnaroute combos switch <name>     # Set default combo
niyatnaroute models                   # List available models (--json, --search)
niyatnaroute keys add | list | remove # Manage API keys from the terminal
niyatnaroute backup                   # Snapshot config + DB
niyatnaroute restore [<timestamp>]    # Restore from a snapshot
niyatnaroute health                   # Detailed health (breakers, cache, memory)
niyatnaroute quota                    # Provider quota usage
niyatnaroute mcp status               # MCP server status
niyatnaroute tunnel list|create|stop  # Cloudflare/Tailscale/ngrok tunnels
niyatnaroute reset-password           # Reset the admin password
niyatnaroute --mcp                    # Start MCP server over stdio
niyatnaroute --port 9999              # Start the server on a custom port
```

Tip: pair `niyatnaroute doctor --json` with your monitoring tool to alert on unhealthy provider connections.
