---
title: "Codex CLI — Configuration with NiyatnaRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Codex CLI — Configuration with NiyatnaRoute

Complete guide for using the Codex CLI pointed at NiyatnaRoute as an OpenAI-compatible backend.
...
**NiyatnaRoute solves this transparently:**

```
Codex CLI
  → wire_api = "responses"
  → POST /v1/responses (NiyatnaRoute)
    → NiyatnaRoute Responses ↔ Chat Completions transformer
    → POST /chat/completions (DeepSeek / Mistral / GLM / Kimi / any provider)
```

You never need a separate translation proxy when using NiyatnaRoute. **All models use `wire_api = "responses"`** — NiyatnaRoute handles the rest.
...
| Model                                | NiyatnaRoute ID                         | Context window         | `auto_compact` | `tool_output_limit` |
| ------------------------------------ | ------------------------------------ | ---------------------- | -------------- | ------------------- |
...
All Codex models in NiyatnaRoute use the `cx/` prefix:

| Codex CLI name          | NiyatnaRoute model    |
| ----------------------- | ------------------ |
...
Other providers use their own prefix (`kmc/`, `glm/`, `ds/`, `ollamacloud/`, `opencode-go/`, `mistral/`) — the prefix matches the NiyatnaRoute provider alias.
...
## Generating profiles automatically with `niyatnaroute setup-codex`

If you run NiyatnaRoute on a VPS, you can auto-generate profile files from the live model catalog:

```bash
# From a VPS (uses local NiyatnaRoute on port 20128)
niyatnaroute setup-codex

# From any machine — point at your VPS
niyatnaroute setup-codex --remote http://100.x.x.x:20128 --api-key sk-xxx

# Preview without writing files
niyatnaroute setup-codex --remote http://100.x.x.x:20128 --dry-run

# Only generate GLM and Kimi profiles
niyatnaroute setup-codex --only glm,kimi

# Write to a custom directory
niyatnaroute setup-codex --codex-home /path/to/.codex
```
...
NiyatnaRoute can also **auto-sync** these same profile files after a successful provider model discovery/import changes the live catalog. This is **opt-in and off by default**: toggle it from the **CLI Code dashboard** ("CLI profile auto-sync" → Codex), or set `NIYATNAROUTE_AUTO_SYNC_CODEX_PROFILES=true` (it also honors `CLI_ALLOW_CONFIG_WRITES`, on by default). When enabled it only writes separate `~/.codex/*.config.toml` profile files; it never changes the active/default `~/.codex/config.toml`, Codex-lb settings, auth, or provider selection.

---

## Launching Codex with `niyatnaroute launch-codex`

Health-checks your NiyatnaRoute instance before launching Codex:

```bash
# Launch against local NiyatnaRoute (default port 20128)
niyatnaroute launch-codex

# Launch with a specific profile
niyatnaroute launch-codex --profile kimi-k27

# Launch against a remote VPS
niyatnaroute launch-codex --remote http://100.x.x.x:20128/v1 --api-key sk-xxx

# Pass extra args to codex
niyatnaroute launch-codex --profile glm52 -- --yolo "fix this bug"
```
...
[model_providers.niyatnaroute.http_headers]
"X-Custom-Header" = "value"

# Headers read from env vars
[model_providers.niyatnaroute.env_http_headers]
"X-Trace-Id" = "TRACE_ID"

# Extra URL query params (useful for Azure api-version)
[model_providers.niyatnaroute.query_params]
...
```toml
[model_providers.niyatnaroute-main]
base_url = "http://192.168.0.1:20128/v1"
env_key  = "NIYATNAROUTE_API_KEY"

[model_providers.niyatnaroute-tailscale]
base_url = "http://100.x.x.x:20128/v1"
env_key  = "NIYATNAROUTE_API_KEY"
```
...
Verify the model exists in NiyatnaRoute with the correct prefix. Use `niyatnaroute models list` or open `/dashboard/providers/<provider>`.

**`Authentication error`**
Confirm `NIYATNAROUTE_API_KEY` is exported: `echo $NIYATNAROUTE_API_KEY`.

**`Connection refused`**
Verify NiyatnaRoute is running and the `base_url` host/port is correct for your network (local vs Tailscale vs VPS).
