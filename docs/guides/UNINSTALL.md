---
title: "NiyatnaRoute — Uninstall Guide"
version: 3.8.40
lastUpdated: 2026-06-28
---

# NiyatnaRoute — Uninstall Guide
...
This guide covers how to cleanly remove NiyatnaRoute from your system.

---

## Quick Uninstall (v3.6.2+)

NiyatnaRoute provides two built-in scripts for clean removal:
...
This removes the NiyatnaRoute application but **preserves** your database, configurations, API keys, and provider settings in `~/.niyatnaroute/`. Use this if you plan to reinstall later and want to keep your setup.
...
- Backup files
- Log files
- All files in the `~/.niyatnaroute/` directory
...
docker rm niyatnaroute

# Remove the volume (deletes all data)
docker volume rm niyatnaroute-data

# (Optional) Remove the image
docker rmi niyatnaroute/niyatnaroute:latest
```
...
NiyatnaRoute stores data in the following locations by default:

| Platform      | Default Path                  | Override                  |
| ------------- | ----------------------------- | ------------------------- |
| Linux         | `~/.niyatnaroute/`               | `DATA_DIR` env var        |
| macOS         | `~/.niyatnaroute/`               | `DATA_DIR` env var        |
| Windows       | `%APPDATA%/niyatnaroute/`        | `DATA_DIR` env var        |
| Docker        | `/app/data/` (mounted volume) | `DATA_DIR` env var        |
| XDG-compliant | `$XDG_CONFIG_HOME/niyatnaroute/` | `XDG_CONFIG_HOME` env var |
...
npm list -g niyatnaroute 2>/dev/null

# Check for data directory
ls -la ~/.niyatnaroute/ 2>/dev/null

# Check for running processes
pgrep -f niyatnaroute
```
