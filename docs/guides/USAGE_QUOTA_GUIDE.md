...
> **TL;DR**: NiyatnaRoute tracks every request's token usage, computes cost, enforces per-API-key quota, and surfaces analytics in the dashboard. This guide explains how it all works.
...
Every request that flows through NiyatnaRoute generates a **usage record** that captures:
...
For providers that don't return usage (some web-cookie providers), NiyatnaRoute **estimates** tokens using a `~4 chars per token` heuristic (see `open-sse/services/autoCombo/pipelineRouter.ts`).

### Cached Tokens

NiyatnaRoute tracks `cached_tokens` separately from `prompt_tokens` because:
...
For models with no pricing data, NiyatnaRoute falls back to **estimating cost** using internal average rates (sourced from LiteLLM's pricing data).
...
| Tool                    | Description                                        |
| ----------------------- | -------------------------------------------------- |
| `niyatnaroute_cost_report` | Generates a per-key cost report for a given period |
| `niyatnaroute_check_quota` | Returns current quota status for an API key        |

Example agent invocation:

```json
{
  "tool": "niyatnaroute_cost_report",
  "args": { "period": "week" }
}
```
