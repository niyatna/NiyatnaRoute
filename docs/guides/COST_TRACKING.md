...
How NiyatnaRoute estimates, records, and reports the cost of every request — and why the
...
NiyatnaRoute attributes a per-request USD cost to every completion by multiplying token
counts by a model's pricing rates. These numbers power the **Costs** dashboard, the
`niyatnaroute cost` / `niyatnaroute usage` CLI, CSV/JSON exports, and per-API-key budgets.

> **The dashboard "cost" is a savings tracker, not a bill.** NiyatnaRoute never charges you
...
- It depends on the pricing table NiyatnaRoute has for each model. A model with no pricing
...
3. **Hardcoded defaults** — shipped with NiyatnaRoute.
...
- API-key spend is buffered and flushed in batches by the
  ...
  | Env var                             | Default | Purpose                            |
  | ----------------------------------- | ------- | ---------------------------------- |
  | `NIYATNAROUTE_SPEND_FLUSH_INTERVAL_MS` | `60000` | Flush interval in milliseconds.    |
  | `NIYATNAROUTE_SPEND_MAX_BUFFER_SIZE`   | `1000`  | Max buffered entries before flush. |
...
NiyatnaRoute's CLI exposes cost, usage, and pricing commands (registered in
...
```bash
niyatnaroute cost                          # last 30d, grouped by provider
niyatnaroute cost --period 7d              # last 7 days
niyatnaroute cost --group-by model         # group by provider | model | combo | api-key | day
niyatnaroute cost --since 2026-06-01 --until 2026-06-13
niyatnaroute cost --api-key <key> --limit 50
```
...
```bash
niyatnaroute usage analytics --period 30d [--provider <id>]   # per-provider cost summary
niyatnaroute usage logs [--limit 100] [--follow] [--api-key <k>] [--search <q>]
niyatnaroute usage quota [--provider <id>] [--check]
niyatnaroute usage utilization [--api-key <k>]
niyatnaroute usage history [--limit 100]
niyatnaroute usage proxy-logs [--limit 100]

# Budgets
niyatnaroute usage budget list
niyatnaroute usage budget get [scope]
niyatnaroute usage budget set <amount> [--scope global] [--period monthly]
niyatnaroute usage budget reset [scope]
```

### `niyatnaroute pricing`

```bash
niyatnaroute pricing list [--provider <p>] [--model <m>] [--limit 200]
niyatnaroute pricing get <model>
niyatnaroute pricing sync [--provider <p>] [--force]   # POST /api/pricing/sync
niyatnaroute pricing diff [--model <m>]
niyatnaroute pricing defaults show
niyatnaroute pricing defaults set [--input <p>] [--output <p>] [--cache-read <p>] [--cache-write <p>]
```
...
- **All costs show $0 / "Legacy / Free".** The models in use have no pricing entry.
  Enable external sync (`PRICING_SYNC_ENABLED=true`) and run `niyatnaroute pricing sync`, or
...
- **Spend lags behind real time.** Per-key spend is batched; lower
  `NIYATNAROUTE_SPEND_FLUSH_INTERVAL_MS` if you need fresher numbers.
