---
title: "🐳 Docker Guide — NiyatnaRoute"
version: 3.8.40
lastUpdated: 2026-06-28
---

# 🐳 Docker Guide — NiyatnaRoute
...
docker run -d \
  ...
  --stop-timeout 40 \
  -p 20128:20128 \
  -v niyatnaroute-data:/app/data \
  niyatnaroute/niyatnaroute:latest
```
...
NiyatnaRoute ships four Compose profiles. Pick the one that matches your environment.

| Profile          | Service          | When to use                                                                                                                       | Command                                      |
| ---------------- | ---------------- | --------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| `base` (default) | `niyatnaroute-base` | Headless server / minimal runtime, no provider CLIs bundled                                                                       | `docker compose --profile base up -d`        |
| `cli`            | `niyatnaroute-cli`  | Agentic workflows that call `niyatnaroute providers/setup/doctor` and bundled CLIs (Codex, Claude Code, Droid, OpenClaw)             | `docker compose --profile cli up -d`         |
| `host`           | `niyatnaroute-host` | Linux hosts that want `network_mode`-like access to host CLIs by mounting `~/.local/bin`, `~/.codex`, `~/.claude`, etc. read-only | `docker compose --profile host up -d`        |
...
NiyatnaRoute relies on Redis to back the distributed rate limiter and shared cache. The `redis` service is **always defined** in `docker-compose.yml` (it has no profile gate) and starts alongside any other profile.

| Detail               | Value                             |
| -------------------- | --------------------------------- |
| Image                | `redis:7-alpine`                  |
| Container name       | `niyatnaroute-redis`                 |
| Internal port        | `6379`                            |
| Host port (override) | `REDIS_PORT` (defaults to `6379`) |
| Volume               | `niyatnaroute-redis-data` → `/data`  |
...
| Image                  | `niyatnaroute:prod` (built from `runner-cli` target)                                  |
| Redis container        | `niyatnaroute-redis-prod` (`redis:8.6.2`, dedicated `redis-prod-data` volume)         |
| Data volume            | `niyatnaroute-prod-data` (named, persisted across rebuilds)                           |
...
```bash
docker build --target runner-base -t niyatnaroute:base .
docker build --target runner-cli  -t niyatnaroute:cli  .
```

Defaults exported by `runner-base`: `PORT=20128`, `HOSTNAME=0.0.0.0`, `NODE_OPTIONS=--max-old-space-size=512`, `DATA_DIR=/app/data`, `NIYATNAROUTE_MIGRATIONS_DIR=/app/migrations`.

Memory behavior in Docker:

- `NODE_OPTIONS=--max-old-space-size=512` is baked into the image as a fallback.
- The actual server process is started by the standalone launcher, which reads `NIYATNAROUTE_MEMORY_MB` and appends `--max-old-space-size=<NIYATNAROUTE_MEMORY_MB>`.
- Node uses the last repeated `--max-old-space-size` value, so setting `NIYATNAROUTE_MEMORY_MB` controls the effective Docker heap limit.
- If `NIYATNAROUTE_MEMORY_MB` is unset, the launcher uses `512`.
...
| ----------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------ |
| `NIYATNAROUTE_WS_BRIDGE_SECRET`  | Shared secret for the WebSocket bridge. **Required in production** — set to a strong random string. | unset (must be provided) |
...
| `AUTO_UPDATE_HOST_REPO_DIR`   | Host path mounted into `cli` profile at `/workspace/niyatnaroute` for self-update workflows            | `.` (current directory)  |
| `NIYATNAROUTE_MEMORY_MB`         | Runtime Node heap ceiling for the Docker standalone server; overrides the image fallback above      | `512`                    |
...
NiyatnaRoute can be securely exposed using Caddy's automatic SSL provisioning. Ensure your domain's DNS A record points to your server's IP.

```yaml
services:
  niyatnaroute:
    image: niyatnaroute/niyatnaroute:latest
    container_name: niyatnaroute
    restart: unless-stopped
    volumes:
      - niyatnaroute-data:/app/data
    environment:
      - PORT=20128
      ...
      - BASE_URL=http://niyatnaroute:20128
  ...
  caddy:
    ...
    ports:
      - "80:80"
      - "443:443"
    command: caddy reverse-proxy --from https://your-domain.com --to http://niyatnaroute:20128

volumes:
  niyatnaroute-data:
```

Caddy sets the standard forwarding headers for the upstream container. NiyatnaRoute uses
...
protection. Only enable `NIYATNAROUTE_TRUST_PROXY` for advanced deployments where you intentionally
want NiyatnaRoute to derive the public origin from trusted forwarded headers instead of explicit
configuration.
...
- Set `CLOUDFLARED_BIN=/absolute/path/to/cloudflared` if you want NiyatnaRoute to use an existing binary instead of downloading one.

## Image Tags

| Image                    | Tag      | Size   | Description           |
| ------------------------ | -------- | ------ | --------------------- |
| `niyatnaroute/niyatnaroute` | `latest` | ~250MB | Latest stable release |
| `niyatnaroute/niyatnaroute` | `3.8.0`  | ~250MB | Current version       |
...
- **SQLite WAL Mode:** `docker stop` should be allowed to finish so NiyatnaRoute can checkpoint the latest changes back into `storage.sqlite`. The bundled Compose files already set a 40s stop grace period. If you run the image directly, keep `--stop-timeout 40`.
