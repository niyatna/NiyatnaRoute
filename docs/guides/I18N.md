...
NiyatnaRoute supports **43 languages** with full dashboard UI translation, translated documentation, and RTL support for Arabic and Hebrew.
...
NiyatnaRoute uses a hash-based incremental translator for docs, backed by an
OpenAI-compatible LLM endpoint (typically `cx/gpt-5.4-mini` through NiyatnaRoute
Cloud):
...
| Variable                            | Purpose                                 |
| ----------------------------------- | --------------------------------------- |
| `NIYATNAROUTE_TRANSLATION_API_URL`     | OpenAI-compatible base URL, e.g. `…/v1` |
| `NIYATNAROUTE_TRANSLATION_API_KEY`     | bearer token (kept out of logs)         |
| `NIYATNAROUTE_TRANSLATION_MODEL`       | model id, e.g. `cx/gpt-5.4-mini`        |
...
- **Chunked batching**: Joins multiple strings with `__NIYATNAROUTE_I18N_SEPARATOR__` delimiters to minimize API calls (max 1800 chars per request)
...
**Secondary translator** — uses any OpenAI-compatible LLM API (including NiyatnaRoute itself) to translate existing `docs/i18n/` markdown files. Best for polishing or re-translating docs with better quality than Google Translate.
...
The `niyatnaroute` CLI has its own i18n layer separate from the Next.js dashboard.
...
| -------- | ------------------------ | --------------------------------------- |
| 1        | `--lang` flag            | `niyatnaroute --lang de status`            |
| 2        | `NIYATNAROUTE_LANG` env var | `NIYATNAROUTE_LANG=ja niyatnaroute providers` |
...
```bash
# Set language and save to ~/.niyatnaroute/.env (persists across sessions)
niyatnaroute config lang set pt-BR

# View current language
niyatnaroute config lang get

# List all 42 available languages
niyatnaroute config lang list

# JSON output
niyatnaroute config lang list --output json
```

The saved preference is written atomically to `~/.niyatnaroute/.env` and is loaded by the
...
niyatnaroute --lang de providers list
```
...
(`NiyatnaRoute`, `OAuth`, `MCP`, `A2A`, `DATA_DIR`, …) that must appear verbatim
...
The generator originally used `code: "in"` (deprecated Google Translate code) for Hindi instead of the correct ISO 639-1 `hi`. This was introduced in upstream commit `952b0b22c` by `niyatnaroute`. Fixed by changing `code: "in"` to `code: "hi"` in the `LOCALE_SPECS` array and removing the orphaned `in.json` file.
