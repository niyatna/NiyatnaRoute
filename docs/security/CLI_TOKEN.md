...
NiyatnaRoute CLI commands authenticate against the local management API using a
`HMAC-SHA256(machine-id, salt)` token sent via the `x-niyatnaroute-cli-token`
request header.

This allows CLI subcommands (`niyatnaroute status`, `niyatnaroute providers`, etc.)
...
3. The CLI sends the token as `x-niyatnaroute-cli-token` on every request to
...
Set `NIYATNAROUTE_CLI_SALT` to rotate the derived token without code changes.
...
export NIYATNAROUTE_CLI_SALT="my-secret-salt-2026"

# Verify new token is in use
niyatnaroute status
```

Default salt: `niyatnaroute-cli-auth-v1`
...
**Opt-out:** set `NIYATNAROUTE_DISABLE_CLI_TOKEN=true` (env or `.env`) to disable the CLI
