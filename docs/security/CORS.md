...
NiyatnaRoute controls which **browser origins** may read cross-origin responses
...
taking seriously, but NiyatnaRoute's exposure is **narrower than the generic case**,
...
> `Access-Control-Allow-Credentials: true`. NiyatnaRoute's shared CORS path never
> does.
...
- If NiyatnaRoute runs behind a reverse proxy / tunnel (nginx, Caddy, Cloudflare
...
```bash
# Vite (5173) + Next.js (3000) dev servers calling a local NiyatnaRoute
...
## Example: reverse proxy in front of NiyatnaRoute

CORS is enforced by NiyatnaRoute itself, so the proxy generally should **not** add or
rewrite `Access-Control-*` headers (double headers break browsers). Terminate TLS
and forward — let NiyatnaRoute answer preflight:

```nginx
# nginx — forward to NiyatnaRoute; do NOT inject Access-Control-* here
location / {
...
Set the allowed browser origins in NiyatnaRoute (`CORS_ALLOWED_ORIGINS` or the
