...
Complete reference for all NiyatnaRoute API endpoints.
...
| ------------------------ | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `X-NiyatnaRoute-No-Cache`   | Request   | Set to `true` to bypass cache                                                                                                                                                     |
| `x-niyatnaroute-no-memory`  | Request   | Set to `true` to skip memory + skills injection for this request (mirrors no-cache; avoids the per-call token/cost overhead)                                                      |
| `X-NiyatnaRoute-Progress`   | Request   | Set to `true` for progress events                                                                                                                                                 |
| `X-Session-Id`           | Request   | Sticky session key for external session affinity                                                                                                                                  |
| `x_session_id`           | Request   | Underscore variant also accepted (direct HTTP)                                                                                                                                    |
| `X-NiyatnaRoute-Session-Id` | Request   | Caller-supplied session/conversation tag (also feeds memory). When present, persisted verbatim to `call_logs.session_tag` for per-session cost attribution (#8249) — never synthesized when absent |
| `Idempotency-Key`        | Request   | Dedup key (5s window)                                                                                                                                                             |
| `X-Request-Id`           | Request   | Alternative dedup key                                                                                                                                                             |
| `X-NiyatnaRoute-Cache`      | Response  | `HIT` or `MISS` (non-streaming)                                                                                                                                                   |
| `X-NiyatnaRoute-Idempotent` | Response  | `true` if deduplicated                                                                                                                                                            |
| `X-NiyatnaRoute-Progress`   | Response  | `enabled` if progress tracking on                                                                                                                                                 |
| `X-NiyatnaRoute-Session-Id` | Response  | Effective session ID used by NiyatnaRoute                                                                                                                                            |
| `X-NiyatnaRoute-Request-Id` | Response  | Request correlation id (when known)                                                                                                                                               |
| `X-NiyatnaRoute-Version`    | Response  | NiyatnaRoute build version (always present)                                                                                                                                          |
| `X-NiyatnaRoute-Cost-Saved` | Response  | USD the cache avoided on a HIT (cache hits only)                                                                                                                                  |
| `X-NiyatnaRoute-Decision`   | Response  | Routing trace: `strategy=<name>; provider=<alias>; latency_ms=<n>` (`<name>` is the combo strategy, or `single` for a non-combo request) — always present on completion responses |

> Nginx note: if you rely on underscore headers (for example `x_session_id`), enable `underscores_in_headers on;`.

> **Cost telemetry headers:** non-streaming success responses also carry the `X-NiyatnaRoute-*` cost-telemetry set — `X-NiyatnaRoute-Response-Cost` (USD, fixed 10 decimals; `0.0000000000` for free/unpriced), `X-NiyatnaRoute-Tokens-In` / `X-NiyatnaRoute-Tokens-Out`, `X-NiyatnaRoute-Model`, `X-NiyatnaRoute-Provider`, `X-NiyatnaRoute-Latency-Ms`, `X-NiyatnaRoute-Cache-Hit`, and `X-NiyatnaRoute-Fallback-Attempts` (only when > 0), plus `X-NiyatnaRoute-Request-Id` and `X-NiyatnaRoute-Version`. These are emitted by chat completions, `/v1/responses`, `/v1/messages`, **and the media endpoints** — `/v1/embeddings`, `/v1/images/generations`, `/v1/audio/speech`, `/v1/audio/transcriptions`, `/v1/rerank`, `/v1/videos/generations`, `/v1/music/generations`, and `/v1/moderations` (always cost `0`). Media cost is computed per modality (per-image, per-second, per-character, per search-unit) when pricing is available, otherwise `0` (fail-open).

> **Cache-hit cost semantics:** on a semantic-cache HIT (`X-NiyatnaRoute-Cache-Hit: true`) no upstream call is made, so `X-NiyatnaRoute-Response-Cost` is `0.0000000000` (the **incremental** cost of serving the hit). The original/would-have-been cost is reported separately in `X-NiyatnaRoute-Cost-Saved`. Billing consumers should sum `X-NiyatnaRoute-Response-Cost` (hits cost nothing); cache analytics can aggregate `X-NiyatnaRoute-Cost-Saved`.
...
```
X-NiyatnaRoute-Compression: <mode>; source=<source>
```
...
- Remote media URLs must be public HTTPS. NiyatnaRoute fetches them server-side with redirect
...
For thinking-capable Claude models, `/v1/models` also advertises a **no-thinking** variant whose id is prefixed with `claude-3-niyatnaroute-no-thinking/`:

```
claude-3-niyatnaroute-no-thinking/<provider>/<model>
```
...
For clients that cannot attach `Authorization: Bearer ...`, NiyatnaRoute also accepts API keys in the URL via either query-string compatibility (`?token=...`, `?apiKey=...`, `?api_key=...`, `?key=...`) or the dedicated `/api/v1/vscode/{token}/...` endpoints documented below.
...
wscat -c "ws://localhost:20128/v1/responses?api_key=<NIYATNAROUTE_API_KEY>"
# (or: -H "Authorization: Bearer <NIYATNAROUTE_API_KEY>")
...
a ChatGPT account`). Send the **bare** id (e.g. `gpt-5.5`). NiyatnaRoute's bridge is
...
Point the Codex CLI at NiyatnaRoute by adding a custom provider with WebSocket
...
env_key = "NIYATNAROUTE_API_KEY"             # holds the NiyatnaRoute API key (Bearer)
```

```bash
export NIYATNAROUTE_API_KEY=sk-...           # an NiyatnaRoute API key (any key if REQUIRE_API_KEY=false)
codex exec "Responda apenas: PONG"
```

The CLI upgrades `base_url + /responses` to a WebSocket and NiyatnaRoute tunnels it
...
```json
{
  ...
  "backupPath": "/home/user/.niyatnaroute/backups/env-repair-2026-04-11.bak"
}
```
...
- URL-based tokens may appear in reverse-proxy logs, browser history, and telemetry outside NiyatnaRoute. Treat them as a compatibility option, not the default authentication mode.
...
Outbound webhook subscriptions for NiyatnaRoute events (request completion, quota exhaustion, key rotation, etc.).
...
Cloud agent tasks (Claude Code, Codex Cloud, OpenHands, etc.) executed remotely on behalf of NiyatnaRoute users.
...
NiyatnaRoute exposes three independent temporary-failure mechanisms; the management endpoints below let operators read and override them:
...
Skill framework for extending NiyatnaRoute with custom executable handlers, plus marketplace integrations.
...
NiyatnaRoute ships an embedded Model Context Protocol server with 3 transports (stdio, SSE, streamable-http) and scoped tools. The dashboard endpoints below read status/audit data and proxy the HTTP transports.
...
NiyatnaRoute exposes an A2A (Agent-to-Agent) JSON-RPC 2.0 endpoint plus a REST wrapper for inspection/dashboard use.

### JSON-RPC

```bash
POST /a2a
Authorization: Bearer your-api-key   # optional unless NIYATNAROUTE_API_KEY is set
Content-Type: application/json
...
**Auth:** the REST helpers run without management auth (dashboard-readable); the JSON-RPC `/a2a` route uses Bearer `NIYATNAROUTE_API_KEY` if configured.
...
Manage CLI tools that integrate with NiyatnaRoute (antigravity, chipotle, commandCode,
...
Manage NiyatnaRoute plugins (third-party extensions).
...
Inspect the runtime guardrails (PII detection, prompt injection detection, vision bridging). Guardrails run on every request; per-call opt-out is via the `x-niyatnaroute-disabled-guardrails` request header — there is no persisted enable/disable surface.
