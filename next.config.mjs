...
const contentSecurityPolicy = [
  ...
  // NiyatnaRoute is reached from a non-loopback host. Same-origin HTTP fetches stay covered
...
// NIYATNAROUTE_BUILD_PROFILE=minimal physically removes four optional privileged
...
// The resulting artifact is intended to be published as `niyatnaroute-secure`
// for security-sensitive environments. See docs/security/SOCKET_DEV_FINDINGS.md.
const isMinimalBuild = process.env.NIYATNAROUTE_BUILD_PROFILE === "minimal";
...
const nextConfig = {
  ...
  // NiyatnaRoute under https://host/niyatnaroute/). Empty by default so root-path
  ...
  basePath: process.env.NIYATNAROUTE_BASE_PATH || "",
  distDir,
  ...
  turbopack: {
    ...
    resolveAlias: {
      ...
      // (Docker sets NIYATNAROUTE_MITM_STUB=1 — #3390 graceful degradation). The
  ...
  // NiyatnaRoute is a proxy for AI APIs — request bodies routinely include
  ...
  experimental: {
    serverActions: {
      bodySizeLimit: process.env.NIYATNAROUTE_SERVER_ACTIONS_BODY_LIMIT || "50mb",
    },
    ...
    // Next's internal router proxy defaults to 30s when this is unset. NiyatnaRoute
    ...
    // PR-2 of niyatnaroute/NiyatnaRoute#3932: tree-shake barrel re-exports so
    ...
    // the internal `@niyatnaroute/open-sse` workspace here: optimizePackageImports
  ...
  transpilePackages: ["@niyatnaroute/open-sse", "@lobehub/icons", "fumadocs-ui", "fumadocs-core"],
  allowedDevOrigins: [
    "localhost",
    "127.0.0.1",
    "192.168.100.6",
  ...
  webpack(config, { webpack }) {
    ...
    config.optimization.splitChunks = {
      ...
      cacheGroups: {
        ...
        // PR-2 of niyatnaroute/NiyatnaRoute#3932: isolate the heavy long-tail
  ...
  async headers() {
    return [
      ...
      // G-10: allow NiyatnaRoute's own dashboard to embed the 9Router UI via our reverse proxy.
  ...
  async rewrites() {
    return [
      ...
      // so they hit the /api/[...niyatnarouteApiCatchAll] route (#6424) — which
