...
// ~/.niyatnaroute/runtime by trayRuntime.ts (loadSystray). The previous inline
...
export async function initSystrayUnix(
  ...
  const items = [
    { title: `NiyatnaRoute  •  port ${port}`, tooltip: "Server running", enabled: false },
    ...
    { title: "Quit NiyatnaRoute", enabled: true },
  ];
  ...
  try {
    tray = new SysTray({
      menu: {
        ...
        title: "",
        tooltip: `NiyatnaRoute — port ${port}`,
        items,
      },
      debug: false,
      copyDir: false,
  ...
  tray.ready().catch((err) => {
    process.stderr.write(`[niyatnaroute][tray] systray2 failed: ${err?.message ?? String(err)}\n`);
  });
