...
export function getBaseUrl(opts = {}) {
  ...
  const envUrl = process.env.NIYATNA_BASE_URL;
...
export async function buildHeaders(opts) {
  ...
  //
  // The active context's scoped token MUST win over the ambient NIYATNA_API_KEY:
  // `omniroute connect <remote>` saves the context's token, but users keep
  // NIYATNA_API_KEY in their shell. The global `--api-key` option is bound to
  // that env var (.env("NIYATNA_API_KEY")), so commands that spread
  ...
  const ambientKey = process.env.NIYATNA_API_KEY || null;
  const explicitKey = opts.apiKey && opts.apiKey !== ambientKey ? opts.apiKey : null;
  let auth = explicitKey;
  if (!auth) {
    try {
      const ctx = await resolveActiveContextAsync(opts.context ?? process.env.NIYATNA_CONTEXT);
  ...
  if (!isLoopbackUrl(destinationUrl)) {
    ...
    const cliToken = opts.cliToken ?? process.env.NIYATNA_CLI_TOKEN ?? (await getCliToken());
...
export async function apiFetch(path, opts = {}) {
  ...
  const timeout =
    opts.timeout ?? (Number.parseInt(process.env.NIYATNA_HTTP_TIMEOUT_MS || "", 10) || 30000);
  const maxAttempts = opts.retry === false ? 1 : (opts.retryMax ?? RETRY_DEFAULTS.maxAttempts);
  const verbose = opts.verbose ?? process.env.NIYATNA_VERBOSE === "1";
