# NiyatnaRoute CLI — Internal Conventions
...
```
niyatnaroute keys add openai sk-xxx
niyatnaroute combo switch fastest
niyatnaroute memory search "react hooks"
```

**Not allowed**:

```
niyatnaroute --add-key openai sk-xxx     # ❌ flag-as-verb
niyatnaroute add-key openai sk-xxx       # ❌ hyphen at the top level
```
...
- Reads base URL from `NIYATNAROUTE_BASE_URL` env or `~/.niyatnaroute/config.json`
  (active profile).
- Injects `Authorization: Bearer ${NIYATNAROUTE_API_KEY}` when available.
- Injects `x-niyatnaroute-cli-token` when applicable (see task 8.12).
...
- Detection order: `--lang` flag → `NIYATNAROUTE_LANG` env → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
- Locale persisted via `config lang set <code>` — saves `NIYATNAROUTE_LANG` to `~/.niyatnaroute/.env`.
