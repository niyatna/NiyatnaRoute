/**
 * niyatnaroute setup-claude — Remote-aware Claude Code profile generator.
 *
 ...
 * fetches the live /v1/models catalog from a (possibly remote) NiyatnaRoute and
 ...
 *
 * Launch a profile with:  niyatnaroute launch --profile <name>
...
export function buildProfileSettings(modelId, baseUrl, cfg) {
  ...
  // NOTE: ANTHROPIC_AUTH_TOKEN is intentionally NOT written here — `niyatnaroute
...
export async function runSetupClaudeCommand(opts = {}) {
  ...
  printHeading("NiyatnaRoute → Claude Code profile generator");
  ...
  if (!dryRun) {
    ...
    console.log("\nTo use a profile:");
    console.log("  niyatnaroute launch --profile <name>     # e.g. niyatnaroute launch --profile glm52");
    console.log(
