...
/**
 * `niyatnaroute connect <host>` — remote mode.
 *
 * Logs into a remote NiyatnaRoute server and saves the result as the active context
...
export async function runConnectCommand(host, opts = {}) {
  ...
  if (!baseUrl) {
    printError("A host is required, e.g. niyatnaroute connect 192.168.0.15");
    return 2;
  }
  ...
  cfg.contexts[name] = {
    ...
    accessToken,
    scope,
    description: `Remote NiyatnaRoute (${host})`,
  };
  ...
  printInfo("Switch back to local with: niyatnaroute contexts use default");
  return 0;
}
...
export function registerConnect(program) {
  program
    ...
    .description(
      t("connect.description") || "Connect to a remote NiyatnaRoute server and enter remote mode"
    )
