...
function readCache() {
  try {
    ...
    if (process.env.NIYATNAROUTE_DEBUG_COMPLETION) {
      console.error("[niyatnaroute completion] readCache failed:", err?.message ?? err);
    }
  }
  return null;
}
...
async function refreshCache(opts = {}) {
  ...
  try {
    ...
    if (process.env.NIYATNAROUTE_DEBUG_COMPLETION) {
      console.error("[niyatnaroute completion] refreshCache failed:", err?.message ?? err);
    }
  }
  ...
  try {
    ...
    if (process.env.NIYATNAROUTE_DEBUG_COMPLETION) {
      console.error("[niyatnaroute completion] writeCache failed:", err?.message ?? err);
    }
  }
  return data;
}
...
function installPath(shell) {
  const home = homedir();
  if (shell === "zsh") return join(home, ".zsh", "completions", "_niyatnaroute");
  if (shell === "fish") return join(home, ".config", "fish", "completions", "niyatnaroute.fish");
  return join(home, ".bash_completion.d", "niyatnaroute");
}

function generateZshScript() {
  return `#compdef niyatnaroute

# NiyatnaRoute zsh completion (dynamic)
_niyatnaroute_get_cache() {
  local key="$1"
  local cache="$HOME/.niyatnaroute/completion-cache.json"
  ...
  if [[ $((now - mtime)) -gt 3600 ]]; then
    niyatnaroute completion refresh --quiet >/dev/null 2>&1
  fi
...
_niyatnaroute() {
  local -a commands
  commands=(
    'serve:Start the NiyatnaRoute server'
    'stop:Stop the server'
    'restart:Restart the server'
    'setup:Configure NiyatnaRoute'
  ...
  case $state in
    ...
    arg)
      case $words[1] in
        combo)
          case $words[2] in
            switch|delete|show)
              local -a combos
              combos=($(_niyatnaroute_get_cache combos))
        ...
        providers|keys)
          case $words[2] in
            add|remove|test)
              local -a providers
              providers=($(_niyatnaroute_get_cache providers))
      ...
      case $state in
        models)
          local -a models
          models=($(_niyatnaroute_get_cache models))
...
compdef _niyatnaroute niyatnaroute
`;
}

function generateBashScript() {
  return `#!/bin/bash
# NiyatnaRoute CLI bash completion (dynamic)
...
complete -F _niyatnaroute niyatnaroute
`;
}

function generateFishScript() {
  return `# NiyatnaRoute CLI fish completion (dynamic)
complete -c niyatnaroute -f
...
for cmd in $commands
  complete -c niyatnaroute -n '__fish_is_nth_token 1' -a $cmd
end

# Subcommands
complete -c niyatnaroute -n '__fish_seen_subcommand_from combo' -a 'list switch create delete show suggest'
complete -c niyatnaroute -n '__fish_seen_subcommand_from keys' -a 'add list remove regenerate revoke reveal usage'
complete -c niyatnaroute -n '__fish_seen_subcommand_from providers' -a 'available list test test-all'
complete -c niyatnaroute -n '__fish_seen_subcommand_from config' -a 'list get set validate contexts'
complete -c niyatnaroute -n '__fish_seen_subcommand_from completion' -a 'zsh bash fish install refresh'
complete -c niyatnaroute -n '__fish_seen_subcommand_from open' -a 'combos providers api-manager cli-tools agents settings logs memory skills evals audit cost resilience'

# Dynamic completions from cache (requires python3)
function __niyatnaroute_cache_get
  set -l key $argv[1]
  set -l cache "$HOME/.niyatnaroute/completion-cache.json"
...
complete -c niyatnaroute -n '__fish_seen_subcommand_from combo; and __fish_seen_subcommand_from switch delete' -a '(__niyatnaroute_cache_get combos)'
complete -c niyatnaroute -l model -a '(__niyatnaroute_cache_get models)'
complete -c niyatnaroute -l combo -a '(__niyatnaroute_cache_get combos)'
`;
}
...
export function registerCompletion(program) {
  ...
  // Backward-compat: `niyatnaroute completion <shell>` (positional arg form)
  comp
    .command("<shell>")
