/**
 * niyatnaroute setup-aider — configure Aider (aider.chat) for NiyatnaRoute.
 *
...
export async function runSetupAiderCommand(opts = {}) {
  ...
  printHeading("NiyatnaRoute → Aider (openai-compatible via LiteLLM)");
...
export function registerSetupAider(program) {
  program
    .command("setup-aider")
    .description("Configure Aider for NiyatnaRoute: write ~/.aider.conf.yml + print the env recipe")
