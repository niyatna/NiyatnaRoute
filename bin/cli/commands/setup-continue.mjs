 ...
 * `apiKey: ${{ secrets.NIYATNA_API_KEY }}` reference (secret never written to
...
const SECRET_REF = "${{ secrets.NIYATNA_API_KEY }}";
...
export async function runSetupContinueCommand(opts = {}) {
  ...
  printInfo("  cn CLI:  export NIYATNA_API_KEY=...   (read from your shell)");
  printInfo("  IDE:     echo 'NIYATNA_API_KEY=...' >> ~/.continue/.env");
