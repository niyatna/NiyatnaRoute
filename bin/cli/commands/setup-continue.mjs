/**
 * niyatnaroute setup-continue — configure Continue (continue.dev) for NiyatnaRoute.
 *
 ...
 * `apiKey: ${{ secrets.NIYATNAROUTE_API_KEY }}` reference (secret never written to
...
const SECRET_REF = "${{ secrets.NIYATNAROUTE_API_KEY }}";
...
/**
 * Merge NiyatnaRoute models into an existing Continue config object: drop any prior
...
export function mergeContinueConfig(existing, newModels, apiBase) {
  ...
  if (!cfg.name) cfg.name = "NiyatnaRoute Config";
...
export async function runSetupContinueCommand(opts = {}) {
  ...
  printHeading("NiyatnaRoute → Continue (config.yaml)");
  ...
  if (dryRun) {
    ...
    printInfo(`[dry-run] ${models.length} NiyatnaRoute model(s) → ${configPath}`);
    return 0;
  }

  mkdirSync(join(configPath, ".."), { recursive: true });
  writeFileSync(configPath, out, "utf8");
  printSuccess(`Wrote ${configPath} (${models.length} NiyatnaRoute models)`);
  printInfo("\nProvide the key (config.yaml references it, not stores it):");
  printInfo("  cn CLI:  export NIYATNAROUTE_API_KEY=...   (read from your shell)");
  printInfo("  IDE:     echo 'NIYATNAROUTE_API_KEY=...' >> ~/.continue/.env");
...
export function registerSetupContinue(program) {
  program
    ...
    .description(
      "Generate ~/.continue/config.yaml (Continue / cn CLI) from the NiyatnaRoute model catalog"
    )
