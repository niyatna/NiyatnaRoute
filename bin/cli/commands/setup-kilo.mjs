/**
 * niyatnaroute setup-kilo — configure Kilo Code to use NiyatnaRoute.
 *
 ...
 * This writes BOTH (matching the NiyatnaRoute dashboard) and prints the UI settings.
 *
...
/** Merge the NiyatnaRoute openai-compatible provider into Kilo's CLI auth.json. */
...
export function buildKiloVscodeSettings(existing, { apiKey, baseUrl, model }) {
  const s = { ...(existing || {}) };
  s["kilocode.customProvider"] = { name: "NiyatnaRoute", baseURL: baseUrl, apiKey: apiKey || "sk_niyatnaroute" };
...
export async function runSetupKiloCommand(opts = {}) {
  ...
  printHeading("NiyatnaRoute → Kilo Code (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(
      JSON.stringify(
        { "openai-compatible": { ...auth["openai-compatible"], apiKey: apiKey ? "set" : "sk_niyatnaroute" } },
        null,
        2
      )
    );
...
export function registerSetupKilo(program) {
  program
    .command("setup-kilo")
    .description(
      "Configure Kilo Code for NiyatnaRoute: write ~/.local/share/kilo/auth.json (CLI) + VS Code kilocode.* settings"
    )
