/**
 * niyatnaroute setup opencode — Wire the bundled @niyatnaroute/opencode-plugin
 * into a local OpenCode install.
 *
 * Closes the gap where `npm install -g niyatnaroute` ships the plugin
 * inside the niyatnaroute package (`@niyatnaroute/opencode-plugin/dist/`) but
 ...
 *   3. Copies the built plugin into `<opencode>/plugins/niyatnaroute/`.
 *   4. Creates or updates `opencode.json` with a single `plugin` entry
 *      pointing at the local copy (so OC ≥1.15 picks it up).
 *   5. Optionally runs `opencode auth login --provider niyatnaroute`
...
// We walk up from this file to find the niyatnaroute package root. The script
// lives at `<niyatnaroute>/bin/cli/commands/setup-open-code.mjs`, so the
...
// The bundled plugin ships at PACKAGE_ROOT/@niyatnaroute/opencode-plugin/
// (see root package.json `files`: ["@niyatnaroute/", ...]). The env override
// exists so tests can point at a fixture without building the real plugin.
const BUNDLED_PLUGIN_DIR =
  process.env.NIYATNAROUTE_OPENCODE_PLUGIN_DIR || join(PACKAGE_ROOT, "@niyatnaroute", "opencode-plugin");
...
/**
 * Locate the bundled @niyatnaroute/opencode-plugin dist. The plugin may be
 * present in two states:
 *
 *   - Built (`dist/index.cjs` + `dist/index.js` exist) — preferred,
 *     ships from a published niyatnaroute tarball after Step 8.8 of
...
function resolveBundledPlugin() {
  if (!existsSync(BUNDLED_PLUGIN_DIR)) {
    throw new Error(
      `Bundled @niyatnaroute/opencode-plugin not found at ${BUNDLED_PLUGIN_DIR}.\n` +
        `This usually means niyatnaroute was installed from a source tree that does not ` +
        `include the workspace package. Try reinstalling niyatnaroute (npm install -g niyatnaroute) ` +
        `or run \`cd @niyatnaroute/opencode-plugin && npm install && npm run build\` from the source repo.`
    );
  }
  ...
  if (!existsSync(esmEntry)) {
    throw new Error(
      `@niyatnaroute/opencode-plugin dist/ not built (looked for ${esmEntry}).\n` +
...
/**
 * Copy the plugin package into `<opencodeConfig>/plugins/niyatnaroute/`. We
 ...
 * resolve the peer deps from the niyatnaroute package's tree, which is
 * unreliable.
 */
function installPluginToOpenCode(pluginInfo, opencodeConfigDir) {
  const targetDir = join(opencodeConfigDir, "plugins", "niyatnaroute");
...
function registerPluginInOpenCodeConfig({
    ...
    try {
      ...
      throw new Error(
        `Failed to parse existing ${configPath}: ${err.message}\n` +
          `Fix or remove the file manually, then re-run \`niyatnaroute setup opencode\`.`
      );
    }
  }
  ...
  const entry = [
    `./plugins/niyatnaroute/dist/index.js`,
    {
      providerId,
      baseURL,
  ...
  // drop a legacy `opencode-niyatnaroute-auth` entry if present — that
  // package is the obsolete predecessor of @niyatnaroute/opencode-plugin
  // and was the root cause of issue #3711.
  const filtered = plugins.filter((p) => {
    if (typeof p === "string") {
      return !p.includes("opencode-niyatnaroute-auth");
    }
    if (Array.isArray(p) && p[1] && typeof p[1] === "object") {
      ...
      if (typeof p[0] === "string" && p[0].includes("opencode-niyatnaroute-auth")) {
        return false;
      }
    }
    return true;
 ...
 *
 * @param {object} opts
 * @param {string} [opts.providerId="niyatnaroute"]
...
export async function runSetupOpenCodeCommand(opts = {}) {
  const providerId = opts.providerId || "niyatnaroute";
  ...
  printHeading("NiyatnaRoute → OpenCode Plugin Setup");
...
/**
 * Register the `niyatnaroute setup opencode` subcommand on the parent
 * `setup` command. Commander builds the doc/help from the chain, so
 * `niyatnaroute setup --help` automatically shows the new subcommand.
 *
...
export function registerSetupOpenCode(setupCommand) {
  setupCommand
    ...
    .description(
      t("setup.opencode") ||
        "Install and register the bundled @niyatnaroute/opencode-plugin with a local OpenCode install"
    )
    .option(
      "--provider-id <id>",
      "OpenCode provider id to register (default: niyatnaroute)",
      "niyatnaroute"
    )
    .option(
      "--base-url <url>",
      "NiyatnaRoute base URL the plugin should talk to (default: active context or http://localhost:20128)"
    )
    .option(
      "--remote <url>",
      "Remote NiyatnaRoute URL, e.g. http://192.168.0.15:20128 (overrides --base-url and the context)"
    )
