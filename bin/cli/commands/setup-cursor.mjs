/**
 * niyatnaroute setup-cursor — guide Cursor to use NiyatnaRoute.
 *
...
export function buildCursorInstructions({ apiBase, models }) {
  const lines = [
    ...
    "  3. Set the OpenAI API Key to your NiyatnaRoute key (NIYATNAROUTE_API_KEY)",
...
export async function runSetupCursorCommand(opts = {}) {
  ...
  printHeading("NiyatnaRoute → Cursor");
...
export function registerSetupCursor(program) {
  program
    ...
    .description("Print the steps to point Cursor at NiyatnaRoute (chat panel; Cursor config is not file-writable)")
