/**
 * niyatnaroute setup-roo — configure Roo Code (RooVeterinaryInc.roo-cline) for NiyatnaRoute.
 *
 ...
 * must support native OpenAI tool-calling (NiyatnaRoute does).
 */
...
export function buildRooImport({ baseUrl, apiKey, model }) {
  return {
    providerProfiles: {
      currentApiConfigName: "NiyatnaRoute",
      apiConfigs: {
        NiyatnaRoute: {
          apiProvider: "openai",
          openAiBaseUrl: baseUrl,
          openAiApiKey: apiKey || "sk_niyatnaroute",
          openAiModelId: model,
...
export async function runSetupRooCommand(opts = {}) {
  ...
  const importPath = opts.importPath ?? opts["import-path"] ?? join(os.homedir(), ".niyatnaroute", "roo-settings.json");
  const vscodePath =
    opts.vscodeSettings ?? opts["vscode-settings"] ?? join(os.homedir(), ".config", "Code", "User", "settings.json");

  printHeading("NiyatnaRoute → Roo Code (OpenAI-compatible)");
  ...
  if (dryRun) {
    console.log(`\n── [dry-run] ${importPath} ──`);
    console.log(JSON.stringify({ ...importDoc, providerProfiles: { ...importDoc.providerProfiles, apiConfigs: { NiyatnaRoute: { ...importDoc.providerProfiles.apiConfigs.NiyatnaRoute, openAiApiKey: apiKey ? "set" : "sk_niyatnaroute" } } } }, null, 2));
  ...
  printInfo(`  API Key:   <your NIYATNAROUTE_API_KEY>`);
...
export function registerSetupRoo(program) {
  program
    .command("setup-roo")
    .description(
      "Configure Roo Code for NiyatnaRoute: write a Roo import JSON + autoImport pointer + print UI steps"
    )
    ...
    .option("--import-path <path>", "Roo import JSON path (default: ~/.niyatnaroute/roo-settings.json)")
