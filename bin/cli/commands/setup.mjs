...
async function resolveProviderInput(opts, prompt, nonInteractive) {
  ...
  if (!apiKey) {
    throw new Error("Provider API key is required. Pass --api-key or NIYATNAROUTE_API_KEY.");
  }
...
export function registerSetup(program) {
  ...
  // Wire up `niyatnaroute setup opencode` subcommand. Kept inside registerSetup
...
export async function runSetupCommand(opts = {}) {
  ...
  try {
    printHeading("NiyatnaRoute Setup");
    const { db, dbPath } = await openNiyatnaRouteDb();
