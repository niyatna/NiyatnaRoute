/**
 * niyatnaroute setup-cline — configure the Cline AI coding agent to use NiyatnaRoute.
 *
 ...
 * writes the CLI-mode files (matching the NiyatnaRoute dashboard) AND prints the
...
export function resolveClineTarget(opts = {}) {
  ...
  if (opts.remote) baseUrl = stripToRoot(opts.remote);
    ...
    try {
      baseUrl = stripToRoot(resolveActiveContext(opts.context ?? process.env.NIYATNAROUTE_CONTEXT)?.baseUrl);
    } catch {
      /* none */
    }
...
/** Merge NiyatnaRoute openai-compatible settings into Cline's globalState (Plan + Act). */
...
export function buildClineSecrets(existing, { apiKey }) {
  return { ...(existing || {}), openAiApiKey: apiKey || "sk_niyatnaroute" };
}
...
export async function runSetupClineCommand(opts = {}) {
  ...
  printHeading("NiyatnaRoute → Cline (OpenAI-compatible)");
  ...
  if (dryRun) {
    ...
    console.log(`\n── [dry-run] ${secPath} ── (openAiApiKey: ${apiKey ? "set" : "sk_niyatnaroute"})`);
  } else {
...
export function registerSetupCline(program) {
  program
    .command("setup-cline")
    .description(
      "Configure Cline for NiyatnaRoute: write ~/.cline/data (CLI mode) + print VS Code extension settings"
    )
