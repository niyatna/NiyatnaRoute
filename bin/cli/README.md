# bin/cli — NiyatnaRoute CLI internals

This directory contains the CLI runtime, helpers, and commands for the `niyatnaroute` binary.
...
├── sqlite.mjs              ← openNiyatnaRouteDb() — DB bootstrap
...
All HTTP calls to the NiyatnaRoute server must go through this wrapper.
...
- `baseUrl` — override base URL (default: `NIYATNAROUTE_BASE_URL` env or `localhost:20128`)
- `apiKey` — override API key (default: `NIYATNAROUTE_API_KEY`)
...
Locale detection order: `NIYATNAROUTE_LANG` → `LC_ALL` → `LC_MESSAGES` → `LANG` → `en`.
...
2. `NIYATNAROUTE_LANG` environment variable
...
```bash
niyatnaroute config lang set pt-BR       # saves to ~/.niyatnaroute/.env
niyatnaroute config lang list            # show all 42 available locales
niyatnaroute config lang get             # show currently active locale
```

**One-time override:**

```bash
niyatnaroute --lang de providers list    # run in German, not persisted
NIYATNAROUTE_LANG=ja niyatnaroute status    # same effect via env
```
