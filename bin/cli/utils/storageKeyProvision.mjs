 ...
 *
 * Purely informational invocations must NOT create `~/.niyatnaroute/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `niyatnaroute --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `niyatnaroute` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `niyatnaroute` runs the default `serve` command → must provision.
