 ...
 *
 * Enforces that every env var referenced in NiyatnaRoute source code appears in
...
const IGNORE_FROM_CODE = new Set([
  ...
  "USERPROFILE",
  "PREFIX",
  // X11 display server — set by the OS/session manager, not NiyatnaRoute config.
  "DISPLAY",
  ...
  // bundler is active — framework-internal. The NiyatnaRoute-facing knob is
  // NIYATNAROUTE_USE_TURBOPACK (scripts/dev/run-next.mjs), which IS documented.
  "TURBOPACK",
  "NODE_TEST_CONTEXT",
  "VITEST",
  // Instruction snippet shown to users (Traffic Inspector HttpProxySnippetCard) — not NiyatnaRoute config.
  ...
  // points at NiyatnaRoute). A downstream client-tool var, not an NiyatnaRoute server
  ...
  // (scripts/check/check-openapi-breaking.mjs) — a build/check signal, not NiyatnaRoute runtime config.
  "BASE_REF",
  // PR body injected by GitHub Actions into the pr-evidence gate (github.event.pull_request.body);
  // a CI-only signal, never an NiyatnaRoute runtime config (Phase 7.10).
  "PR_BODY",
  // CLI machine-id token opt-out (server-side flag; not user-configurable via .env).
  "NIYATNAROUTE_DISABLE_CLI_TOKEN",
  ...
  // ("http://192.168.0.15:20128" / null), never NiyatnaRoute runtime config (#5151).
  "COMBO_LIVE_BASE_URL",
  "COMBO_LIVE_API_KEY",
  ...
  "NIYATNAROUTE_NO_UPDATE_NOTIFIER",
  // Headless CLI execution flag for Electron.
  "NIYATNAROUTE_HEADLESS",
  // Platform / OS detection vars read by CLI environment helper (bin/cli/utils/environment.mjs).
  // These are external signals set by the host OS or cloud provider — not NiyatnaRoute config.
  "CODESPACES",
  ...
  // internals, never NiyatnaRoute runtime config.
  "VERIFY_DEADLINE_S",
  "VERIFY_PORT",
  "VERIFY_VERSION",
  // Doctor diagnostic flags (no runtime behavior yet — placeholders).
  "NIYATNAROUTE_DOCTOR_HOST",
  "NIYATNAROUTE_DOCTOR_LIVENESS_URL",
  "NIYATNAROUTE_PROVIDER_CATALOG_PATH",
  "NIYATNAROUTE_PROVIDER_TEST_MODEL",
  ...
  "NIYATNAROUTE_CLI_SKIP_REPO_ENV",
  ...
  // A dev/ops measurement tool, never NiyatnaRoute runtime config.
  "NIYATNAROUTE_EVAL_CREDENTIALS",
  ...
  "NIYATNAROUTE_INTERNAL_SCHEME",
  // Source typo / placeholder.
  "OMNIROUT",
  // Static config alias path (the canonical var is NIYATNAROUTE_PAYLOAD_RULES_PATH).
  "PAYLOAD_RULES_PATH",
  // Node.js module resolution path — OS/Node internal, not an NiyatnaRoute config var.
  ...
  "NVIDIA_BASE_URL",
  "NVIDIA_MODEL",
  // XDG standard data directory — set by OS/desktop session, not NiyatnaRoute config.
  ...
  "NIYATNAROUTE_OPENCODE_PLUGIN_DIR",
]);
...
const DOC_ONLY_ALLOWLIST = new Set([
  ...
  "NIYATNAROUTE_CRYPT_KEY",
  "NIYATNAROUTE_API_KEY_BASE64",
