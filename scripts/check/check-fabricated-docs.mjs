...
//   3. CLI commands `niyatnaroute ...`     → must exist in bin/cli/commands/ or bin/
...
const ENV_VAR_ALLOWLIST = new Set([
  ...
  "VERBOSE",
  "LOG_LEVEL",
  "PORT", // generic, not NiyatnaRoute-specific
  "DATA_DIR",
  "REQUIRE_API_KEY",
  "NIYATNAROUTE_BUILD_PROFILE", // build-time only
  "NIYATNAROUTE_BUILD_SHA",
  "NIYATNAROUTE_URL", // used by ad-hoc tooling, validated elsewhere
  "NIYATNAROUTE_KEY", // ditto
  ...
  // subprocess — never read via `process.env.X` in NiyatnaRoute's own source, so the
  ...
  "OPENAI_BASE_URL", // env var NiyatnaRoute passes to downstream CLIs (AGENT_PROTOCOLS_GUIDE.md)
...
const ENDPOINT_ALLOWLIST = new Set([
  ...
  // paths on the UPSTREAM service (Claude.ai web, Blackbox), not NiyatnaRoute routes.
...
// Env var helper wrappers used across NiyatnaRoute — envInt(NAME, 5), envBool(NAME),
...
export function buildCodebaseIndex(root = ROOT) {
  ...
  // documented NiyatnaRoute env var (including external-CLI / docker / electron vars
  ...
  // Set of `niyatnaroute <subcommand>` strings that exist in bin/
...
const COARSE_PATTERNS = {
  ...
  // niyatnaroute <verb> <sub> ... — only on the same line, captures first 2 tokens
  cliCmd: /\bniyatnaroute\s+([a-z][a-z0-9-]+)(?:\s+([a-z][a-z0-9-]+))?/g,
...
export function scanDocFile(absPath, index, root = ROOT) {
  ...
  // 3) CLI commands: `niyatnaroute foo bar` — only flag when the line is in
  //    a code-like context (inside backticks or a shell block). Bare prose
  //    like "we use niyatnaroute and..." is not a command claim.
  for (const m of textNoCode.matchAll(COARSE_PATTERNS.cliCmd)) {
    ...
    const isShellLike = /^[ \t]*\$\s|^```sh|^```bash|^```shell|`niyatnaroute/.test(lineText);
    if (!isShellLike) continue;
    if (/example|placeholder|tbd/i.test(lineText)) continue;
    findings.push({
      kind: "cli-cmd",
      value: `niyatnaroute ${sub}`,
      line: ln,
      msg: `niyatnaroute subcommand '${sub}' not registered in bin/`,
    });
  }
  ...
  for (const m of textNoCode.matchAll(COARSE_PATTERNS.fileRef)) {
    ...
    // (`@niyatnaroute/opencode-provider/src/index.ts`). Neither resolves from repo root.
...
export function formatHumanReport(result) {
  ...
  const KIND_LABELS = {
    ...
    "cli-cmd": "niyatnaroute subcommands not registered",
