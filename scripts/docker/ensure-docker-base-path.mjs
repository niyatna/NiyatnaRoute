...
/**
 * Compares NIYATNA_BASE_PATH at container start against BUILD_NIYATNA_BASE_PATH
...
const SENTINEL = "BUILD_NIYATNA_BASE_PATH";
...
export function ensureDockerBasePath(opts = {}) {
  ...
  const runtime = normalizeBasePath(opts.env?.NIYATNA_BASE_PATH ?? process.env.NIYATNA_BASE_PATH);
  ...
  if (!runtime && baked) {
    throw new Error(
      `This OmniRoute image was built for subpath ${baked}, but NIYATNA_BASE_PATH is unset. ` +
        `Set NIYATNA_BASE_PATH=${baked} or rebuild without the build-arg.`
    );
  }
  ...
  try {
    ...
    console.error(
      "[ensure-docker-base-path] Rebuild the image with the same subpath, e.g.\n" +
        "  docker compose build --build-arg NIYATNA_BASE_PATH=/omniroute"
    );
    process.exit(1);
  }
}
