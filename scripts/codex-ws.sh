#!/usr/bin/env bash
# =============================================================================
# codex-ws — roda a OpenAI Codex CLI contra um NiyatnaRoute LOCAL usando o
#            transporte Responses-over-WebSocket (em vez do HTTP da Cloud).
#
# POR QUE ESTE WRAPPER EXISTE
# ---------------------------
# O NiyatnaRoute expõe um proxy WebSocket para a API de Responses do Codex em
...
#     account"). É preciso mandar o id "puro" -> "gpt-5.5". (O NiyatnaRoute, no
...
#   NIYATNAROUTE_WS_BASE   base URL do NiyatnaRoute local   (default abaixo)
#   NIYATNAROUTE_WS_MODEL  modelo codex (id puro)        (default "gpt-5.5")
#   NIYATNAROUTE_LOCAL_KEY API key (qualquer valor se REQUIRE_API_KEY=false)
...
NIYATNAROUTE_WS_BASE="${NIYATNAROUTE_WS_BASE:-http://127.0.0.1:20128/v1}"  # base do NiyatnaRoute local
NIYATNAROUTE_WS_MODEL="${NIYATNAROUTE_WS_MODEL:-gpt-5.5}"                  # id PURO (sem "codex/")
...
export NIYATNAROUTE_LOCAL_KEY="${NIYATNAROUTE_LOCAL_KEY:-local}"
...
# Gerado por codex-ws.sh — config isolada para o WS local do NiyatnaRoute.
model = "$NIYATNAROUTE_WS_MODEL"
model_provider = "niyatnaroute-local"

[model_providers.niyatnaroute-local]
name = "NiyatnaRoute Local (WS)"
base_url = "$NIYATNAROUTE_WS_BASE"   # a URL WebSocket é derivada desta base pela CLI
...
env_key = "NIYATNAROUTE_LOCAL_KEY"   # a CLI lê a key Bearer desta env var
...
overrides=(-c model="$NIYATNAROUTE_WS_MODEL" -c model_provider="niyatnaroute-local")
