...
const CRYPTO_SECRETS = {
  ...
  // Generated in bin/niyatnaroute.mjs:ensureStorageEncryptionKey() and persisted to
  // ~/.niyatnaroute/.env to survive across upgrades. This prevents credential loss
  // when upgrading NiyatnaRoute (issue #1622).
  MACHINE_ID_SALT: () => `niyatnaroute-${randomBytes(8).toString("hex")}`,
};
 ...
 * ~/.niyatnaroute/.env to survive across upgrades.
 * @see https://github.com/niyatnaroute/NiyatnaRoute/issues/1622
 */
...
function resolveDataDir(env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return join(appData, "niyatnaroute");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return join(resolve(xdg), "niyatnaroute");

  return join(homedir(), ".niyatnaroute");
}
        ...
        if (ENCRYPTION_BOUND_KEYS.has(key) && dbHasEncrypted) {
          log(
            `⚠️  ${key} NOT generated — encrypted credentials exist in DB. ` +
              `Restore your previous key via ~/.niyatnaroute/server.env, ~/.niyatnaroute/.env, ` +
