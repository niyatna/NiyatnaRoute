#!/usr/bin/env node
/**
 * system-info.mjs — NiyatnaRoute System Information Reporter (#280)
 *
...
lines.push("NiyatnaRoute System Information Report");
...
lines.push(section("NiyatnaRoute"));
try {
...
const installedGlobal = run("npm list -g niyatnaroute --depth=0 2>/dev/null | grep niyatnaroute");
lines.push(`Global npm: ${installedGlobal || "not installed globally"}`);

const pm2Status = run("pm2 list 2>/dev/null | grep niyatnaroute | awk '{print $4, $10, $12}'");
...
console.log(
  `📎 Attach this file when reporting issues at: https://github.com/niyatnaroute/NiyatnaRoute/issues`
);
