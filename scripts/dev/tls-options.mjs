...
//
// Pure, dependency-light helpers for NiyatnaRoute's opt-in native HTTPS/TLS serving
 ...
 *
 * Reads `NIYATNAROUTE_TLS_CERT` and `NIYATNAROUTE_TLS_KEY` (filesystem paths). Returns
...
export function resolveTlsOptions(
  ...
  const certPath = typeof env?.NIYATNAROUTE_TLS_CERT === "string" ? env.NIYATNAROUTE_TLS_CERT.trim() : "";
  const keyPath = typeof env?.NIYATNAROUTE_TLS_KEY === "string" ? env.NIYATNAROUTE_TLS_KEY.trim() : "";
  ...
  if (!certPath || !keyPath) {
    warn(
      `[niyatnaroute][tls] HTTPS not enabled: both NIYATNAROUTE_TLS_CERT and NIYATNAROUTE_TLS_KEY ` +
  ...
  try {
    ...
    warn(
      `[niyatnaroute][tls] HTTPS not enabled: could not read TLS cert/key ` +
