...
const bootstrapMode = process.env.NIYATNAROUTE_E2E_BOOTSTRAP_MODE || "auth";
const playwrightPassword =
  process.env.NIYATNAROUTE_E2E_PASSWORD || process.env.INITIAL_PASSWORD || "niyatnaroute-e2e-password";
const testServerEnv = {
  ...
  NEXT_PUBLIC_NIYATNAROUTE_E2E_MODE: process.env.NEXT_PUBLIC_NIYATNAROUTE_E2E_MODE || "1",
  NIYATNAROUTE_DISABLE_BACKGROUND_SERVICES:
    process.env.NIYATNAROUTE_DISABLE_BACKGROUND_SERVICES || "true",
  NIYATNAROUTE_DISABLE_TOKEN_HEALTHCHECK: process.env.NIYATNAROUTE_DISABLE_TOKEN_HEALTHCHECK || "true",
  NIYATNAROUTE_DISABLE_LOCAL_HEALTHCHECK: process.env.NIYATNAROUTE_DISABLE_LOCAL_HEALTHCHECK || "true",
  NIYATNAROUTE_HIDE_HEALTHCHECK_LOGS: process.env.NIYATNAROUTE_HIDE_HEALTHCHECK_LOGS || "true",
  ...(bootstrapMode === "open"
    ? {
        INITIAL_PASSWORD: "",
        NIYATNAROUTE_E2E_PASSWORD: "",
        NIYATNAROUTE_API_KEY: "",
      }
    : {
        INITIAL_PASSWORD: playwrightPassword,
        NIYATNAROUTE_E2E_PASSWORD: playwrightPassword,
      }),
  ...(process.env.NIYATNAROUTE_USE_TURBOPACK
    ? {
        NIYATNAROUTE_USE_TURBOPACK: process.env.NIYATNAROUTE_USE_TURBOPACK,
      }
    : {}),
};
...
export function shouldUseWebpackForPlaywrightDev({ mode, env }) {
  ...
  return mode === "dev" && env.NIYATNAROUTE_USE_TURBOPACK === "0";
}
...
async function runBuildForStart() {
  ...
  if (process.env.NIYATNAROUTE_PLAYWRIGHT_SKIP_BUILD === "1") return;
