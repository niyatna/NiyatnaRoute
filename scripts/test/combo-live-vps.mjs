 ...
 *
 * Phase-3 VPS HTTP scenario driver for NiyatnaRoute combo routing.
...
// Known approximate input cost ($/M tokens) from NiyatnaRoute's default-pricing constants.
...
//
// NiyatnaRoute's sortModelsByCost uses getPricingForModel which merges
...
async function scenarioCostOptimized(healthy) {
  ...
  try {
    ...
    // Both direct match and provider-substring match are accepted since NiyatnaRoute
...
//   404, NiyatnaRoute calls recordProviderCooldown(providerA, undefined) — a provider-
...
//   1. SSH to read STORAGE_ENCRYPTION_KEY from /root/.niyatnaroute/.env
//   2. Encrypt a wrong API key using scryptSync(key,"niyatnaroute-field-encryption-v1",32)+AES-256-GCM
...
// ---------------------------------------------------------------------------
async function main() {
  console.log("=== NiyatnaRoute combo-live-vps scenario driver ===");
