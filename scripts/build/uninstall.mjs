...
const uninstallAlreadyInProgress =
  process.env.NIYATNAROUTE_SKIP_UNINSTALL_HOOK === "1" ||
  process.env.npm_lifecycle_event === "uninstall";

console.log("🛑 NiyatnaRoute Uninstaller");
console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");

// 1. Stop PM2 process if it exists
try {
  console.log("Stopping and removing background PM2 processes...");
  execSync("pm2 delete niyatnaroute 2>/dev/null", { stdio: "ignore" });
} catch {
...
const dataDir = process.env.DATA_DIR || path.join(os.homedir(), ".niyatnaroute");
...
if (uninstallAlreadyInProgress) {
  ...
  try {
    execSync("npm uninstall -g niyatnaroute", {
      stdio: "inherit",
      env: {
        ...process.env,
        NIYATNAROUTE_SKIP_UNINSTALL_HOOK: "1",
      },
    });
    console.log("\n✅ NiyatnaRoute has been successfully uninstalled from your system.");
    if (!fullUninstall) {
