...
/**
 * NiyatnaRoute — Prepublish Build Script
 *
...
console.log("🔨 NiyatnaRoute — Building for npm publish...\n");
...
const cliSrcFile = join(ROOT, "bin", "niyatnaroute.ts");
const cliDestFile = join(ROOT, "bin", "niyatnaroute.mjs");
...
// ── Step 8.8: Build @niyatnaroute/opencode-plugin ──────────────
// The plugin ships bundled inside the niyatnaroute npm package (see root
// package.json "files": ["@niyatnaroute/", ...]). Its built `dist/` MUST be
// present in the publish tarball so `niyatnaroute setup opencode` can copy it
...
const opencodePluginSrc = join(ROOT, "@niyatnaroute", "opencode-plugin");
...
if (existsSync(opencodePluginSrc) && existsSync(join(opencodePluginSrc, "package.json"))) {
  ...
  if (!pluginAlreadyBuilt) {
    console.log("\n  🔨 Building @niyatnaroute/opencode-plugin (tsup)...");
    try {
      ...
      console.log("  ✅ @niyatnaroute/opencode-plugin bundled to @niyatnaroute/opencode-plugin/dist/");
    } catch (err: any) {
      console.error("  ❌ Failed to build @niyatnaroute/opencode-plugin:", err.message);
      console.error("     The published package would be missing the plugin dist.");
      console.error(
        "     Run `cd @niyatnaroute/opencode-plugin && npm install && npm run build` to debug."
      );
      process.exit(1);
    }
  } else {
    console.log("  ✅ @niyatnaroute/opencode-plugin dist/ already present (skipping rebuild)");
  }
  ...
  if (existsSync(pluginNodeModules)) {
    ...
    console.log("  🧹 Removed @niyatnaroute/opencode-plugin/node_modules (hard link guard)");
  }
} else {
  console.log("  ⏭️  @niyatnaroute/opencode-plugin not found in workspace (skipping build)");
}
