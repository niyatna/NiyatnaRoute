#!/usr/bin/env node
/**
 * NiyatnaRoute — Zero-Config Bootstrap
 *
 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.niyatnaroute/.env -> ./.env)
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by NiyatnaRoute bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  if (needsPersist) {
    merged.NIYATNAROUTE_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "niyatnaroute-field-encryption-v1", 32)
              const legacySalt = "niyatnaroute-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.niyatnaroute/server.env or ~/.niyatnaroute/.env,"
                );
                log(
                  "   or run: niyatnaroute reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }
