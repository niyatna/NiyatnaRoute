#!/usr/bin/env node
/**
 * NiyatnaRoute — Docs translation pipeline (hash-based, incremental).
 *
 ...
 *   NIYATNAROUTE_TRANSLATION_API_URL     e.g. https://cloud.niyatnaroute.dev/v1
 *   NIYATNAROUTE_TRANSLATION_API_KEY     bearer token (kept out of logs)
 *   NIYATNAROUTE_TRANSLATION_MODEL       e.g. cx/gpt-5.6-sol
 *   NIYATNAROUTE_TRANSLATION_TIMEOUT_MS  optional, default 60000
 *   NIYATNAROUTE_TRANSLATION_CONCURRENCY optional, default 4
 */
...
function backendConfig() {
  const apiUrl = requireEnv("NIYATNAROUTE_TRANSLATION_API_URL").replace(/\/$/, "");
  const apiKey = requireEnv("NIYATNAROUTE_TRANSLATION_API_KEY");
  const model = requireEnv("NIYATNAROUTE_TRANSLATION_MODEL");
  const timeoutMs = Number(process.env.NIYATNAROUTE_TRANSLATION_TIMEOUT_MS || 60000);
...
async function main() {
  ...
  if (!opts.dryRun) {
    ...
    else backend.concurrency = Number(process.env.NIYATNAROUTE_TRANSLATION_CONCURRENCY || 4);
    logInfo(
      `backend: ${backend.apiUrl} (model=${backend.model}, concurrency=${backend.concurrency}, timeout=${backend.timeoutMs}ms)`
    );
  } else {
    const apiUrl = (process.env.NIYATNAROUTE_TRANSLATION_API_URL || "").replace(/\/$/, "");
